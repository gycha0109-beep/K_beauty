...
export function buildAnalysisResultRow({
  ...
  return {
    ...
    summary,
    routine_am: Array.isArray(result?.morning)
      ? result.morning.map((item) => String(item || "").trim()).filter(Boolean).slice(0, 3)
      : result?.amFocus
        ? [String(result.amFocus).trim()].filter(Boolean)
        : [],
    routine_pm: Array.isArray(result?.night)
      ? result.night.map((item) => String(item || "").trim()).filter(Boolean).slice(0, 3)
      : result?.pmFocus
        ? [String(result.pmFocus).trim()].filter(Boolean)
        : [],
