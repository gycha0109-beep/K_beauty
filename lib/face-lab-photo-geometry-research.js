...
export const STRUCTURAL_MEASUREMENT_VERSION =
  'face-space-structural-measurement-v0';
export const STRUCTURAL_MEASUREMENT_SEMANTICS_VERSION =
...
export function validatePhotoGeometryManifest(manifest) {
  ...
  if (manifest?.semanticContractVersion !== STRUCTURAL_MEASUREMENT_SEMANTICS_VERSION) {
    errors.push('semantic_contract_version_invalid');
  }
...
export function buildPhotoGeometryPacketFromMetricLandmarks({
  sampleId,
  sourceVersion,
  landmarks,
  faceCount = 1,
  ...
  if (!manifestValidation.ok) {
    throw new Error('photo_geometry_manifest_invalid:' + manifestValidation.errors.join(','));
  }
  if (!nonEmpty(sampleId)) throw new Error('photo_geometry_sample_id_missing');
  if (sourceVersion !== manifest.providerVersion) {
    throw new Error('photo_geometry_source_version_mismatch');
  }
  if (faceCount !== 1) throw new Error('photo_geometry_single_face_required');
  if (sourceImagePersisted !== false) {
    throw new Error('photo_geometry_source_image_persistence_forbidden');
  }
  if (!Array.isArray(landmarks) || landmarks.length !== manifest.landmarkTopology) {
    throw new Error('photo_geometry_landmark_topology_mismatch');
  }

  const selected = {};
  for (const anchorName of REQUIRED_ANCHORS) {
    const index = manifest.anchorMap[anchorName];
    const candidate = landmarks[index];
    if (!point(candidate)) {
      throw new Error('photo_geometry_landmark_missing:' + anchorName + ':' + index);
    }
  ...
  return {
    schemaVersion: PHOTO_GEOMETRY_PACKET_SCHEMA_VERSION,
    sampleId: sampleId.trim(),
    source: manifest.provider,
    sourceVersion,
    coordinateSpace: manifest.coordinateSpace,
    landmarkTopology: manifest.landmarkTopology,
    faceCount: 1,
    poseNormalized: true,
    metricGeometry: true,
    sourceImagePersisted: false,
...
export function measurePhotoGeometry(packet, manifest) {
  ...
  const record = {
    ...
    semanticsVersion: STRUCTURAL_MEASUREMENT_SEMANTICS_VERSION,
