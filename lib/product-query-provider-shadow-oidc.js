...
export const DATA_AI4_GITHUB_OIDC_ISSUER =
  "https://token.actions.githubusercontent.com";
export const DATA_AI4_RUNTIME_PROBE_AUDIENCE =
  "urn:bejewely:data-ai4:provider-shadow";
export const DATA_AI4_RUNTIME_PROBE_REPOSITORY =
  "gycha0109-beep/K_beauty";
export const DATA_AI4_RUNTIME_PROBE_REPOSITORY_ID = "1205065704";
export const DATA_AI4_RUNTIME_PROBE_WORKFLOW_PATH =
  ".github/workflows/data-ai4-provider-shadow.yml";

const OIDC_METADATA_URL =
  `${DATA_AI4_GITHUB_OIDC_ISSUER}/.well-known/openid-configuration`;
...
function validateClaims(payload, { expectedDeploymentSha, expectedGitRef }, nowSeconds) {
  ...
  const expectedWorkflowRef =
    `${DATA_AI4_RUNTIME_PROBE_REPOSITORY}/${DATA_AI4_RUNTIME_PROBE_WORKFLOW_PATH}@${expectedRef}`;

  if (payload?.iss !== DATA_AI4_GITHUB_OIDC_ISSUER) return "invalid_issuer";
  if (!audienceMatches(payload?.aud, DATA_AI4_RUNTIME_PROBE_AUDIENCE)) {
    return "invalid_audience";
  }
  if (payload?.repository !== DATA_AI4_RUNTIME_PROBE_REPOSITORY) {
    return "invalid_repository";
  }
  if (
    String(payload?.repository_id || "") !== DATA_AI4_RUNTIME_PROBE_REPOSITORY_ID
  ) {
...
export function getDataAi4BearerTokenFromRequest(request) {
  const authorization = request?.headers?.get?.("authorization") || "";
  const match = authorization.match(/^Bearer\s+([^\s]+)$/i);
  return match ? match[1] : null;
}

export async function verifyDataAi4GitHubActionsOidcToken(
  token,
  {
  ...
  try {
    ...
    if (metadata?.issuer !== DATA_AI4_GITHUB_OIDC_ISSUER) {
