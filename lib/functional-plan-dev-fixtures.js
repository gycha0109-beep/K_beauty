...
const FINDING_FIXTURE_PRODUCTS = {
  cleanser: {
    id: "finding-dev-cleanser",
    brand: "Dev Fixture",
    name: "Low pH Gel Cleanser",
    category: "cleanser",
    ingredient_signals: {
      functional: [
        { label: "skin hydration", count: 12 },
        { label: "skin protection", count: 3 },
        { label: "exfoliation", count: 1 }
      ]
    },
    irritation_risk: "low",
  ...
  porePad: {
    id: "finding-dev-pore-pad",
    brand: "Dev Fixture",
    name: "Pore Clear Toner Pad",
    category: "toner_pad",
    ingredient_signals: {
      functional: [
        { label: "exfoliation", count: 5 },
        { label: "soothing/astringent", count: 2 }
      ]
    },
  ...
  hydrationCream: {
    id: "finding-dev-hydration-cream",
    brand: "Dev Fixture",
    name: "Hydration Barrier Cream",
    category: "moisturizer_cream",
    ingredient_signals: {
      functional: [
        { label: "skin hydration", count: 9 },
...
function selectedFixture(category, product) {
  return {
    status: "selected",
    category,
...
const RAW_FUNCTIONAL_PLAN_DEV_SCENARIOS = [
  makeScenario({
...
const CURRENT_PRODUCTS_BY_SCENARIO = {
  "start-unknown": {
    selections: [
      { status: "not_in_db", category: "treatment", name: "User-entered product" },
      { status: "not_using", category: "sunscreen" },
      { status: "unanswered", category: "toner_essence" }
    ]
  },
  "start-mismatch": {
    selections: [selectedFixture("cleanser", FINDING_FIXTURE_PRODUCTS.cleanser)]
  },
  optimize: {
    selections: [selectedFixture("treatment", FINDING_FIXTURE_PRODUCTS.poreSerum)]
  },
  consolidate: {
    selections: [
      selectedFixture("treatment", FINDING_FIXTURE_PRODUCTS.poreSerum),
      selectedFixture("toner_pad", FINDING_FIXTURE_PRODUCTS.porePad)
    ]
  },
  "next-optimize": {
    selections: [selectedFixture("moisturizer_cream", FINDING_FIXTURE_PRODUCTS.hydrationCream)]
  }
};

export const FUNCTIONAL_PLAN_DEV_SCENARIOS = RAW_FUNCTIONAL_PLAN_DEV_SCENARIOS.map((scenario) => ({
  ...scenario,
  currentProducts: CURRENT_PRODUCTS_BY_SCENARIO[scenario.id] || scenario.currentProducts
}));
