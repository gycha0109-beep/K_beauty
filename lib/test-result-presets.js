import { PRODUCT_DB } from "@/lib/backups/product-db.backup";
import { buildFallbackAnalysis, buildRuleBasedPlan } from "@/lib/fallback-analysis";
import { buildRecommendationBundleFromProducts } from "@/lib/recommendation";
...
export function getTestResultPreset(id) {
  ...
  const recommendation = buildRecommendationBundleFromProducts(
    preset.submission?.form || {},
    PRODUCT_DB,
    { includeAlternative: true }
  );
  const ruleBasedPlan = buildRuleBasedPlan(preset.submission?.form || {});
  const generatedResult = buildFallbackAnalysis(
    preset.submission?.form || {},
    recommendation,
    ruleBasedPlan
  );
  ...
  return {
    ...
    result: {
      ...
      meta: {
        source: "preset",
        notice: "프리셋 화면도 현재 추천 엔진 기준으로 다시 계산된 결과입니다."
      }
    }
  };
}
