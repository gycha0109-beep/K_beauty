...
export const COLLECTION_LEDGER_SCHEMA_VERSION = 'face-lab-reverse-archetype-collection-ledger-v1';
...
export function validateReverseArchetypeRawCandidate(candidate, {
  ...
  const resultUrl = parseHttpUrl(candidate?.resultUrl);
  const landingUrl = parseHttpUrl(candidate?.landingUrl);
  const imageUrl = parseHttpUrl(candidate?.imageUrl);

  if (!resultUrl || !landingUrl || !imageUrl) {
    errors.push('provenance_url_invalid');
  }

  if (!nonEmpty(candidate?.originDomain)) {
    errors.push('origin_domain_missing');
  } else if (
    landingUrl &&
    normalizeHostname(candidate.originDomain) !== normalizeHostname(landingUrl.hostname)
  ) {
    errors.push('origin_domain_landing_url_mismatch');
  }
...
export function buildReverseArchetypeCollectionCoverage(batches, {
  ...
  for (const batch of batchList) {
    ...
    if (batchIds.has(batch?.batchId)) {
      errors.push('duplicate_batch_id:' + (batch?.batchId || 'unknown'));
    }
    batchIds.add(batch?.batchId);

    for (const record of Array.isArray(batch?.records) ? batch.records : []) {
      if (candidateIds.has(record?.candidateId)) {
        errors.push('duplicate_candidate_id_across_batches:' + (record?.candidateId || 'unknown'));
      }
      candidateIds.add(record?.candidateId);
    }
...
export function validateReverseArchetypeCollectionLedger(ledger, {
  sourceManifest,
  queryManifest
} = {}) {
  const errors = [];

  if (!isObject(ledger) || ledger.schemaVersion !== COLLECTION_LEDGER_SCHEMA_VERSION) {
    errors.push('schema_version_invalid');
  }
  if (ledger?.runFamily !== sourceManifest?.runFamily || ledger?.runFamily !== queryManifest?.runFamily) {
    errors.push('run_family_mismatch');
  }
  if (!RUN_ID_PATTERN.test(ledger?.runId || '')) errors.push('run_id_invalid');
  if (!['open', 'sealed'].includes(ledger?.status)) errors.push('status_invalid');
  if (!nonEmpty(ledger?.createdAt) || Number.isNaN(Date.parse(ledger.createdAt))) {
    errors.push('created_at_invalid');
  }
  if (!Array.isArray(ledger?.batches)) errors.push('batches_invalid');

  for (const batch of ledger?.batches || []) {
    if (batch?.runId !== ledger.runId) errors.push('batch_run_id_mismatch');
    if (batch?.runFamily !== ledger.runFamily) errors.push('batch_run_family_mismatch');
  }

  let coverage = null;
  if (Array.isArray(ledger?.batches)) {
    coverage = buildReverseArchetypeCollectionCoverage(ledger.batches, {
      sourceManifest,
      queryManifest
    });
    for (const error of coverage.errors) errors.push('coverage:' + error);
    if (ledger.status === 'sealed' && coverage.pendingBatches !== 0) {
      errors.push('sealed_ledger_has_pending_batches');
    }
  }
