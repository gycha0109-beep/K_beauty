...
export const G3_GITHUB_OIDC_ISSUER = "https://token.actions.githubusercontent.com";
export const G3_RUNTIME_PROBE_AUDIENCE = "urn:bejewely:v21-admission-g3:runtime-probe";
export const G3_RUNTIME_PROBE_REPOSITORY = "gycha0109-beep/K_beauty";
export const G3_RUNTIME_PROBE_REPOSITORY_ID = "1205065704";
export const G3_RUNTIME_PROBE_WORKFLOW_PATH =
  ".github/workflows/v21-admission-g3-production-candidate-gate.yml";

const OIDC_METADATA_URL = `${G3_GITHUB_OIDC_ISSUER}/.well-known/openid-configuration`;
...
function validateClaims(payload, { expectedDeploymentSha, expectedGitRef }, nowSeconds) {
  ...
  const expectedWorkflowRef = `${G3_RUNTIME_PROBE_REPOSITORY}/${G3_RUNTIME_PROBE_WORKFLOW_PATH}@${expectedRef}`;
  if (payload?.iss !== G3_GITHUB_OIDC_ISSUER) return "invalid_issuer";
  if (!audienceMatches(payload?.aud, G3_RUNTIME_PROBE_AUDIENCE)) return "invalid_audience";
  if (payload?.repository !== G3_RUNTIME_PROBE_REPOSITORY) return "invalid_repository";
  if (String(payload?.repository_id || "") !== G3_RUNTIME_PROBE_REPOSITORY_ID) return "invalid_repository_id";
...
export function getG3BearerTokenFromRequest(request) {
  const authorization = request?.headers?.get?.("authorization") || "";
  const match = authorization.match(/^Bearer\s+([^\s]+)$/i);
  return match ? match[1] : null;
}

export async function verifyG3GitHubActionsOidcToken(
  token,
  {
  ...
  try {
    ...
    if (metadata?.issuer !== G3_GITHUB_OIDC_ISSUER) {
  ...
  const claimError = validateClaims(
    payload,
    { expectedDeploymentSha: String(expectedDeploymentSha || ""), expectedGitRef },
