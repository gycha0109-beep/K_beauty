...
function combineSupport(fields, inputFeatures, options = {}) {
  ...
  if (!available.length) {
    ...
    const unsupportedOnly = statuses.length > 0 && statuses.every((status) => status === "unsupported");
    return createDerived(
      unsupportedOnly ? "unsupported" : "unavailable",
      null,
      inputFeatures,
      [],
      unsupportedOnly ? "unsupported_from_single_photo" : "input_unavailable"
    );
  }
...
function deriveSurfaceStress(atomicSkin) {
  ...
  const textureAvailable = [dry, flaking].some((field) => field?.status === "available");
  if (!textureAvailable) {
    return createDerived(
      "insufficient_evidence",
      null,
      ["visibleDryTexture", "visibleFlaking", "visibleRedness"],
      fieldEvidence(redness),
