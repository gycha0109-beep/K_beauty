...
const ACTUAL_RUNTIME_EVIDENCE_VERSION = "2026-07-14.phase46.2";
...
function isLocalActualRuntimeEvidenceEnabled(envLike = {}) {
  return isLocalRecommendationEvidenceEnabled(envLike) &&
...
const EXPECTED_RUNTIME_RECEIVER_GROUPS = new Map([
...
const RUNTIME_EXPOSURE_GROUPS = ["hidden_candidate", "collapsed_candidate", "insufficient_evidence_candidate"];
const RUNTIME_VIOLATION_KEYS = [
  "highRiskCollapsed",
...
function runtimeRecommendationEvidence(recommendationResult = {}) {
  return recommendationIdEvidence(recommendationResult);
}

function runtimeExcludedCandidates(policyRuntime = {}) {
  return Object.fromEntries(RUNTIME_EXPOSURE_GROUPS.map((group) => [
    group,
    (Array.isArray(policyRuntime.exposureRows) ? policyRuntime.exposureRows : [])
      .filter((row) => row?.appliedExposureGroup === group)
      .map((row) => ({
        productId: normalizeId(row.productId),
        receiverDecision: String(row.receiverDecision || ""),
        reasonCode: String(row.rejectionReason || "")
      }))
  ]));
}

function runtimeViolationCounts(value = {}) {
  return Object.fromEntries(RUNTIME_VIOLATION_KEYS.map((key) => [key, Number(value?.[key] || 0)]));
}

function validIdList(value) {
  return Array.isArray(value) && value.every((id) => normalizeId(id));
}

export function validateLocalActualRuntimeEvidence(evidence = {}, { comparisonRunId = null, condition = null } = {}) {
  const errors = [];
  if (!evidence || typeof evidence !== "object" || Array.isArray(evidence)) return { valid: false, errors: ["evidence_not_object"] };
  if (evidence.evidenceType !== "local_actual_candidate_policy_execution") errors.push("invalid_evidence_type");
  if (evidence.schemaVersion !== ACTUAL_RUNTIME_EVIDENCE_VERSION) errors.push("invalid_schema_version");
  if (!UUID_PATTERN.test(String(evidence.comparisonRunId || ""))) errors.push("invalid_comparison_run_id");
  if (comparisonRunId && evidence.comparisonRunId !== comparisonRunId) errors.push("comparison_run_id_mismatch");
  if (!RECOMMENDATION_CONDITIONS.has(evidence.condition) || (condition && evidence.condition !== condition)) errors.push("invalid_condition");
  if (typeof evidence.runtimeEnabled !== "boolean" || typeof evidence.runtimeExecuted !== "boolean" || typeof evidence.runtimeConnected !== "boolean") errors.push("invalid_runtime_state");
  if (evidence.runtimeExecuted && (!evidence.runtimeEnabled || !evidence.runtimeConnected)) errors.push("runtime_state_inconsistent");
  if (evidence.condition === "off" && (evidence.runtimeEnabled || evidence.runtimeExecuted || evidence.runtimeConnected)) errors.push("flag_off_runtime_enabled");
  if (evidence.condition === "on" && (!evidence.runtimeEnabled || !evidence.runtimeExecuted || !evidence.runtimeConnected)) errors.push("flag_on_runtime_not_executed");
  if (!Number.isInteger(evidence.unexpectedReceiverCount) || evidence.unexpectedReceiverCount < 0) errors.push("invalid_unexpected_receiver_count");
  for (const field of ["inputCandidateIdsInOrder", "visibleCandidateIdsInOrder"]) {
    if (!validIdList(evidence[field])) errors.push(`invalid_${field}`);
  }
  if (!evidence.excludedCandidates || typeof evidence.excludedCandidates !== "object") errors.push("missing_excluded_candidates");
  for (const group of RUNTIME_EXPOSURE_GROUPS) {
    const rows = evidence.excludedCandidates?.[group];
    if (!Array.isArray(rows) || rows.some((row) => {
      const expectedReceiver = EXPECTED_RUNTIME_RECEIVER_GROUPS.get(row?.receiverDecision) === group;
      const failClosedReceiver = group === "hidden_candidate" && row?.reasonCode === "unexpected_receiver_result";
      return !normalizeId(row?.productId) || !row?.reasonCode || (!expectedReceiver && !failClosedReceiver);
    })) {
      errors.push(`invalid_${group}`);
    }
  }
  const unexpectedRows = (evidence.excludedCandidates?.hidden_candidate || [])
    .filter((row) => row?.reasonCode === "unexpected_receiver_result").length;
  if (evidence.unexpectedReceiverCount !== unexpectedRows) errors.push("unexpected_receiver_count_mismatch");
  if (!evidence.safetyViolationCounts || RUNTIME_VIOLATION_KEYS.some((key) => !Number.isInteger(evidence.safetyViolationCounts[key]) || evidence.safetyViolationCounts[key] < 0)) errors.push("invalid_safety_violation_counts");
  if (!evidence.recommendation || !validateLocalShadowRecommendationEvidence({
    ...
    forbiddenFieldDetected: evidence.forbiddenFieldDetected
  }, { comparisonRunId: evidence.comparisonRunId, condition: evidence.condition }).valid) errors.push("invalid_recommendation_snapshot");
  if (evidence.forbiddenFieldDetected !== false || hasForbiddenRecommendationField(evidence)) errors.push("forbidden_field_present");
  return { valid: errors.length === 0, errors };
}
...
export async function writeLocalActualRuntimeEvidence({
  policyRuntime = null,
  ...
  if (!isLocalActualRuntimeEvidenceEnabled(envLike)) {
    return { attempted: false, written: false, skipped: true, skipReason: "local_actual_runtime_evidence_disabled" };
  }

  const target = resolveLocalRecommendationEvidenceDirectory(envLike);
  const runtimeEnabled =
    envLike?.ENABLE_EVALUATOR_BOUNDARY_CANDIDATE_POLICY_RUNTIME === "1" &&
    envLike?.DISABLE_EVALUATOR_BOUNDARY_CANDIDATE_POLICY_RUNTIME !== "1";
  const inputCandidateIdsInOrder = (Array.isArray(candidateSource?.products) ? candidateSource.products : [])
    .map((product) => {
      const value = product?.id || product?.productId || product?.product_id;
      return value == null ? null : normalizeId(String(value));
    })
    .filter(Boolean);
  const runtimeExecuted = runtimeEnabled && policyRuntime?.runtimeConnected === true;
  const evidence = {
    evidenceType: "local_actual_candidate_policy_execution",
    schemaVersion: ACTUAL_RUNTIME_EVIDENCE_VERSION,
    comparisonRunId: target?.comparisonRunId || null,
    condition: target?.condition || null,
    runtimeEnabled,
    runtimeExecuted,
    runtimeConnected: runtimeExecuted,
    unexpectedReceiverCount: runtimeExecuted ? Number(policyRuntime?.unexpectedReceiverCount || 0) : 0,
    inputCandidateIdsInOrder,
    visibleCandidateIdsInOrder: runtimeExecuted
      ? (Array.isArray(policyRuntime.visibleCandidateIds) ? policyRuntime.visibleCandidateIds : [])
      : inputCandidateIdsInOrder,
    excludedCandidates: runtimeExecuted
      ? runtimeExcludedCandidates(policyRuntime)
      : Object.fromEntries(RUNTIME_EXPOSURE_GROUPS.map((group) => [group, []])),
    safetyViolationCounts: runtimeViolationCounts(policyRuntime?.violationCounts),
    recommendation: runtimeRecommendationEvidence(recommendationResult),
    forbiddenFieldDetected: false
  };
  if (!target || !validateLocalActualRuntimeEvidence(evidence, { comparisonRunId: target.comparisonRunId, condition: target.condition }).valid) {
    return { attempted: false, written: false, skipped: true, skipReason: "local_actual_runtime_evidence_contract_invalid" };
  }

  const directory = path.join(target.directory, "runtime");
  const filePath = path.join(directory, `runtime-flag-${target.condition}.json`);
  try {
    await fileSystem.mkdir(directory, { recursive: true });
    await fileSystem.writeFile(filePath, `${JSON.stringify(evidence, null, 2)}\n`, { encoding: "utf8", flag: "wx" });
    return { attempted: true, written: true, skipped: false, skipReason: null, filePath: path.relative(process.cwd(), filePath).replace(/\\/g, "/") };
  } catch {
    return { attempted: true, written: false, skipped: false, skipReason: "local_actual_runtime_evidence_write_failed" };
  }
}
