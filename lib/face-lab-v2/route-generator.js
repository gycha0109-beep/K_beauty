export const STYLE_ROUTE_GENERATOR_VERSION = "face-lab-style-route-v2";
...
function createRoute(strategy, actions, locale, targetStyle = null, activeTargetAxes = new Set()) {
  ...
  let adjustedActions = actions.map((item) => {
    if (
      item.domain === "makeup" &&
      makeupIntensity === "light" &&
      ["moderate", "strong"].includes(item.strength)
    ) {
      return { ...item, strength: "light" };
    }
    return item;
  });
  ...
  if (changeTolerance === "minimal") {
    adjustedActions = adjustedActions
      ...
      .map((item) => ({
        ...item,
        strength: ["moderate", "strong"].includes(item.strength) ? "light" : item.strength
      }));
  }
