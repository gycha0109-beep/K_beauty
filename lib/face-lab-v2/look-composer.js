export const LOOK_COMPOSER_VERSION = "face-lab-look-composer-v2";
...
export function buildLookComposer({
  ...
  const routeDomains = new Set(Array.isArray(route.domains) ? route.domains : []);
  const conflicts = Array.isArray(styleDelta?.conflicts)
    ? styleDelta.conflicts
        .filter((item) => {
          ...
          return !domains.length || domains.some((domain) => routeDomains.has(domain));
        })
        .map((item, index) => ({
