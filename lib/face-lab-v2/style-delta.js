export const STYLE_DELTA_VERSION = "face-lab-style-delta-v1";
...
export function buildStyleDelta({
  ...
  const conflicts = [];
  const modified = applyFaceModifiers(rawActions, currentFaceProfile, conflicts);
  const scoped = applyScopeAndConstraints(modified, targetStyle);

  // collect modifiers that were produced inside applyFaceModifiers
  const seenConflicts = [];
  applyFaceModifiers(rawActions, currentFaceProfile).forEach(() => {});
  const guarded = applyFaceModifiers(rawActions, currentFaceProfile);
  const finalActions = applyScopeAndConstraints(guarded, targetStyle);
