...
import {
  TARGET_FINDER_CANDIDATE_SET_VERSION,
  buildTargetFinderResult
} from "./target-finder.js";
...
export function normalizeFaceLabV2TargetFinderResult(value) {
  ...
  if (Array.isArray(value.rounds) && value.rounds.length) {
    const recomputed = buildTargetFinderResult(value.rounds);
    return {
      candidateSetVersion: TARGET_FINDER_CANDIDATE_SET_VERSION,
      rounds: recomputed.rounds,
      candidateLabels: recomputed.candidateLabels,
      estimatedVector: recomputed.estimatedVector,
...
export function normalizeFaceLabV2SurveyAnswers(value) {
  ...
  const dailyMinutes = [5, 15, 30].includes(Number(lifestyle.dailyMinutes))
    ...
    : 15;
  const budgetBand = ["low", "standard", "flexible"].includes(lifestyle.budgetBand)
    ? lifestyle.budgetBand
    : "standard";
  const maintenanceTolerance = ["low", "medium", "high"].includes(lifestyle.maintenanceTolerance)
    ? lifestyle.maintenanceTolerance
    : "medium";
