...
export const FACE_LAB_RESULT_PRESENTATION_VERSION = "face-lab-result-presentation-v1";
...
function extraKoDomainLines(key, result) {
  if (key === "hair") {
    return unique(result?.hair?.value?.avoidOrModerate || []);
  }
  if (key === "grooming") {
    return unique(result?.grooming?.value?.maintenancePlan || []);
  }
  return [];
}
...
function mapExecution(result, locale, copy, selectedRoute) {
  ...
  const domains = orderedKeys.map((key) => {
    ...
    const actions = unique([
      ...(grouped.get(key) || []),
      ...(locale === "ko" ? extraKoDomainLines(key, result) : [])
    ]);
...
function conflictText(item, locale) {
  ...
  const key = item?.type || item?.conflictId;
...
export function buildFaceLabV2ResultPresentation(result, { locale = "ko" } = {}) {
  ...
  return {
    ...
    look: selectedRoute ? {
      ...
      routeTitle: safeText(selectedRoute.title, resolvedLocale, selectedRoute.routeId),
      summary: safeText(selectedRoute.summary, resolvedLocale, actionText(selectedRoute.actions?.[0], resolvedLocale)),
      why: safeText(selectedRoute.whyThisRoute, resolvedLocale, ""),
      pieces: execution.domains.map((domain) => ({
