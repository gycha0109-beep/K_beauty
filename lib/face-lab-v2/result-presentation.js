...
export const FACE_LAB_RESULT_PRESENTATION_VERSION = "face-lab-result-presentation-v6";

const COPY = {
  ko: {
    currentTitle: "지금 인상을 만드는 요소",
    currentFallback: "사진에서 확인 가능한 얼굴 특징만 스타일 판단에 사용했습니다.",
    unavailableTitle: "이번 분석으로는 스타일 경로를 만들기 어렵습니다",
    unavailableBody: "현재 얼굴 특징을 스타일 판단에 사용할 근거가 충분하지 않아 임의로 추천하지 않았습니다. 얼굴 분석이 가능한 결과에서 다시 확인해 주세요.",
    boundedTitle: "선택한 조건 안에서는 실행할 변화가 충분하지 않습니다",
    boundedBody: "현재 얼굴과 추구미는 확인했지만 선택한 스타일링 범위 안에서 안전하게 제안할 변화가 부족합니다. 추구미나 적용 범위를 조정해 주세요.",
    limitedEvidenceTitle: "일부 판단은 확인 가능한 얼굴 특징만 사용했습니다",
    limitedEvidenceBody: "사진에서 확인 가능한 근거만으로 경로를 구성했습니다. 근거가 부족한 특징은 추천에 사용하지 않았으므로 실행안은 현재 표시된 범위 안에서 참고해 주세요.",
    ...
    tradeoff: {
      ...
      daily_time_constraint_5: "하루 5분 안에서는 이 경로가 손이 많이 갑니다.",
      daily_time_constraint_15: "하루 15분 안에서는 이 경로가 손이 조금 더 갑니다.",
  ...
  en: {
    ...
    unavailableTitle: "This analysis cannot support a style route yet",
    unavailableBody: "There is not enough usable face evidence to make a styling recommendation safely, so Face Lab does not guess. Try again from a result with usable face analysis.",
    boundedTitle: "There is not enough actionable change within your current setup",
    boundedBody: "Your current face and target are available, but the selected styling scope does not leave enough supported changes. Adjust the target or styling scope to continue.",
    limitedEvidenceTitle: "Some decisions use only the face evidence available in this photo",
    limitedEvidenceBody: "This route uses only facial cues supported by the current image. Features without enough evidence are excluded, so treat the execution plan as bounded to what is shown here.",
    ...
    tradeoff: {
      ...
      daily_time_constraint_5: "This route is demanding for a 5-minute daily routine.",
      daily_time_constraint_15: "This route may take a little more effort within a 15-minute daily routine.",
...
function mapRoute(route, locale, copy, selectedRouteId) {
  ...
  return {
    ...
    tradeoffs,
    targetFit: safeText(
      route.targetFit?.explanation,
      locale,
      route.targetFit?.status === "supported"
...
export function buildFaceLabV2ResultPresentation(result, { locale = "ko" } = {}) {
  ...
  const routeCards = (result.routes?.routes || []).map((route) =>
    mapRoute(route, resolvedLocale, copy, result.routes?.selectedRouteId || null)
  );
  const notice = !routeCards.length
    ? {
        kind: "bounded",
        title: copy.boundedTitle,
        body: copy.boundedBody
      }
    : result.status === "partial"
      ? {
          kind: "limited_evidence",
          title: copy.limitedEvidenceTitle,
          body: copy.limitedEvidenceBody
        }
      : null;
