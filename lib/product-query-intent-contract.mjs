...
export const PRODUCT_QUERY_INTENT_JSON_SCHEMA = Object.freeze({
  ...
  properties: {
    ...
    category: {
      ...nullableEnum(PRODUCT_QUERY_CATEGORIES),
      description:
        "Primary requested product family. Product-family nouns own category first and must not be duplicated into texture from the same claim."
    },
    skin_type: {
      ...nullableEnum(PRODUCT_QUERY_SKIN_TYPES),
      description:
        "Explicit skin type or plain skin-type tendency. A single degree-bearing sensitivity claim belongs to sensitivity instead of being duplicated here."
    },
    concerns: {
      type: "array",
      description:
        "Explicit skin problems or reduction goals. A plain skin-type claim such as oily skin belongs to skin_type and must not become oiliness unless the query separately frames oiliness, shine, or sebum as a concern.",
      items: {
        type: "string",
    ...
    sensitivity: {
      ...nullableEnum(PRODUCT_QUERY_SENSITIVITY),
      description:
        "Explicit degree of general skin reactivity. Do not infer a degree from sensitive skin type alone, and do not duplicate one degree-bearing claim into skin_type."
    },
    texture: {
      ...nullableEnum(PRODUCT_QUERY_TEXTURES),
      description:
        "Independent texture or form preference only. A product-family noun such as cream or gel that already establishes category must not also populate texture."
    },
    ...
    tone_up_wanted: {
      ...nullableBoolean(),
      description:
        "Explicit tone-up preference. If the query contains incompatible positive and negative evidence for this dimension, this field must be null and the conflict must remain unresolved."
    },
    ...
    unresolved_terms: {
      type: "array",
      description:
        "Explicit concepts that cannot safely map to supported structured fields, including unsupported concepts and same-dimension conflicts. Pure ambiguity stays empty.",
      items: {
        type: "string",
        maxLength: 80
      },
