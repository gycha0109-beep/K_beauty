...
function normalizeReferenceCorpusEvidence(referenceCorpusSummary) {
  if (referenceCorpusSummary === undefined || referenceCorpusSummary === null) {
    return {
      referenceCorpusSummaryPresent: false,
  ...
  if (
    ...
    referenceCorpusSummary.authority?.productionAuthority !== false ||
    referenceCorpusSummary.authority?.normalizationAuthority !== false ||
    referenceCorpusSummary.authority?.referenceStatisticsAuthority !== false ||
    referenceCorpusSummary.authority?.thresholdAuthority !== false ||
    !isObject(referenceCorpusSummary.evidenceState)
  ) {
    throw new Error("face_space_normalization_reference_corpus_summary_invalid");
  }
  ...
  return {
    referenceCorpusSummaryPresent: true,
...
function normalizeEvidenceState(evidence = {}, referenceCorpusSummary) {
  const corpusEvidence = normalizeReferenceCorpusEvidence(referenceCorpusSummary);
  const realPoseEvidenceKind =
    nonEmpty(evidence.realPoseEvidenceKind) ? evidence.realPoseEvidenceKind : null;
  const realExpressionEvidenceKind =
    nonEmpty(evidence.realExpressionEvidenceKind)
      ? evidence.realExpressionEvidenceKind
      : null;

  return {
    referenceCorpusSummaryPresent:
      corpusEvidence.referenceCorpusSummaryPresent,
    ...
    realPoseStability:
      evidence.realPoseStability === true &&
...
export function evaluateFaceSpaceNormalizationReadiness({
  ...
  const state = normalizeEvidenceState(evidence, referenceCorpusSummary);
