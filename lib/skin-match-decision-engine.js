...
function applyPhotoWeights(scoreCard, photoAnalysis) {
  if (photoAnalysis?.imageEligibility?.skinAnalysisEligible !== true) {
    return;
  }
...
export async function buildSkinMatchDecisionBundle(input, options = {}) {
  ...
  const imageEligibility = photoAnalysis?.imageEligibility || null;
  const photoEvidenceState = photoAnalysis?.photoEvidenceState || null;
  ...
  if (evaluatorBoundaryPolicyRuntimeRequested()) {
    ...
    if (runtimeControl.runtimeEnabled) {
      ...
      try {
        const [policyRuntimeModule, goalPolicyModule, surveyContractModule] = await Promise.all([
          import("@/lib/evaluator-boundary-policy-runtime"),
          import("@/lib/functional-goal-policy"),
          import("@/lib/survey-input-contract")
        ]);
        const surveyContract = surveyContractModule.buildSurveyInputContract(input, {
          source: "skin_match_decision_boundary_policy_runtime"
        });
        const goalPolicy = goalPolicyModule.resolveFunctionalGoalPolicy({
          surveyContract,
  ...
  const premiumReport = {
    ...
    ...(imageEligibility ? { imageEligibility } : {}),
  ...
  if (options.includeEvaluatorBoundaryPolicyShadow) {
    const [policyShadowModule, goalPolicyModule, surveyContractModule] = await Promise.all([
      import("@/lib/evaluator-boundary-policy-shadow"),
      import("@/lib/functional-goal-policy"),
      import("@/lib/survey-input-contract")
    ]);
    const surveyContract = surveyContractModule.buildSurveyInputContract(input, {
      source: "skin_match_decision_boundary_policy_shadow"
    });
    const goalPolicy = goalPolicyModule.resolveFunctionalGoalPolicy({
      surveyContract,
      freeResultPriority: { axis: priority.axis },
