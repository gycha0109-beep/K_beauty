...
const CATEGORY_SLOT_LABELS = {
  ...
  toner_pad: { ko: "토너 패드", en: "Toner Pad" },
  serum_ampoule: { ko: "세럼 · 앰플", en: "Serum / Ampoule" },
...
function isCalmingSerum(product) {
  return (
    getProductSlot(product?.category) === "serum_ampoule" &&
...
function getTopCategorySlot(priorityAxis, answers, scoreCard) {
  switch (priorityAxis) {
    ...
    case "acne":
      return answers.sensitivity === "high" ? "serum_ampoule" : "cleanser";
    case "oiliness":
      return answers.outdoorExposure ? "sunscreen" : "cleanser";
    case "uneven_tone":
      return scoreCard.uv.total >= 12 ? "sunscreen" : "serum_ampoule";
    default:
      return "serum_ampoule";
  }
}
...
function getEnvironmentAdjustment(product, answers, scoreCard) {
  ...
  if (exposures.includes("mask") && (slot === "serum_ampoule" || slot === "moisturizer") && isLowIrritation(product)) {
    total += 4;
    reasons.push("mask-calming");
  }

  if (exposures.includes("humidity") && slot === "toner_pad" && scoreCard.pores.total >= 12) {
    total += 4;
...
function getHeroBoost(product, answers, scoreCard, targetSlot) {
  ...
  if (poresHigh && oilHigh && answers.sensitivity === "low" && slot === "toner_pad") {
    total += 12;
...
function getHardPenalty(product, answers, scoreCard) {
  ...
  if (scoreCard.barrier.total >= 18 && slot === "toner_pad" && !isLowIrritation(product)) {
    total -= 16;
...
function buildDecisionProduct(product, answers, scoreCard, priorityAxis, targetSlot, locale = "ko") {
  ...
  return {
    ...
    reason: buildDefaultReason(product, answers, scoreCard, priorityAxis, hardPenalty.reasons, locale),
    comparison_reason: "",
    decision_meta: {
...
function buildPremiumRoutine(topPick, supportingProducts, amFocus, pmFocus, locale = "ko") {
  const morning = [amFocus];
  const night = [pmFocus];
...
export async function buildSkinMatchDecisionBundle(input, options = {}) {
  ...
  const premiumRoutine = buildPremiumRoutine(resolvedTopPick, supportingProducts, amFocus, pmFocus, locale);
  const premiumReport = {
  ...
  return {
    ...
    premiumReport,
    morning: amFocus ? [amFocus] : [],
    night: pmFocus ? [pmFocus] : [],
    avoid: warnings,
    scoring: {
