...
function getProductSlot(category) {
  return isMoisturizerCategory(category) ? "moisturizer" : SLOT_ALIAS[category] || category || "";
}
...
function buildRoleBasedSupportingProducts({
  ...
  const sameConcernProduct =
    pickProduct((product) => getProductSlot(product.category) === targetSlot && productMatchesConcern(product, priorityAxis)) ||
    pickProduct((product) => getProductSlot(product.category) === targetSlot) ||
  ...
  const lowIrritationProduct =
    pickProduct((product) => getProductSlot(product.category) !== targetSlot && isLowIrritationProduct(product)) ||
...
function buildAlternativeReason(product, topPick, answers, scoreCard, priorityAxis, locale = "ko") {
  ...
  const productSlot = getProductSlot(product.category);
  const topPickSlot = getProductSlot(topPick?.category);
...
function pickRoutineProduct(slots = [], { topPick, pool = [], usedIds = new Set() } = {}) {
  const normalizedSlots = slots.map((slot) => SLOT_ALIAS[slot] || slot);
  const topPickSlot = getProductSlot(topPick?.category);
  ...
  const product = pool.find((item) => {
    ...
    return normalizedSlots.includes(getProductSlot(item.category));
  });
...
function buildRoutineStep(order, definition, context) {
  ...
  const productRole = product ? getProductSlot(product.category) : definition.slots[0];
