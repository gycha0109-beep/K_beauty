...
function getKoreanCategoryName(product) {
  switch (product?.category) {
    ...
    case "ampoule":
      return "앰플";
    default: {
      switch (getProductSlot(product?.category)) {
        case "cleanser":
          return "클렌저";
        case "toner_essence":
          return "토너/에센스";
...
function buildKoreanCategoryAwareReason(
  ...
  if (slot === "sunscreen") {
    ...
    if (hasOutdoor || priorityAxis === "uv") {
      if (highOil) {
        leadSentence = "야외 노출이 있고 유분 신호도 보여, 선크림은 자외선 차단과 산뜻한 마무리를 함께 보는 쪽이 맞습니다.";
      } else if (sensitiveState) {
        leadSentence = "야외 노출이 있고 예민함도 잡혀, 선크림은 자외선 차단과 낮은 자극 부담을 함께 보는 쪽이 중요합니다.";
      } else {
        leadSentence = "야외 노출이 있는 편이라 아침에는 자외선 차단을 먼저 안정적으로 잡는 게 중요합니다.";
      }
    } else if (highOil) {
      leadSentence = "오후에 유분이 빠르게 올라오는 편이라, 선크림도 끈적임이 덜한 쪽을 우선하는 편이 맞습니다.";
    } else if (highDry) {
      leadSentence = "건조감이 잡혀 있어, 선크림도 지나치게 뽀송하게 마르는 쪽보다는 편안한 쪽이 먼저입니다.";
    } else if (sensitiveState) {
      leadSentence = "예민함과 붉은기가 함께 잡힌 상태라, 선크림도 자극 부담이 낮은 쪽이 더 중요합니다.";
    } else {
      leadSentence = "선크림 단계는 낮 동안 무너지지 않게 이어지는 사용감이 먼저입니다.";
    }

    let fitSentence = "";

    if (highOil) {
      fitSentence = `이 제품은 ${finishLabel} 쪽이라 유분이 빠르게 올라오는 날에도 부담이 덜합니다.`;
    } else if (highDry) {
      fitSentence = "이 제품은 지나치게 매트하게 마르지 않는 쪽이라 당김이 있는 날에도 비교적 쓰기 편합니다.";
    } else if (sensitiveState) {
      fitSentence = "이 제품은 자극 부담이 낮은 편이라 예민함이나 붉은기가 있는 날에도 무리가 덜합니다.";
    } else {
      fitSentence = `이 제품은 ${textureLabel}과 ${finishLabel} 쪽이라 데일리로 이어가기 편한 편입니다.`;
    }

    if (answers.whiteCastHate && (product?.white_cast === "none" || product?.white_cast === "low")) {
      fitSentence += " 백탁 부담도 낮은 편이라 꺼내기 쉽습니다.";
    } else if (answers.makeupUse && product?.pilling_risk === "low") {
      fitSentence += " 메이크업 전에도 밀림 부담이 비교적 적은 편입니다.";
    }
  ...
  if (slot === "cleanser") {
    const leadSentence = sensitiveState
      ? "세안 뒤 당김이나 예민함이 같이 잡혀, 세정력만 강한 클렌저보다 부담이 덜한 쪽이 먼저입니다."
      : highOil || highPores || acneState
        ? "유분과 막힘 신호가 같이 보여, 세안 단계에서 잔여감과 번들거림을 먼저 정리하는 쪽이 맞습니다."
        : highDry
          ? "세안 뒤 당김이 잡혀 있어, 과하게 벗겨내지 않는 클렌저부터 맞추는 편이 좋습니다."
          : `${withTopicParticle(categoryName)} 현재 고민을 정리하는 첫 단계로 가장 무난합니다.`;
    const fitSentence = sensitiveState
      ? "이 제품은 비교적 순한 세정감이라 필요한 정리감은 주되 과하게 뽀드득해지지 않게 가기 좋습니다."
      : highOil || acneState
        ? "이 제품은 잔여감이 무겁지 않은 쪽이라 번들거림과 막힘 부담을 줄이기 좋습니다."
        : highDry
          ? "이 제품은 세안 뒤 건조감을 덜 남기는 쪽이라 데일리로 이어가기 편합니다."
          : `이 제품은 ${textureLabel} 쪽이라 첫 단계 부담을 크게 남기지 않는 편입니다.`;

    return `${leadSentence} ${fitSentence}`.trim();
  }

  if (slot === "toner_essence") {
    const leadSentence = highPores || priorityAxis === "pores"
      ? "모공과 결 정돈이 우선이라, 토너 단계에서 표면 흐름을 한 번 정리하는 편이 맞습니다."
      : sensitiveState
        ? "예민한 편이라 토너 단계도 자극을 크게 남기지 않으면서 필요한 정돈만 주는 쪽이 중요합니다."
        : `${withTopicParticle(categoryName)} 현재 루틴에 가볍게 붙이기 좋은 보조 단계입니다.`;
    const fitSentence = sensitiveState
      ? "이 제품은 표면 결을 정리하는 데 맞지만, 예민한 날에는 매일보다는 간격을 두고 쓰는 편이 안전합니다."
      : "이 제품은 표면 결과 모공 흐름을 가볍게 정리하기 좋고, 현재 조건에서 단계 부담도 큰 편은 아닙니다.";
  ...
  if (slot === "serum") {
    const leadSentence = sensitiveState
      ? "예민함이나 붉은기가 같이 잡혀, 세럼 단계에서 진정과 컨디션 회복을 먼저 챙기는 편이 맞습니다."
      : acneState || priorityAxis === "redness"
        ? "트러블과 붉은기 축이 같이 올라와, 세럼 단계에서 자극 부담을 낮추면서 바로 잡는 쪽이 중요합니다."
        : highDry
          ? "건조감이 잡혀 있어, 세럼 단계에서도 수분과 편안함을 먼저 이어주는 편이 좋습니다."
          : `${withTopicParticle(categoryName)} 현재 고민을 가장 직접적으로 다루는 중심 단계입니다.`;
    const fitSentence = sensitiveState || acneState || priorityAxis === "redness"
      ? "이 제품은 현재 조건에서 진정 쪽에 무게가 실려 있어 과하게 자극적으로 밀지 않고 붙이기 좋습니다."
      : highDry
        ? "이 제품은 수분감을 얇게 보태는 쪽이라 건조감이 올라오는 날에도 무겁지 않게 이어가기 좋습니다."
        : `이 제품은 ${textureLabel} 쪽이라 메인 고민을 건드리면서도 루틴이 무겁게 흐르지 않게 잡아줍니다.`;
  ...
  if (slot === "moisturizer") {
    const leadSentence = sensitiveState || priorityAxis === "barrier"
      ? "장벽과 예민함이 함께 잡혀 있어, 보습 단계에서는 편안함이 오래 남는 쪽을 먼저 보는 편이 맞습니다."
      : highDry || priorityAxis === "dehydration"
        ? "세안 뒤 당김이 이어지는 편이라, 보습 단계에서 수분이 쉽게 끊기지 않게 잡아주는 쪽이 중요합니다."
        : hatesHeavy
          ? "보습이 필요해도 무거운 마무리는 피하고 싶어, 답답하지 않은 보습제를 먼저 보는 편이 맞습니다."
          : `${withTopicParticle(categoryName)} 루틴 마무리를 흔들리지 않게 잡아주는 단계입니다.`;
    const fitSentence = sensitiveState
      ? "이 제품은 과하게 답답하지 않으면서 편안함을 유지하기 좋아 현재 조건에 무리가 덜합니다."
      : highDry
        ? "이 제품은 수분감을 붙들어 주는 쪽이라 건조감이 올라오는 날에도 마무리가 급하게 마르지 않습니다."
        : hatesHeavy || highOil
          ? "이 제품은 무겁게 눌러앉는 편은 아니라 보습이 필요해도 마무리 부담을 크게 남기지 않습니다."
          : `이 제품은 ${finishLabel} 쪽이라 마무리 단계를 안정적으로 이어가기 좋습니다.`;

    return `${leadSentence} ${fitSentence}`.trim();
  }

  return `${withTopicParticle(categoryName)} 현재 고민을 정리하는 데 가장 먼저 맞는 축입니다. 이 제품은 ${textureLabel}과 ${finishLabel} 쪽이라 지금 조건에서 부담을 크게 남기지 않는 선택입니다.`;
}
