...
async function buildCanonicalCandidateSafetyContext({
  answers,
  scoreCard,
  priority,
  ...
  const [sharedContextModule, functionalPolicyModule, candidateSafetyModule] =
    await Promise.all([
      import("@/lib/shared-skin-decision-context"),
      import("@/lib/functional-policy"),
      import("@/lib/candidate-policy-runtime-safety")
    ]);
  const { context: sharedContext } =
    sharedContextModule.buildSharedSkinDecisionContext({
      freeResult: {
        priority: {
          axis: priority.axis,
      ...
      source: "skin_match_candidate_policy_runtime_safety"
    });
  const functionalPolicy = functionalPolicyModule.buildFunctionalPolicy({
    locale,
    answers: sharedContext.survey.answers,
    priorityAxis: sharedContext.skinState.priorityAxis,
    scoreCard,
    safetyState: sharedContext.safetyState,
    productExposureState: sharedContext.productExposureState,
    sharedContext
  });
  return candidateSafetyModule.buildCandidatePolicyRuntimeSafetyContext({
    sharedContext,
    functionalPolicy,
...
export async function buildSkinMatchDecisionBundle(input, options = {}) {
  ...
  let candidateSafetyContextPromise = null;
  const resolveCandidateSafetyContext = () => {
    if (!candidateSafetyContextPromise) {
      candidateSafetyContextPromise = buildCanonicalCandidateSafetyContext({
        answers,
        scoreCard,
        priority,
        currentProductsReport,
    ...
    return candidateSafetyContextPromise;
  };
