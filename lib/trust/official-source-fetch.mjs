import { buildOfficialProductSemanticObservationV1, stableSemanticJsonV1 } from "./official-source-semantic-adapter.mjs";
...
async function readBoundedBytes(response, maxBytes = MAX_RESPONSE_BYTES) {
...
export async function fetchOfficialSitemapBytes(rawUrl, fetchImpl = fetch) {
  let url = await assertSafeOfficialUrl(rawUrl);
  for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount += 1) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
    ...
    try {
      response = await fetchImpl(url, {
        ...
        headers: {
          ...
          accept: "application/xml,text/xml,text/plain;q=0.9,application/xhtml+xml;q=0.8,*/*;q=0.1",
        },
      });
    } catch (error) {
      clearTimeout(timer);
      const wrapped = new Error("TRANSIENT_FAILURE:sitemap_fetch_error");
      wrapped.cause = error;
      throw wrapped;
    }
    clearTimeout(timer);
    ...
    if ([301, 302, 303, 307, 308].includes(response.status)) {
      const location = response.headers.get("location");
      if (!location || redirectCount === MAX_REDIRECTS) throw new Error("SOURCE_BLOCKED:sitemap_redirect_limit");
      url = await assertSafeOfficialUrl(new URL(location, url).toString());
      continue;
    }
    if (TRANSIENT_HTTP.has(response.status) || response.status >= 500) {
      const error = new Error(`TRANSIENT_FAILURE:sitemap_http_${response.status}`);
      error.retryAfterSeconds = response.status === 429 ? 900 : 300;
      throw error;
    }
    if (!response.ok) throw new Error(`SOURCE_BLOCKED:sitemap_http_${response.status}`);
    ...
    if (!supported) throw new Error("SOURCE_BLOCKED:unsupported_sitemap_content_type");

    const bytes = await readBoundedBytes(response);
    return { bytes, finalUrl: url.toString(), contentType };
  }
  throw new Error("SOURCE_BLOCKED:sitemap_redirect_limit");
}

export async function fetchOfficialAssetBytes(rawUrl, fetchImpl = fetch) {
  let url = await assertSafeOfficialUrl(rawUrl);
  for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount += 1) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
    ...
    try {
      ...
      clearTimeout(timer);
      const wrapped = new Error("TRANSIENT_FAILURE:asset_fetch_error");
      wrapped.cause = error;
      throw wrapped;
    }
    clearTimeout(timer);
    ...
    if ([301, 302, 303, 307, 308].includes(response.status)) {
      const location = response.headers.get("location");
      if (!location || redirectCount === MAX_REDIRECTS) throw new Error("SOURCE_BLOCKED:asset_redirect_limit");
      url = await assertSafeOfficialUrl(new URL(location, url).toString());
      continue;
    }
    if (TRANSIENT_HTTP.has(response.status) || response.status >= 500) {
      const error = new Error(`TRANSIENT_FAILURE:asset_http_${response.status}`);
      error.retryAfterSeconds = response.status === 429 ? 900 : 300;
      throw error;
    }
    if (!response.ok) throw new Error(`SOURCE_BLOCKED:asset_http_${response.status}`);

    const contentType = (response.headers.get("content-type") || "").toLowerCase();
    if (!contentType.startsWith("image/")) {
      throw new Error("SOURCE_BLOCKED:unsupported_asset_content_type");
    }
    const bytes = await readBoundedBytes(response, MAX_ASSET_BYTES);
    return { bytes, finalUrl: url.toString(), contentType };
  }
  throw new Error("SOURCE_BLOCKED:asset_redirect_limit");
}

export function digestOfficialClaimAsset(bytes) {
  return {
    digest: sha256Hex(bytes),
    digestBasis: "official-claim-asset-bytes-v1",
    adapterKey: "official-claim-asset",
    adapterVersion: "v1",
...
export function digestOfficialContent(bytes, adapterKey = "live-page-bytes", adapterVersion = "v1", context = {}) {
  ...
  if (adapterKey === "official-product-semantic" && adapterVersion === "v1") {
    const observation = buildOfficialProductSemanticObservationV1(bytes, context);
    const canonical = stableSemanticJsonV1(observation);
    return {
      digest: sha256Hex(Buffer.from(canonical, "utf8")),
      digestBasis: "canonical-official-product-semantics-v1",
      adapterKey,
      adapterVersion,
      canonicalLength: Buffer.byteLength(canonical, "utf8"),
