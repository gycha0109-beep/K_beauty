...
import { preflightOfficialSourceRelocation } from "./official-source-relocation-preflight.mjs";
...
export function buildOfficialSourceRelocationConfirmationRequest(preflight, source) {
  ...
  const canonicalPreflight = preflightOfficialSourceRelocation(source);
  requireCondition(
    canonicalPreflight.status === "READY_FOR_ADMIN_RELOCATION_CONFIRMATION",
    "TRUST_PHASE8H_CONFIRMATION_SOURCE_PREFLIGHT_NOT_READY",
  );
  requireCondition(
    canonicalPreflight.prestate_digest === preflight.prestate_digest,
    "TRUST_PHASE8H_CONFIRMATION_PRESTATE_DIGEST_MISMATCH",
  );
  requireCondition(
    canonicalPreflight.relocation_plan_digest === preflight.relocation_plan_digest,
    "TRUST_PHASE8H_CONFIRMATION_PLAN_DIGEST_MISMATCH",
  );
  requireCondition(
    canonicalPreflight.replacement_external_id === preflight.replacement_external_id,
