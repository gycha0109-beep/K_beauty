import {
  PRODUCT_RUNTIME_SHA, STOP_CONDITIONS, SHA40, deepFreeze, exactKeys,
  hasForbiddenKey, nonNegative, stopMap, unique, validIso,
...
import {
  NORMALIZED_DEPLOYMENT_SCHEMA, READ_ONLY_ADAPTER_SCHEMA
} from "./candidate-exposure-policy-read-only-hosted-adapter.js";
import {
  buildMatrix, validateTelemetry
} from "./candidate-exposure-policy-hosted-execution.js";

export const HOSTED_EVIDENCE_SCHEMA_V2 =
  "candidate-exposure-policy-hosted-execution-evidence-v2";
...
const ADAPTER_KEYS = [
  "contract", "routeReadiness", "getDeploymentMetadata", "probeAnalyze",
  "cleanup", "implementationDigest"
];
const CONTRACT_KEYS = [
  "schemaVersion", "deploymentMutationAllowed", "environmentReadAllowed",
  "environmentMutationAllowed", "runtimeLogReadAllowed",
  "bypassMutationAllowed", "productionAllowed", "automaticRetryAllowed",
  "maxDeploymentMetadataReads", "maxAnalyzeRequests", "perRequestTimeoutMs",
  "maxResponseBytes"
];
const PROBE_KEYS = [
  ...
  "snapshotFingerprintMatch", "candidateOrderMatch",
  "unexpectedDivergenceCount", "unclassifiedDivergenceCount",
  "shadowExceptionCount", "fallbackCount", "invalidContextCount",
  "candidateLevelTelemetryDetected", "productionOrProjectConfigurationChange"
];
...
function safeAdapter(value) {
  ...
  return exactKeys(value, ADAPTER_KEYS) && exactKeys(c, CONTRACT_KEYS) &&
    c.schemaVersion === READ_ONLY_ADAPTER_SCHEMA &&
    c.deploymentMutationAllowed === false && c.environmentReadAllowed === false &&
    c.environmentMutationAllowed === false && c.runtimeLogReadAllowed === false &&
    c.bypassMutationAllowed === false && c.productionAllowed === false &&
    c.automaticRetryAllowed === false && c.maxDeploymentMetadataReads === 2 &&
    c.maxAnalyzeRequests === 16 && c.perRequestTimeoutMs === 90_000 &&
    c.maxResponseBytes === 2_097_152 &&
    typeof value.getDeploymentMetadata === "function" &&
    typeof value.probeAnalyze === "function" && typeof value.cleanup === "function";
}
...
function cleanupValid(value) {
  const keys = [
    ...
    "temporaryFileResidue", "projectEnvironmentMutationCount",
    "productionChangeCount", "deploymentMetadataReadCount",
    "analyzeRequestCount", "runtimeLogReadCount", "environmentReadCount",
    "deploymentMutationCount", "accessMaterialUseCount",
  ...
  return exactKeys(value, keys) && keys.filter((key) => key.endsWith("Count"))
    ...
    typeof value.routeContractReady === "boolean";
}

export function buildExecutionPlan({ executionGrant, runtimeAttestation, fixtureManifest, routeReadiness, now } = {}) {
  ...
  const blockers = unique([
    ...
    ...(validateRuntimeAttestation(runtimeAttestation) ? [] : ["runtime_attestation_invalid"]),
    ...(routeReady(routeReadiness) ? [] : ["diagnostic_route_contract_unsupported"]),
    ...(validateStopConditions(stopMap()) ? [] : ["stop_conditions_invalid"])
  ]);
  return deepFreeze({
    schemaVersion: "candidate-exposure-policy-hosted-execution-plan-v2",
    status: blockers.length ? "blocked_before_execution" : "plan_ready_for_approved_hosted_execution",
    ...
    requestCount: 16, metadataReadCount: 2, matrix: buildMatrix(),
    fixtureSemanticFingerprints: fixtures.fingerprints, stopConditions: stopMap(), blockers
  });
}

export async function executeApproved({
  executionGrant, runtimeAttestation, fixtureManifest, adapters,
  now = new Date(), wallClockNow = () => new Date(), monotonicNow = () => Date.now()
} = {}) {
  if (!safeAdapter(adapters)) throw new Error("safe_read_only_adapter_required");
  const plan = buildExecutionPlan({
    executionGrant, runtimeAttestation, fixtureManifest,
    routeReadiness: adapters.routeReadiness, now
  });
  const empty = (stopCondition, blocker, cleanup = null) => deepFreeze({
    status: "blocked_before_execution", completedRequestCount: 0,
    networkRequestCount: 0, http200Count: 0, finalDiagnosticStageCount: 0,
    stopCondition, blocker, telemetry: [], cleanup
  });
  if (plan.status !== "plan_ready_for_approved_hosted_execution") {
    ...
    return empty(
      blocker === "diagnostic_route_contract_unsupported"
        ...
        : blocker === "execution_grant_invalid"
          ? "approvalMissing" : "evidenceSerializationFailure",
      blocker
    );
  }
  ...
  try {
    ...
    if (!metadataValid(controlMeta, executionGrant.controlDeploymentId, executionGrant.approvedSourceSha)) {
      return empty("controlDeploymentMismatch", "control_deployment_metadata_mismatch", await adapters.cleanup(cleanupContext));
    }
    ...
    if (!metadataValid(canaryMeta, executionGrant.canaryDeploymentId, executionGrant.approvedSourceSha)) {
      return empty("canaryDeploymentMismatch", "canary_deployment_metadata_mismatch", await adapters.cleanup(cleanupContext));
    }
  } catch (error) {
    let cleanup = null;
    try { cleanup = await adapters.cleanup(cleanupContext); } catch {}
    return empty("evidenceSerializationFailure", error instanceof Error ? error.message : String(error), cleanup);
  }
  ...
  try {
    for (const entry of buildMatrix()) {
      if (Date.parse(wallClockNow()) >= expiresAt) { stopCondition = "approvalExpired"; blocker = "execution_grant_expired"; break; }
      if (monotonicNow() - started > 1_800_000) { stopCondition = "timeBudgetExceeded"; blocker = "execution_time_budget_exceeded"; break; }
      ...
      let probe;
      try {
        probe = await adapters.probeAnalyze({
          ...entry,
          deploymentId: entry.mode === "control" ? executionGrant.controlDeploymentId : executionGrant.canaryDeploymentId,
          fixture, expectedFixtureSemanticFingerprint: expectedFingerprint,
      ...
      const valid = exactKeys(probe, PROBE_KEYS) && probe.httpStatus === 200 &&
        probe.sourceSha === executionGrant.approvedSourceSha && probe.finalDiagnosticStage === true &&
        typeof probe.shadowExecution === "boolean" &&
        probe.telemetry?.fixtureScenario === entry.scenario && probe.telemetry?.locale === entry.locale &&
        probe.telemetry?.mode === entry.mode &&
        probe.telemetry?.fixtureSemanticFingerprint === expectedFingerprint &&
        validateTelemetry(probe.telemetry).valid;
      if (!valid) { stopCondition = "evidenceSerializationFailure"; blocker = "probe_contract_invalid"; break; }
      stopCondition = stopFromProbe(entry, probe);
      if (stopCondition) { blocker = `probe_stop:${stopCondition}`; break; }
  ...
  if (!cleanupValid(cleanup)) return deepFreeze({
    status: "cleanup_failed", completedRequestCount: telemetry.length,
    networkRequestCount, http200Count, finalDiagnosticStageCount,
    stopCondition: "protectionCleanupFailure", blocker: "local_cleanup_contract_invalid",
    telemetry, cleanup
  });
  return deepFreeze({
    status: !stopCondition && telemetry.length === 16 ? "completed_pass" : "stopped_on_contract_violation",
    completedRequestCount: telemetry.length, networkRequestCount, http200Count,
    finalDiagnosticStageCount, stopCondition: stopCondition || "none", blocker, telemetry, cleanup
  });
}

export function buildEvidence({ executionGrant, harnessImplementationSha, startedAt, completedAt, result } = {}) {
  const grant = validateExecutionGrant(executionGrant, { now: new Date(executionGrant?.issuedAt || 0) });
  const telemetry = result?.telemetry || [];
  const count = (key) => telemetry.reduce((n, entry) => n + entry[key], 0);
  return deepFreeze({
    ...
    controlDeploymentId: executionGrant?.controlDeploymentId,
    canaryDeploymentId: executionGrant?.canaryDeploymentId,
    startedAt, completedAt, plannedRequestCount: 16,
    ...
    shadowExceptionCount: count("shadowExceptionCount"), fallbackCount: count("fallbackCount"),
    ...
    stopCondition: result?.stopCondition || "none", blocker: result?.blocker || null,
...
export function validateEvidence(evidence) {
  const keys = [
    "schemaVersion", "planVersion", "approvalIdHash", "provisioningReceiptDigest",
    "executionGrantDigest", "approvedSourceSha", "productRuntimeAuthoritySha",
    "harnessImplementationSha", "controlDeploymentId", "canaryDeploymentId",
    "startedAt", "completedAt", "plannedRequestCount", "completedRequestCount",
    "http200Count", "runtimeShaMatchCount", "finalDiagnosticStageCount",
    "defaultOffExecutionCount", "canaryExecutionCount", "validTelemetryCount",
    "mutationFingerprintMatchCount", "unexpectedDivergenceCount",
    "unclassifiedDivergenceCount", "shadowExceptionCount", "fallbackCount",
    "invalidContextCount", "candidateLevelTelemetryIncidentCount",
    "networkRequestCount", "deploymentMetadataReadCount", "temporaryBypassCreatedCount",
    ...
    "accessMaterialUseCount", "setCookieDiscardCount", "stopCondition", "blocker",
    "cleanupStatus", "status", "authorization"
  ];
  const errors = [];
  if (!exactKeys(evidence, keys) || evidence?.schemaVersion !== HOSTED_EVIDENCE_SCHEMA_V2 || hasForbiddenKey(evidence)) errors.push("shape");
  ...
  if (evidence?.status === "completed_pass" && (
    evidence.completedRequestCount !== 16 || evidence.networkRequestCount !== 16 ||
    evidence.deploymentMetadataReadCount !== 2 || evidence.http200Count !== 16 ||
    evidence.runtimeShaMatchCount !== 16 || evidence.finalDiagnosticStageCount !== 16 ||
    evidence.defaultOffExecutionCount !== 0 || evidence.canaryExecutionCount !== 8 ||
    ...
    evidence.runtimeLogReadCount !== 0 || evidence.environmentReadCount !== 0 ||
    evidence.deploymentMutationCount !== 0 || evidence.projectEnvironmentMutationCount !== 0 ||
    evidence.productionChangeCount !== 0 || evidence.temporaryBypassCreatedCount !== 0 ||
    evidence.temporaryBypassRevokedCount !== 0 || evidence.stopCondition !== "none" ||
    evidence.blocker !== null || evidence.cleanupStatus !== "completed"
  )) errors.push("completed_pass_contract");
