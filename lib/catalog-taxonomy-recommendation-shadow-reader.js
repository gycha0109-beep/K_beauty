...
export const DATA_TAXONOMY5_READ_RPC = "recommendation_shadow.read_catalog_taxonomy_recommendation_overlay_v2";
...
function normalizeRows(rows) {
  ...
  return rows.map((row) => Object.freeze({
    ...
    assignmentState: normalizeNullable(row.assignment_state),
    assignmentMethod: normalizeNullable(row.assignment_method),
    legacyProjectionKey: normalizeNullable(row.legacy_projection_key),
    projectionPresent: row.projection_present === true,
...
export async function readCatalogTaxonomyRecommendationOverlay() {
  ...
  try {
    const rows = await withTimeout(sql`select * from recommendation_shadow.read_catalog_taxonomy_recommendation_overlay_v2()`);
