...
export function getFaceLabObservationAnalysis(value) {
  if (!isFaceLabResultEnvelope(value) || value.source !== FACE_LAB_SOURCE_VISION) {
    return null;
  }
  ...
  if (!eligibilityAllowsFaceLab || value.status === FACE_LAB_STATUS_UNAVAILABLE) {
    return null;
  }

  return getCanonicalFaceLabObservationAnalysis(getObjectData(value.data));
}
