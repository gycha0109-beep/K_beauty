...
export const PRODUCT_OFFER_READ_PATH_VERSION = "product-offer-read-v2";

const DEFAULT_MARKET_CODE = "KR";
const TRUSTED_LINK_SOURCE_POLICIES = new Map([
  [
    "legacy_product_buy_link_v1::oliveyoung",
    Object.freeze({
      sourceHint: "olive_young",
      allowedProductScopeStates: new Set(["product_subject_unresolved"]),
      allowedLinkHealthStates: new Set(["healthy", "unknown"]),
      priceAuthority: false
    })
  ],
  [
    "torriden_official::torriden_official",
    Object.freeze({
      sourceHint: "official:torriden",
      allowedProductScopeStates: new Set(["product"]),
      priceAuthority: false
    })
  ]
]);
...
function preserveProductPriceProjection(product) {
  return {
...
function resolveTrustedLinkSourcePolicy(offer) {
  return TRUSTED_LINK_SOURCE_POLICIES.get(
...
function buildAdmissibleOfferCandidate(offer, product, marketCode) {
  ...
  const sourcePolicy = resolveTrustedLinkSourcePolicy(offer);
  if (!sourcePolicy) {
    return null;
  }

  const productScopeState = normalizeString(offer.product_scope_state);
  if (!sourcePolicy.allowedProductScopeStates.has(productScopeState)) {
    return null;
  }

  if (
    sourcePolicy.allowedLinkHealthStates &&
    !sourcePolicy.allowedLinkHealthStates.has(normalizeString(offer.link_health_state))
  ) {
    return null;
  }
  ...
  const trustedListingUrl = getTrustedDirectPurchaseUrl({
    ...
    brand: product.brand,
    name: product.name,
    sourceHint: sourcePolicy.sourceHint
  });
  ...
  return {
    ...
    productScopePriority: PRODUCT_SCOPE_PRIORITY[productScopeState] || 0,
...
function resolveOfferPriceProjection(candidate, product) {
  if (!candidate?.sourcePolicy?.priceAuthority) {
    return preserveProductPriceProjection(product);
  }
