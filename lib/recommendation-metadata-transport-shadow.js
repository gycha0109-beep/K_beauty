...
function buildCommonDiagnostic({
  ...
  return {
    ...
    hardGateChanged,
    explanationChanged,
    unknownMetadataCount: (envelope?.metadataMissing || [])
      .filter((field) => metadataUsed.includes(field)).length,
    metadataUsed,
    metadataMissing: (envelope?.metadataMissing || [])
      .filter((field) => metadataUsed.includes(field)),
    ...
    products: moisturizerProducts
      ...
      .map((product) => {
        ...
        return {
          ...
          classifications: [
            classification.classification,
            ...(classification.reviewRequired ? ["review_required"] : [])
          ],
