...
function buildReferenceSplitFingerprint(manifest) {
  const payload = {
    ...
    provider: {
      ...
      sourceVersion: manifest.provider.sourceVersion,
      adapterId: manifest.provider.adapterId
    },
    records: manifest.records
      .filter((record) => record.split === "reference")
      .map((record) => ({
        ...
        sourceReceipt: record.sourceReceipt
          ? {
              runnerVersion: record.sourceReceipt.runnerVersion,
              sourceImageSha256: record.sourceReceipt.sourceImageSha256
            }
          : null,
        measurement: {
          source: record.measurement.source,
          sourceVersion: record.measurement.sourceVersion,
          adapterId: record.measurement.adapterId,
          dimensions: [...record.measurement.dimensions]
            .map((dimension) => ({
              id: dimension.id,
              unit: dimension.unit,
...
export function validateFaceSpaceReferenceCorpus(manifest = {}) {
  ...
  for (const record of manifest.records) {
    ...
    const sourceExecutionPresent =
      manifest.sourceExecution !== undefined &&
      manifest.sourceExecution !== null;
    const sourceReceiptPresent =
      record.sourceReceipt !== undefined &&
      record.sourceReceipt !== null;

    if (sourceExecutionPresent !== sourceReceiptPresent) {
      throw new Error("reference_corpus_source_receipt_boundary_invalid");
    }
    if (
      sourceExecutionPresent &&
      (
        manifest.sourceExecution.runnerVersion !==
          "face-space-reference-corpus-measurement-runner-v0" ||
        !/^sha256:[a-f0-9]{64}$/.test(
          manifest.sourceExecution.sourceManifestDigest || ""
        ) ||
        manifest.sourceExecution.rawImagePersisted !== false ||
        manifest.sourceExecution.rawLandmarksPersisted !== false ||
        manifest.sourceExecution.identityEmbeddingCreated !== false ||
        manifest.sourceExecution.biometricIdentityMatchPerformed !== false ||
        record.sourceReceipt.runnerVersion !==
          manifest.sourceExecution.runnerVersion ||
      ...
      throw new Error("reference_corpus_source_receipt_invalid");
    }
    if (
  ...
  return {
    ...
    referenceSplitFingerprint: buildReferenceSplitFingerprint(manifest),
    sourceExecution: manifest.sourceExecution
      ? {
          runnerVersion: manifest.sourceExecution.runnerVersion,
          sourceManifestDigest: manifest.sourceExecution.sourceManifestDigest
        }
      : null,
