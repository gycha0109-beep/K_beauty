...
export const NEUTRAL_FACE_COUNT_RECEIPT_COOKIE = "facelab_neutral_receipt_v1";
...
export async function persistNeutralFaceCountSubmission({ payload, testSubmission = false }) {
  const validation = validateNeutralFaceCountSubmission(payload, neutralAuthority);
  if (!validation.ok) {
    throw new NeutralFaceCountIntakeError("neutral_submission_invalid", 400);
  }
  if (!testSubmission) getReceiptSigningKey();
  const supabase = createSupabaseAdminClient();
  if (!supabase) throw new NeutralFaceCountIntakeError("neutral_intake_unavailable", 503);
  ...
  if (error) {
    ...
    const { data: existing, error: existingError } = await supabase
      ...
      .single();
    if (
      existingError || !existing || existing.submission_status !== expectedStatus ||
  ...
  return {
    ...
    receiptValue: testSubmission
      ? null
      : issueReceiptValue({ id: persisted.id, sessionId: persisted.session_id, responsePayloadSha256 })
  };
}
...
export async function requireVerifiedNeutralFaceCountReceipt(request, hostedSessionId) {
