import { getCurrentProductVerdictSlotKey } from "./current-product-verdicts.js";
...
function attachProducts(policy, report) {
  ...
  const find = (step, windowKey) => {
    const product = pool.find((item) => {
      ...
      return !used[windowKey].has(id) && productSlot(item) === step.role;
    }) || null;
...
function displayStep(step, locale) {
  const [title, action] = COPY[locale][step.stepKey] || [step.stepKey, ""];
  return {
    order: step.order,
    title,
    ...
    caution: getCaution(step, locale),
    adjustment: getCaution(step, locale),
...
function buildCurrentProductVerdicts(policy, locale) {
  return (policy.productActions || [])
    ...
    .map((item) => {
      ...
      return {
        slotKey: getCurrentProductVerdictSlotKey(mode, slot, category),
        productId: item.productId || null,
        status,
        title: titleMap[status] || titleMap.check_needed,
        summary: locale === "en"
