...
function normalizeSourceNotes(value) {
  return Array.from(
    new Set(
...
function normalizeCompleteness(value) {
  return ["complete", "partial", "final_results_only", "unavailable"].includes(value)
    ? value
    : "unavailable";
}

function normalizeSourceStage(value, completeness) {
  const valid = [
  ...
  if (completeness === "complete") {
    return "post_score_candidate_pool";
  }

  if (completeness === "partial") {
    return "post_filter_candidate_pool";
  }

  if (completeness === "final_results_only") {
...
function normalizeIdentityMode(value, products) {
  if (["product_row", "product_id_only", "mixed", "unavailable"].includes(value)) {
    return value;
  }

  return products.length ? "product_row" : "unavailable";
}

function resolveCandidateSource({ candidateSource, candidateProducts, existingRecommendationResult }) {
  const boundaryProducts = Array.isArray(candidateSource?.products)
    ? uniqueProducts(candidateSource.products)
    : [];

  if (candidateSource && typeof candidateSource === "object") {
    const boundaryCompleteness = normalizeCompleteness(candidateSource.completeness);
    const completeness = boundaryProducts.length ? boundaryCompleteness : "unavailable";

    return {
      completeness,
      sourceStage: normalizeSourceStage(candidateSource.sourceStage, completeness),
      sourceNotes: normalizeSourceNotes(candidateSource.sourceNotes),
      candidateIdentityMode: normalizeIdentityMode(candidateSource.candidateIdentityMode, boundaryProducts),
      products: boundaryProducts
    };
  }
  ...
  if (provided.length) {
    return {
      ...
      sourceNotes: ["legacy_candidate_products_argument"],
  ...
  if (finalProducts.length) {
    return {
      ...
      sourceStage: "final_results_only",
      sourceNotes: ["legacy_result_only"],
      candidateIdentityMode: "product_row",
  ...
  return {
    ...
    sourceNotes: ["candidate_rows_not_available_in_route"],
...
function buildCaptureFixture({
  ...
  const resolvedCandidateSource = resolveCandidateSource({
    candidateSource,
    candidateProducts,
    existingRecommendationResult
  });
  const existingRecommendationSnapshot = buildExistingRecommendationSnapshot({
    ...
    candidateSourceCoverage:
      resolvedCandidateSource.completeness === "complete"
        ? "complete"
        : resolvedCandidateSource.completeness === "partial"
          ? "partial"
          : "final_result_only",
    candidateSourceStage: resolvedCandidateSource.sourceStage,
    candidateIdentityMode: resolvedCandidateSource.candidateIdentityMode,
    candidateSourceNotes: resolvedCandidateSource.sourceNotes
  });
  ...
  return {
    ...
    candidateSource: {
      completeness: resolvedCandidateSource.completeness,
      sourceStage: resolvedCandidateSource.sourceStage,
      sourceCount: resolvedCandidateSource.products.length,
      sourceNotes: resolvedCandidateSource.sourceNotes,
      candidateIdentityMode: resolvedCandidateSource.candidateIdentityMode,
      products: resolvedCandidateSource.products
    },
