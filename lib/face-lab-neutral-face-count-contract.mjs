...
export function validateNeutralFaceCountAuthority(authority) {
  ...
  if (authority.schemaVersion !== "face-count-neutral-review-authority-v1") errors.push("schema_version");
  if (authority.authorityRef !== "authority.facelab.neutral_face_count.shared_review.v1") errors.push("authority_ref");
  if (authority.authorityVersion !== "1.0.0") errors.push("authority_version");
  ...
  const expectedTokens = ["none", "one", "two_or_more", "not_assessable"];
  if (JSON.stringify(authority.responseTokens) !== JSON.stringify(expectedTokens)) errors.push("response_tokens");

  if (!Array.isArray(authority.orderedItems) || authority.orderedItems.length !== 3) {
    ...
    for (const item of authority.orderedItems) {
      ...
      if (typeof item.assetPath !== "string" || !/^\/facelab\/neutral-review\/v1\/assets\/fcneutral_[0-9]{2}\.jpg$/.test(item.assetPath)) errors.push("asset_path");
    ...
    if (ids.size !== 3 || paths.size !== 3 || digests.size !== 3) errors.push("item_uniqueness");
  }
  ...
  if (!labels || Object.keys(labels).sort().join("|") !== [...expectedTokens].sort().join("|")) errors.push("response_labels");
  ...
  for (const forbidden of ["Earth_apollo17", "Eisenhower", "Group_Portrait", "HistoryTrustSA", "groundTruth", "ground_truth"]) {
