...
} from "@/lib/product-category-normalizer";
...
export const PRODUCT_SOURCE_UNAVAILABLE_CODE = "PRODUCT_SOURCE_UNAVAILABLE";

export class ProductSourceUnavailableError extends Error {
  constructor(reason, options = {}) {
    super("Recommendation products are temporarily unavailable.");
    this.name = "ProductSourceUnavailableError";
    this.code = PRODUCT_SOURCE_UNAVAILABLE_CODE;
    this.reason = reason;
    this.cause = options.cause;
  }
}

export function isProductSourceUnavailableError(error) {
  return Boolean(error && error.code === PRODUCT_SOURCE_UNAVAILABLE_CODE);
}
...
const CURRENT_PRODUCT_OPTION_FIELDS =
...
function resolveRecommendationCategory(product) {
  const category = inferMoisturizerCategory(product) || product?.category;
...
export function buildRecommendationProductFromSource(product) {
  const categorySemantics = resolveRecommendationCategory(product);

  if (!categorySemantics.authorizesRecommendationCategory) {
    return null;
  }
  const category = categorySemantics.canonicalCategory;
  const productForm = categorySemantics.productForm;
  ...
  return {
    ...
    category_family: categorySemantics.productFamily,
    recommendation_slot: categorySemantics.routineSlot,
...
function buildSupabaseProduct(product) {
  return buildRecommendationProductFromSource(product);
}
...
async function fetchSupabaseProducts() {
  ...
  if (!supabaseConfig) {
    throw new ProductSourceUnavailableError("missing_config");
  }
  ...
  try {
    ...
    console.error(
      "[product-source] failed to load Supabase products",
  ...
  for (const product of (data || []).filter((item) => item?.name && item?.brand)) {
    const builtProduct = buildSupabaseProduct(product);
    ...
    const reason = resolveRecommendationCategory(product).unresolvedReason || "unresolved";
    skippedByCategoryReason[reason] = (skippedByCategoryReason[reason] || 0) + 1;
  }

  if (!products.length) {
    console.error("[product-source] no usable Supabase products", {
      rowCount: Array.isArray(data) ? data.length : 0,
      skippedByCategoryReason
    });
    throw new ProductSourceUnavailableError("no_usable_products");
  }

  if (Object.keys(skippedByCategoryReason).length > 0) {
    console.warn("[product-source] skipped products with unresolved recommendation category", {
      skippedByCategoryReason
    });
  }
...
function mapCurrentProductOption(product) {
  return {
    id: product?.id || "",
    brand: product?.brand || "",
    name: product?.name || "",
...
function mapCurrentProductSnapshot(product) {
  const option = mapCurrentProductOption(product);
  const sensitivitySafe = typeof product?.sensitivity_safe === "boolean"
    ? product.sensitivity_safe
    : null;
  const irritationRisk = product?.irritation_risk == null
    ? null
    : mapIrritationRisk(product.irritation_risk, sensitivitySafe);

  return {
    ...option,
    skin_types: mapSkinTypes(product?.skin_types, []),
    concerns: mapConcerns(product?.concerns, []),
    texture: product?.texture ? mapTexture(product.texture, "") : "",
    finish: product?.finish ? mapFinish(product.finish, "") : "",
    irritation_risk: irritationRisk,
    sensitivity_safe: sensitivitySafe,
    price_min: Number(product?.price_min || 0) || null,
    price_max: Number(product?.price_max || 0) || null,
    price_range: mapPriceRange(product?.price_min, product?.price_max, ""),
    ingredient_signals: product?.ingredient_signals && typeof product.ingredient_signals === "object"
      ? product.ingredient_signals
      : null,
    review_signals: product?.review_signals == null ? null : product.review_signals,
    market_signals: product?.market_signals && typeof product.market_signals === "object"
      ? product.market_signals
      : null
  };
}

export async function fetchCurrentProductOptions({ category = "" } = {}) {
  const supabaseConfig = getSupabaseConfig();
  ...
  const normalizedCategory = normalized.unsupported ? "" : normalized.canonicalCategory;

  let query = supabase
    .from("products")
    .select(CURRENT_PRODUCT_OPTION_FIELDS)
  ...
  if (normalizedCategory) {
    if (normalizedCategory === "moisturizer") {
  ...
  if (error) {
    console.error("[product-source] failed to load current product options", error.message);
    return [];
  }

  return (data || [])
    .filter((product) => product?.id && product?.name && product?.brand)
    .map(mapCurrentProductOption);
}
...
export async function fetchCurrentProductSnapshotsByIds(productIds = []) {
  ...
  if (error) {
    console.error("[product-source] failed to load current product snapshots", error.message);
    return [];
  }

  return (data || [])
    .filter((product) => product?.id)
    .map(mapCurrentProductSnapshot);
}
