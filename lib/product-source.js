...
function buildFallbackReason(name, category) {
  const labelMap = {
    ...
    serum_ampoule: "adds a targeted treatment step without overloading the routine",
...
function buildFallbackNotes(category) {
  const noteMap = {
    ...
    serum_ampoule: "Supabase serum/ampoule metadata.",
...
async function loadRecommendationProducts() {
  ...
  return baseProducts.map(applyRecommendationProductOverrides);
}
