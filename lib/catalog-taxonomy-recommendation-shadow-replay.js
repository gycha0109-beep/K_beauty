...
export const DATA_TAXONOMY5_REPLAY_VERSION = "data-taxonomy15-production-recommendation-parity-catalog-only-v2";
...
export async function runCatalogTaxonomyRecommendationShadowReplay() {
  ...
  for (const product of products) {
    ...
    if (overlay.exactEquivalent !== true || overlay.catalogOnlyShadowValid === true) {
  ...
  const cardinality = evaluateCatalogTaxonomyOverlayCardinality({
    rows: probe.rows,
    liveIds,
    legacyCorpusIds: LEGACY_RECOMMENDATION_CORPUS_IDS,
  });
  ...
  const pass = probe.credentialAvailable && probe.runtimeRoleMatch && probe.rawProductSelectDenied && probe.rawTaxonomySelectDenied && products.length === DATA_TAXONOMY5_EXPECTED_RECOMMENDATION_PRODUCT_COUNT && cardinality.pass && duplicateOverlayCount === 0 && invalidTaxonomyStateCount === 0 && unmatchedProductCount === 0 && legacyIdentityMismatchCount === 0 && projectedProducts.length === products.length && scenarioResults.length === SCENARIOS.length && scoreDeltaCount === 0 && slotDeltaCount === 0 && fullOrderDeltaCount === 0 && top1DeltaCount === 0 && top3DeltaCount === 0;

  return Object.freeze({
    result: pass ? "PASS" : "FAIL_CLOSED",
    replayVersion: DATA_TAXONOMY5_REPLAY_VERSION,
    expectedProductCount: cardinality.expectedProductCount,
    expectedLegacyExactOverlayCount: cardinality.expectedLegacyExactOverlayCount,
    expectedRecommendationProductCount: DATA_TAXONOMY5_EXPECTED_RECOMMENDATION_PRODUCT_COUNT,
    expectedOverlayOnlyCount: cardinality.expectedOverlayOnlyCount,
    ...
    exactEquivalentCount: cardinality.exactEquivalentCount,
    catalogOnlyShadowCount: cardinality.catalogOnlyShadowCount,
    nonExactOverlayCount: cardinality.nonExactOverlayCount,
    invalidOverlayCount: cardinality.invalidOverlayCount,
    overlayOnlyCount: cardinality.overlayOnlyCount,
    overlayOnlyNonLegacyCount: cardinality.overlayOnlyNonLegacyCount,
    overlayOnlyCatalogOnlyCount: cardinality.overlayOnlyCatalogOnlyCount,
    missingLegacyLiveCount: cardinality.missingLegacyLiveCount,
    unexpectedLiveCount: cardinality.unexpectedLiveCount,
    catalogOnlyRecommendationLeakCount: cardinality.catalogOnlyRecommendationLeakCount,
