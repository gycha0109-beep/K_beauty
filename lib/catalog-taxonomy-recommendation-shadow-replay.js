...
export async function runCatalogTaxonomyRecommendationShadowReplay() {
  ...
  if (unmatchedProductCount === 0 && projectedProducts.length === products.length) {
  ...
  const pass = probe.credentialAvailable && probe.runtimeRoleMatch && probe.rawProductSelectDenied && probe.rawTaxonomySelectDenied && probe.rows.length > 0 && duplicateOverlayCount === 0 && nonExactOverlayCount === 0 && invalidTaxonomyStateCount === 0 && unmatchedProductCount === 0 && legacyIdentityMismatchCount === 0 && projectedProducts.length === products.length && scoreDeltaCount === 0 && slotDeltaCount === 0 && fullOrderDeltaCount === 0 && top1DeltaCount === 0 && top3DeltaCount === 0;
