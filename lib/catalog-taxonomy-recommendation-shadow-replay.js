...
export async function runCatalogTaxonomyRecommendationShadowReplay() {
  ...
  const overlayOnlyCount = probe.rows.filter((row) => !liveIds.has(row.productId)).length;
  ...
  const pass = probe.credentialAvailable && probe.runtimeRoleMatch && probe.rawProductSelectDenied && probe.rawTaxonomySelectDenied && products.length === DATA_TAXONOMY5_EXPECTED_PRODUCT_COUNT && probe.rows.length === DATA_TAXONOMY5_EXPECTED_PRODUCT_COUNT && overlayOnlyCount === 0 && duplicateOverlayCount === 0 && nonExactOverlayCount === 0 && invalidTaxonomyStateCount === 0 && unmatchedProductCount === 0 && legacyIdentityMismatchCount === 0 && projectedProducts.length === products.length && scenarioResults.length === SCENARIOS.length && scoreDeltaCount === 0 && slotDeltaCount === 0 && fullOrderDeltaCount === 0 && top1DeltaCount === 0 && top3DeltaCount === 0;
