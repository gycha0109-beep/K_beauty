...
function jsonComparableValue(value, arrayEntry = false) {
  if (typeof value === "number" && !Number.isFinite(value)) return null;
  if (["undefined", "function", "symbol"].includes(typeof value)) {
    return arrayEntry ? null : value;
  }
  return value;
}
    ...
    return Object.keys(value)
      .filter(
        (key) =>
          ...
          !["undefined", "function", "symbol"].includes(typeof value[key])
      )
      .sort();
  } catch {
    return [];
...
export function diffPremiumSnapshots(existingValue, nextValue, options = {}) {
  ...
  const visit = (existing, next, path, depth, existingPresent = true, nextPresent = true) => {
    ...
    existing = jsonComparableValue(existing);
    next = jsonComparableValue(next);
    ...
    if (existingType === "array") {
      ...
      for (let index = 0; index < length && !state.truncated; index += 1) {
        visit(
          jsonComparableValue(existing[index], true),
          jsonComparableValue(next[index], true),
