...
export const DATA_TAXONOMY5_GITHUB_OIDC_ISSUER = "https://token.actions.githubusercontent.com";
export const DATA_TAXONOMY5_RUNTIME_PROBE_AUDIENCE =
  "urn:bejewely:data-taxonomy5:production-recommendation-parity";
export const DATA_TAXONOMY5_RUNTIME_PROBE_REPOSITORY = "gycha0109-beep/K_beauty";
export const DATA_TAXONOMY5_RUNTIME_PROBE_REPOSITORY_ID = "1205065704";
export const DATA_TAXONOMY5_RUNTIME_PROBE_WORKFLOW_PATH =
  ".github/workflows/data-taxonomy5-production-recommendation-parity.yml";

const OIDC_METADATA_URL =
  `${DATA_TAXONOMY5_GITHUB_OIDC_ISSUER}/.well-known/openid-configuration`;
...
function validateClaims(payload, { expectedDeploymentSha, expectedGitRef }, nowSeconds) {
  ...
  const expectedWorkflowRef = `${DATA_TAXONOMY5_RUNTIME_PROBE_REPOSITORY}/${DATA_TAXONOMY5_RUNTIME_PROBE_WORKFLOW_PATH}@${expectedRef}`;
  if (payload?.iss !== DATA_TAXONOMY5_GITHUB_OIDC_ISSUER) return "invalid_issuer";
  if (!audienceMatches(payload?.aud, DATA_TAXONOMY5_RUNTIME_PROBE_AUDIENCE)) return "invalid_audience";
  if (payload?.repository !== DATA_TAXONOMY5_RUNTIME_PROBE_REPOSITORY) return "invalid_repository";
  if (String(payload?.repository_id || "") !== DATA_TAXONOMY5_RUNTIME_PROBE_REPOSITORY_ID) return "invalid_repository_id";
...
export function getDataTaxonomy5BearerTokenFromRequest(request) {
  const authorization = request?.headers?.get?.("authorization") || "";
  const match = authorization.match(/^Bearer\s+([^\s]+)$/i);
  return match ? match[1] : null;
}

export async function verifyDataTaxonomy5GitHubActionsOidcToken(
  token,
  {
  ...
  try {
    ...
    if (metadata?.issuer !== DATA_TAXONOMY5_GITHUB_OIDC_ISSUER) {
