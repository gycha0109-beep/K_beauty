...
export async function ensureBrowserSupabaseSession() {
  ...
  if (session?.user) {
    console.debug("[supabase/browser-client] existing session ready", {
      hasUser: true,
  ...
  if (!browserAnonymousSessionPromise) {
    console.debug("[supabase/browser-client] creating anonymous session");
  ...
  console.debug("[supabase/browser-client] anonymous session created", {
