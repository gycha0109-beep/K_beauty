...
export function buildRealPhotoStabilityAdequacyReview(reviewPacket) {
  if (
    ...
    !Number.isInteger(reviewPacket?.sourceReportCount) ||
    reviewPacket.sourceReportCount < 488 ||
    !Array.isArray(reviewPacket?.observations) ||
    reviewPacket.observations.length !== reviewPacket.sourceReportCount
  ) {
  ...
  const rollSummary = nuisanceClasses.find(
    (item) => item.nuisanceClass === "head_roll"
  );
  const rollEvidenceLimitation =
    rollSummary.subjectLinkageEvidenceRefCount > 1
      ? `Head roll evidence spans ${rollSummary.reportCount} observations across ${rollSummary.subjectLinkageEvidenceRefCount} linkage groups; this broadens descriptive stability evidence but does not by itself establish population-level roll stability or adequacy.`
  ...
  const review = {
    ...
    reviewVersion:
      ...
      "real-photo-expression-yaw-pitch-roll-stability-review-v2"
        ? "real-photo-stability-adequacy-review-v2"
        : "real-photo-stability-adequacy-review-v1",
