...
const SKIN_TYPES = new Set(["dry", "oily", "combination"]);
...
function buildSkinState(report = {}, context = {}) {
  ...
  return {
    ...
    poresBurden: scoreOrNull(rawScores, "pores"),
    textureBurden: null,
...
function normalizePhotoStatus(value) {
  ...
  if (["unavailable", "analysis_failed", "failed", "provider_failure"].includes(normalized)) return "unavailable";
  if (["insufficient_evidence", "insufficient"].includes(normalized)) return "insufficient_evidence";
  if (["ineligible", "input_ineligible"].includes(normalized)) return "ineligible";
  return "unknown";
}

function classifyPhotoFailure(status, failureReason, explicitFailureClass) {
  const declared = text(explicitFailureClass).toLowerCase();
  if ([
  ...
  if (status === "ineligible") return "input_ineligible";
  if (status === "insufficient_evidence") return "input_insufficient";
  if (status !== "unavailable") return null;
  const reason = text(failureReason).toLowerCase();
  if (reason === "api_key_missing") return "provider_unavailable";
  if (reason.startsWith("provider_") || reason === "vision_request_failed") return "provider_failure";
...
function buildPhotoState(report = {}, context = {}) {
  ...
  const rawEligibility =
    report?.imageEligibility ||
    report?.freeResult?.imageEligibility ||
    previous?.eligibility ||
    null;
  const eligibility = rawEligibility ? normalizeImageAnalysisEligibility(rawEligibility) : null;
  ...
  let status = observations ? "available" : explicitStatus;

  if (!observations && status === "unknown" && eligibility?.source === "vision") {
    if (eligibility.skinAnalysisEligible) status = "insufficient_evidence";
    else status = eligibility.status === "ineligible" ? "ineligible" : "insufficient_evidence";
  }

  const eligibilityFailureReason = eligibility?.skinAnalysisEligible === false
    ? text(eligibility.skinFailureReason) || null
    : null;
  const failureReason = ["unavailable", "insufficient_evidence", "ineligible"].includes(status)
    ? text(explicit?.failureReason || explicit?.unavailableReason) || eligibilityFailureReason || "unspecified"
    : null;
  const failureClass = classifyPhotoFailure(status, failureReason, explicit?.failureClass);

  return {
    status,
    source: observations
      ? text(explicit?.source || report?.photoObservationsSource) || "persisted_observations"
      : text(explicit?.source || eligibility?.source) || null,
    failureReason,
    failureClass,
    eligibility,
    analysisEligible: eligibility ? eligibility.skinAnalysisEligible : null,
    observations,
...
function buildUncertaintyState({
  ...
  if (["unavailable", "insufficient_evidence", "ineligible"].includes(photo.status)) {
    reasons.push("photo_analysis_unavailable");
  }
  if (["provider_unavailable", "provider_failure"].includes(photo.failureClass)) {
    reasons.push("photo_provider_unavailable");
  }
  if (photo.failureClass === "technical_failure") reasons.push("photo_technical_failure");
  if (photo.failureClass === "input_ineligible") reasons.push("photo_input_ineligible");
  if (photo.failureClass === "input_insufficient") reasons.push("photo_evidence_insufficient");
