...
function normalizeProductionSource(value) {
  return SOURCES.has(value) ? value : "UNKNOWN_PRODUCTION_SOURCE";
}
...
export function validateV21_9LOrganicEvidenceRows(rows) {
  ...
  for (const row of rows) {
    ...
    if (row.actual_exclusion_count !== 0) errors.push("shadow_actual_exclusion_nonzero");
  }
...
export function buildV21_9LOrganicEvidenceRows({ input = {}, observation = {}, now = Date.now() } = {}) {
  ...
  const source = normalizeProductionSource(
    observation?.productionSource || telemetry?.productionSource
  );
