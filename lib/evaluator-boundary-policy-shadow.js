...
function emptyResult(
  ...
  const currentFindingsContext = goalResolution?.goalPolicy?.currentProductFindings || null;
  const currentFindingsSummary = currentFindingsContext?.summary || {};
  return {
    ...
    currentFindingsContextVersion: currentFindingsContext?.version || null,
    currentFindingsExposureState: currentFindingsContext?.exposureState || "unavailable",
    currentFindingsCount: Number(currentFindingsSummary.productCount || 0),
    currentFindingsEvaluableCount: Number(currentFindingsSummary.evaluableSelectedCount || 0),
    currentFindingsUnknownCount: Number(currentFindingsSummary.unknownProductCount || 0),
...
export function buildEvaluatorBoundaryPolicyExecution({
  ...
  const effectiveCurrentProductFindings =
    currentProductFindings || effectiveGoalPolicy?.currentProductFindings || null;
  ...
  for (const product of Array.isArray(products) ? products : []) {
    ...
    const candidateEvaluation = evaluateFunctionalRankingCandidate({
      ...
      currentProductFindings: effectiveCurrentProductFindings
    });
