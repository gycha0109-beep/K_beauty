...
export function buildExistingRecommendationSnapshot(existingResult = {}) {
  const notes = normalizeList(existingResult?.candidateSourceNotes)
  ...
  const candidateSourceCoverage =
    ...
    "final_result_only";
  const candidateSourceStage =
    existingResult?.candidateSourceStage ||
    existingResult?.shadowCandidateSourceStage ||
    (candidateSourceCoverage === "final_result_only" ? "final_results_only" : null);
  const candidateIdentityMode =
    existingResult?.candidateIdentityMode ||
    existingResult?.shadowCandidateIdentityMode ||
    null;
