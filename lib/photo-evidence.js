import {
  ...
  createInvalidImageAnalysisEligibility,
...
export function buildFallbackPhotoAnalysis(locale = "ko") {
  return {
    ...
    imageEligibility: createInvalidImageAnalysisEligibility()
  };
}
...
export function normalizePhotoAnalysis(parsed, locale = "ko") {
  ...
  const imageEligibility = normalizeImageAnalysisEligibility(parsed?.eligibility);

  if (!imageEligibility.skinAnalysisEligible) {
    return {
      ...fallback,
      imageEligibility
    };
