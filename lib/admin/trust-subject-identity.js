...
export function normalizeReviewedSubjectIdentity(value) {
  ...
  const variantKeyReviewedAsNull = value.variantKeyReviewedAsNull === true;
  if ((variantKey && variantKeyReviewedAsNull) || (!variantKey && !variantKeyReviewedAsNull)) {
    throw new TrustSubjectIdentityError(
...
function assertLineage(snapshot) {
  ...
  if (
    ...
    uuid(candidate.matched_product_id) !== productId
  ) {
    throw new TrustSubjectIdentityError(
