...
function normalizeProviders(value) {
  ...
  return providers.sort((left, right) =>
    canonicalTrustSubjectJson(left).localeCompare(canonicalTrustSubjectJson(right))
  );
}
...
export function buildTrustSubjectIdentityProposal(
  ...
  const evidenceBasis = {
    ...
    reviewed_at: text(evidence.reviewed_at) ?? text(candidate.reviewed_at),
    reviewed_by: uuid(candidate.reviewed_by),
    providers,
  ...
  const evidenceDigest = digestTrustSubjectValue(evidenceBasis);
  ...
  const formulationLabel =
    [text(product.brand), text(product.name)].filter(Boolean).join(" ") ||
    productId;
