...
function validateOperationInput({ candidateId, decision, reason }) {
  const normalizedCandidateId = normalizeNullableText(candidateId);
...
export async function runProductReviewPreflight({
  ...
  const actorId = normalizeNullableText(actorUserId);
...
export async function confirmProductReview({
  ...
  const actorId = normalizeNullableText(actorUserId);
  const input = validateOperationInput({ candidateId, decision, reason });
  const expectedCandidateUpdatedAt = normalizeNullableText(candidateUpdatedAt);
  const expectedReviewUpdatedAt = normalizeNullableText(reviewUpdatedAt);
  const expectedEvidenceHash = normalizeNullableText(evidenceHash);
  const expectedPreflightHash = normalizeNullableText(preflightHash);
