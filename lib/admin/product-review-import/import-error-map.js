...
const RETRYABLE_CODES = new Set(["confirm_failed", "unexpected_error"]);
const SAFE_STATUSES = new Set([400, 401, 403, 409, 413, 415, 422, 500, 503]);
...
export class ProductReviewImportError extends Error {
  constructor(code, status = 400) {
    ...
    this.status = SAFE_STATUSES.has(status) ? status : 400;
  }
}
...
export function mapProductReviewImportCode(value, fallback = "unexpected_error") {
  ...
  if (
    code.includes("manifest_hash") ||
    code.includes("evidence_hash") ||
    code.includes("candidate_ids_mismatch") ||
    code.includes("source_snapshot_mismatch")
  ) {
  ...
  if (
    code.includes("reviewed") ||
    code.includes("review_import_confirm_requires_passing_dry_run")
  ) {
...
export function publicImportError(error, fallback = "unexpected_error") {
  ...
  const fallbackStatus =
    mapped === "request_too_large"
      ? 413
      : mapped === "unexpected_error" || mapped === "confirm_failed"
        ? 500
        : 400;
  const status = SAFE_STATUSES.has(error?.status) ? error.status : fallbackStatus;

  return {
    code: mapped,
    message: getProductReviewImportMessage(mapped),
    status,
    retryable: RETRYABLE_CODES.has(mapped)
  };
}
