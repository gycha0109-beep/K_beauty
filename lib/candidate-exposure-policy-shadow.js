...
import {
  EXFOLIATION_NON_NUMERIC_PDA_SHADOW_ADAPTER_VERSION,
  buildExfoliationNonNumericPdaShadowDecisionInputs
} from "./exfoliation-non-numeric-pda-shadow-adapter.js";
import {
  EXFOLIATION_NON_NUMERIC_PDA_SHADOW_CONSUMER_VERSION,
  consumeExfoliationNonNumericPdaShadowDecisionInputs
} from "./exfoliation-non-numeric-pda-shadow-consumer.js";

import DEFAULT_EXFOLIATION_PDA_ARTIFACT from "../evidence/product-decision-axis-non-numeric-shadow-v1/exfoliation-non-numeric-pda-offline-shadow-output-v1.json" with { type: "json" };
...
function runExfoliationPdaShadowAdapter({ candidates, pdaArtifact, canonicalState }) {
  try {
    return buildExfoliationNonNumericPdaShadowDecisionInputs({
      candidates,
      pdaArtifact,
      canonicalState
    });
...
function runExfoliationPdaShadowConsumer(adapterResult) {
  try {
    return consumeExfoliationNonNumericPdaShadowDecisionInputs(adapterResult);
  } catch {
    return {
      consumer_version: EXFOLIATION_NON_NUMERIC_PDA_SHADOW_CONSUMER_VERSION,
      status: "consumer_execution_failed",
      shadow_only: true,
      rows: []
    };
  }
...
export function runCandidateExposurePolicyShadow({
  ...
  responseValue,
  snapshotValue,
  exfoliationPdaArtifact = DEFAULT_EXFOLIATION_PDA_ARTIFACT,
  ...
  try {
    const exfoliationPdaShadow = runExfoliationPdaShadowAdapter({
      candidates,
      pdaArtifact: exfoliationPdaArtifact,
      canonicalState
    });
    const exfoliationPdaShadowConsumer = runExfoliationPdaShadowConsumer(exfoliationPdaShadow);
    ...
    if (!emitted.emitted) {
      ...
      return {
        ...
        exfoliationPdaShadow,
        exfoliationPdaShadowConsumer,
        fingerprints,
