...
export function resolveCandidateExposurePolicyShadowControl(envLike = {}) {
  ...
  const validEnvironment = ["", "development", "preview", "production"].includes(vercelEnvironment);
  const enabled = optInRequested && !killSwitchRequested && !production && validEnvironment;
