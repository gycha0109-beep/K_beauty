...
export function buildPremiumCurrentProductVerdicts(currentProducts, report, locale = "ko") {
  const freeResult = report?.freeResult && typeof report.freeResult === "object"
    ? report.freeResult
    : {};
  const currentProductVerdicts = buildCurrentProductVerdicts(currentProducts, {
    locale,
  ...
  if (report && typeof report === "object" && !Array.isArray(report)) {
    report.currentProducts = currentProducts;
    report.currentProductVerdicts = currentProductVerdicts;
    applyPremiumDecisionState(report, {
      locale,
      source: "full_report_current_products"
    });
  }

  return currentProductVerdicts;
}
