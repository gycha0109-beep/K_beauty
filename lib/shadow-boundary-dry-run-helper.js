...
export function summarizeShadowBoundaryDryRunComparison(input = {}) {
  ...
  const summary = {
    ...
    activeOnlyViolationCount: Number(comparison.activeOnlyViolationCount || 0),
  ...
  const blockedReasons = [
    ...
    summary.activeOnlyViolationCount > 0 ? "active_only_receiver_contract_violation_not_zero" : null,
...
export function buildShadowBoundaryDryRunArtifact(input = {}) {
  ...
  const artifact = {
    ...
    comparison: {
      ...
      sensitivityUnsafeCollapsedReceiverCount: Number(input.comparisonSnapshot.sensitivityUnsafeCollapsedReceiverCount || 0),
      strongCautionCollapsedReceiverCount: Number(input.comparisonSnapshot.strongCautionCollapsedReceiverCount || 0),
      activeOnlyViolationCount: Number(input.comparisonSnapshot.activeOnlyViolationCount || 0)
    },
