...
const CONTEXT_VERSION = "shared-skin-decision-context-v3";
...
function getSurveyAnswers(report = {}, freeResult = {}, previousContext = {}) {
  ...
  const baseAnswers = candidates.find(
    (candidate) => candidate && typeof candidate === "object" && !Array.isArray(candidate)
  ) || {};
  const premiumAnswers =
    report?.premiumIntake?.answers &&
    typeof report.premiumIntake.answers === "object" &&
    !Array.isArray(report.premiumIntake.answers)
      ? report.premiumIntake.answers
      : {};
...
function getScaleThreshold(scores, options = {}) {
  if (normalizeText(options.concernScoreScale).toLowerCase() === "skin_match_raw") {
    return 18;
  }
  return Math.max(...Object.values(scores).map(parseFiniteNumber), 0) > 40 ? 70 : 18;
}

function buildSafetyState({ priority, concernScores, productExposureState, surveyAnswers, options = {} }) {
  const highThreshold = getScaleThreshold(concernScores, options);
...
export function buildSharedSkinDecisionContext(report = {}, options = {}) {
  ...
  const safetyState = buildSafetyState({ priority, concernScores, productExposureState, surveyAnswers, options });
