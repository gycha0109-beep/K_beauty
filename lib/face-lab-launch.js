...
function localizeColorLabel(value, locale = "ko") {
  ...
  return matched?.label || (hasKoreanText(text) ? "" : text);
}
...
function buildStyleKeywords(faceLab, fallback, locale = "ko") {
  ...
  return unique([
    ...(locale === "en" ? explicit.map((item) => localizeColorLabel(item, locale)).filter(Boolean) : explicit),
...
function buildFaceLine(faceLab, fallback, locale = "ko") {
  ...
  const rawFaceShape = cleanText(faceLab?.base_data?.face_shape);
  return locale === "en"
