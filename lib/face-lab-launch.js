...
function getFallback(locale = "ko") {
  if (locale === "en") {
    return {
      teaserLine: "Keep the top line light, control side width, and let the lower face stay clean.",
      faceMood: {
        primary: "고양이상",
        secondary: ["토끼상", "두부상"],
  ...
  return {
    teaserLine: "윗선은 가볍게, 옆 폭은 좁게, 하관 실루엣은 깨끗하게 잡는 방향이 좋습니다.",
    faceMood: {
      primary: "고양이상",
      secondary: ["토끼상", "두부상"],
...
function buildStructureContent(faceLab, fallback, locale = "ko") {
  ...
  const shapeLine = faceShape
    ? locale === "en"
      ? `${faceShape} is the face-shape label used as the base for this read.`
      : `${faceShape} 흐름이 얼굴선 판단의 기준으로 잡힙니다.`
    : "";
  const structuralSummary =
    faceSummary && !styleInstructionPattern.test(faceSummary) && textMatches(faceSummary, structuralPatterns)
      ? faceSummary
      : "";

  return unique([
    structuralSummary,
    firstUsableLine(featureLines, structuralPatterns),
    shapeLine,
    ...landmarkLines,
...
function buildColorDirection(faceLab, fallback, locale = "ko") {
  ...
  if (palette.length) {
    return locale === "en"
      ? `For color, keep ${palette.join(", ")} as the base before adding stronger accents.`
      : `컬러는 ${palette.join(", ")}처럼 얼굴보다 먼저 튀지 않는 톤을 베이스로 잡습니다.`;
  }

  return colorLine || fallback.directionRecommended[1];
}
...
function buildDirectionRecommended(faceLab, fallback, locale = "ko") {
  ...
  return unique([
    hairLine || fallback.directionRecommended[0],
    buildColorDirection(faceLab, fallback, locale),
    fallback.directionRecommended[2]
  ]).slice(0, 3);
}

function buildDirectionAvoid(faceLab, fallback) {
  ...
  return unique([
    hairAvoid || fallback.directionAvoid[0],
    colorAvoid || fallback.directionAvoid[1],
...
function buildGuideBaseSetup(faceLab, fallback, locale = "ko") {
  ...
  const paletteLine = palette.length
    ? locale === "en"
      ? `Use ${palette[0]} as the base tone, then keep ${palette[1] || palette[0]} as a small accent.`
      : `${palette[0]}을 베이스 톤으로 두고 ${palette[1] || palette[0]}은 작은 포인트로만 씁니다.`
...
function buildStyleKeywords(faceLab, fallback, locale = "ko") {
  ...
  const palette = compactList(faceLab?.features?.color_tone_recommendation?.palette, 4);
...
function buildFaceLine(faceLab, fallback, locale = "ko") {
  ...
  return cleanText(faceLab?.base_data?.face_shape) || "부드러운 곡선형";
}
...
function buildFocusPoint(faceLab, fallback, locale = "ko") {
  ...
  return unique(points).slice(0, 2).join(" / ") || "이마 / 눈매";
}
...
function buildFaceMood(faceLab, fallback, locale = "ko") {
  ...
  return {
    primary: moodTypes.primary,
    secondary: moodTypes.secondary.length ? moodTypes.secondary : fallback.faceMood.secondary,
...
function buildFaceLabSections(faceLab, locale = "ko") {
  ...
  return [
    ...
    },
    {
      ...
      avoid: buildDirectionAvoid(faceLab, fallback)
    },
    {
      id: "guide",
