...
const FACE_LAB_KEYWORD_CONCEPTS = {
  softness: {
    ko: "부드러움",
    en: "Softness",
    variants: new Set(["부드러움", "부드러운", "부드럽게", "soft", "softly", "softness"])
  }
};
...
function getFaceLabKeywordConcept(value) {
  const normalized = normalizeFaceLabToken(value);

  for (const [key, concept] of Object.entries(FACE_LAB_KEYWORD_CONCEPTS)) {
    if (concept.variants.has(normalized)) {
      return key;
    }
  }
...
function isFaceLabPlaceholderKeyword(value) {
  const text = cleanText(value);

  return (
    /^(?:특징|feature)\s*\d+$/i.test(text) ||
    /^(?:fallback|default|mock|pending|placeholder)(?:\s+\w+)*$/i.test(text) ||
    text === "기본 키워드"
  );
}

export function formatFaceLabKeywordList(values = [], locale = "ko", limit = 3) {
  ...
  for (const value of Array.isArray(values) ? values : []) {
    ...
    if (!rawValue || isFaceLabPlaceholderKeyword(rawValue)) {
      continue;
    }

    const concept = getFaceLabKeywordConcept(rawValue);

    if (seenConcepts.has(concept)) {
      continue;
    }

    const configuredConcept = FACE_LAB_KEYWORD_CONCEPTS[concept];
    const label = configuredConcept
      ? configuredConcept[locale] || configuredConcept.ko
      : formatFaceLabDisplayText(rawValue, locale);
    const isUnknownEnglishFallback =
      locale !== "en" &&
      /^[a-z][a-z\s_-]{1,40}$/i.test(rawValue) &&
      label === "사진 기준 스타일 무드";

    if (!label || isUnknownEnglishFallback) {
      continue;
    }
...
function collectFieldEvidence(source, paths, limit = 8) {
  const evidence = [];
  ...
  return unique(evidence).slice(0, limit);
}

function createFaceLabField({ status, source = null, confidence = null, evidence = [], unavailableReason = null, value = null }) {
  return {
    status,
    source: status === "available" ? source : null,
    confidence: status === "available" ? confidence : null,
    evidence: unique(evidence),
    unavailableReason: status === "available" ? null : unavailableReason || "insufficient_evidence",
    value: status === "available" ? value : null
  };
}

function createUnavailableFaceLabField(reason = "insufficient_evidence") {
  return createFaceLabField({
...
function isAvailableFaceLabField(field) {
  return Boolean(
    field?.status === "available" &&
      (field.source === "vision" || field.source === "derived_from_vision") &&
      Array.isArray(field.evidence) &&
      field.evidence.some((item) => cleanText(item)) &&
      isObject(field.value)
  );
}

function normalizeStructuredFaceLabField(field) {
  if (!isObject(field)) {
    return null;
  }
  ...
  return createFaceLabField({
    status,
    source: field.source === "vision" || field.source === "derived_from_vision" ? field.source : null,
    confidence: typeof field.confidence === "number" ? field.confidence : null,
    evidence: compactList(field.evidence, 12),
    unavailableReason: cleanText(field.unavailableReason) || null,
    value: isObject(field.value) ? field.value : null
  });
}

function normalizeStructuredFaceLabData(structured) {
  if (!isObject(structured)) {
    return null;
  }

  return {
    mood: normalizeStructuredFaceLabField(structured.mood) || createUnavailableFaceLabField(),
    color: normalizeStructuredFaceLabField(structured.color) || createUnavailableFaceLabField(),
    style: normalizeStructuredFaceLabField(structured.style) || createUnavailableFaceLabField()
  };
}

function deriveAnimalTypeFromEvidence(evidence, locale = "ko") {
  ...
  const moodTypes = pickFaceMoodTypes({ base_data: {}, features: { physiognomy: { overall_impression: text } } }, locale);
  const hasMoodCue = /fox|cat|puppy|rabbit|wolf|dino|deer|tofu|sharp|defined|jaw|soft|round|curve|eye|gaze|friendly|warm|long|vertical/i.test(text);

  return hasMoodCue ? localizeFaceMoodType(moodTypes.primary, locale) : null;
}

function buildStructuredMoodField(faceLab, locale = "ko") {
  const evidence = collectFieldEvidence(faceLab, [
    "features.physiognomy.headline_label",
    "features.physiognomy.headline_result",
    "features.physiognomy.overall_impression",
    "features.physiognomy.interpretation_axes",
    "features.physiognomy.feature_based_interpretation",
    "base_data.embedding",
    "base_data.landmarks"
  ]);
  const traits = formatFaceLabDisplayList(
    [
      ...(Array.isArray(faceLab?.features?.physiognomy?.interpretation_axes)
        ? faceLab.features.physiognomy.interpretation_axes
        : []),
  ...
  const primary = formatFaceLabDisplayText(
    faceLab?.features?.physiognomy?.headline_label ||
      faceLab?.features?.physiognomy?.headline_result ||
      faceLab?.features?.physiognomy?.overall_impression ||
      "",
    locale
  );
  const animalType = deriveAnimalTypeFromEvidence(evidence, locale);

  if (!evidence.length || (!primary && !traits.length && !animalType)) {
    return createUnavailableFaceLabField("mood_evidence_missing");
  }

  return createFaceLabField({
    status: "available",
    source: animalType ? "derived_from_vision" : "vision",
    confidence: animalType ? 0.72 : 0.68,
    evidence,
    value: {
      primary: primary || animalType || traits[0],
      traits,
      animalType
    }
  });
...
function buildStructuredColorField(faceLab, locale = "ko") {
  const evidence = collectFieldEvidence(faceLab, [
    "base_data.color_values.undertone",
    "base_data.color_values.brightness",
    "base_data.color_values.contrast",
    "base_data.color_values.saturation",
    "features.color_tone_recommendation.summary",
    "features.color_tone_recommendation.palette",
    "features.color_tone_recommendation.recommendations"
  ]);
  const palette = localizePalette(faceLab?.features?.color_tone_recommendation?.palette, locale, 4);
  const directions = formatFaceLabDisplayList(
    [
      faceLab?.features?.color_tone_recommendation?.summary,
      ...(Array.isArray(faceLab?.features?.color_tone_recommendation?.recommendations)
  ...
  if (!evidence.length || (!palette.length && !directions.length)) {
    return createUnavailableFaceLabField("color_evidence_missing");
  }

  return createFaceLabField({
    status: "available",
    source: "vision",
    confidence: palette.length ? 0.78 : 0.66,
    evidence,
    value: {
      palette,
      directions
...
function buildStructuredStyleField(faceLab, locale = "ko") {
  const evidence = collectFieldEvidence(faceLab, [
    "base_data.face_shape",
    "base_data.landmarks",
    "features.face_shape_hairstyle.summary",
    "features.face_shape_hairstyle.recommendations",
    "features.face_shape_hairstyle.avoid"
  ]);
  const hairDirections = formatFaceLabDisplayList(
    [
      faceLab?.features?.face_shape_hairstyle?.summary,
      ...(Array.isArray(faceLab?.features?.face_shape_hairstyle?.recommendations)
        ? faceLab.features.face_shape_hairstyle.recommendations
        : [])
    ],
    locale,
    4
  );
  const stylingDirections = formatFaceLabDisplayList(
    [
      faceLab?.base_data?.face_shape,
  ...
  if (!evidence.length || (!hairDirections.length && !stylingDirections.length)) {
    return createUnavailableFaceLabField("style_evidence_missing");
  }

  return createFaceLabField({
    status: "available",
    source: "vision",
    confidence: hairDirections.length ? 0.76 : 0.64,
    evidence,
    value: {
      hairDirections,
      stylingDirections
...
export function buildFaceLabStructuredData(faceLab, locale = "ko") {
  if (!isObject(faceLab)) {
    return {
      mood: createUnavailableFaceLabField("face_lab_unavailable"),
      color: createUnavailableFaceLabField("face_lab_unavailable"),
      style: createUnavailableFaceLabField("face_lab_unavailable")
    };
  }

  return {
    mood: buildStructuredMoodField(faceLab, locale),
    color: buildStructuredColorField(faceLab, locale),
    style: buildStructuredStyleField(faceLab, locale)
  };
}
...
function buildStructuredSummary(structured = {}, locale = "ko") {
  const mood = isAvailableFaceLabField(structured.mood) ? structured.mood.value : null;
  const color = isAvailableFaceLabField(structured.color) ? structured.color.value : null;
  const style = isAvailableFaceLabField(structured.style) ? structured.style.value : null;
  const recommended = unique([
    ...(Array.isArray(style?.hairDirections) ? style.hairDirections : []),
    ...(Array.isArray(color?.palette) ? color.palette : []),
    ...(Array.isArray(color?.directions) ? color.directions : []),
    ...(Array.isArray(mood?.traits) ? mood.traits : [])
  ]).slice(0, 3);

  return {
    faceLine: Array.isArray(style?.stylingDirections) ? style.stylingDirections[0] || "" : "",
    focusPoint: Array.isArray(mood?.traits) ? mood.traits[0] || "" : "",
    recommendedDirection: recommended.join(" 쨌 "),
    avoidDirection: ""
  };
}

function buildStructuredFaceMood(structured = {}, locale = "ko") {
  const mood = isAvailableFaceLabField(structured.mood) ? structured.mood.value : null;
  const color = isAvailableFaceLabField(structured.color) ? structured.color.value : null;
  const style = isAvailableFaceLabField(structured.style) ? structured.style.value : null;
  ...
  return {
    primary: mood.animalType || mood.primary || "",
    secondary: Array.isArray(mood.traits) ? mood.traits.slice(0, 2) : [],
    keywords: formatFaceLabKeywordList([
      ...(Array.isArray(mood.traits) ? mood.traits : []),
      ...(Array.isArray(color?.palette) ? color.palette : []),
      ...(Array.isArray(style?.stylingDirections) ? style.stylingDirections : [])
    ], locale, 8),
    impression: mood.primary || ""
  };
}

function buildStructuredFreeTeaser(structured = {}) {
  const style = isAvailableFaceLabField(structured.style) ? structured.style.value : null;
  const color = isAvailableFaceLabField(structured.color) ? structured.color.value : null;
  const mood = isAvailableFaceLabField(structured.mood) ? structured.mood.value : null;

  return cleanText(
    style?.hairDirections?.[0] ||
      color?.directions?.[0] ||
      mood?.primary ||
      ""
  );
...
export function buildFaceLabLaunchData(faceLab, locale = "ko") {
  const structured =
    normalizeStructuredFaceLabData(faceLab?.structured) ||
    buildFaceLabStructuredData(null, locale);
  const generatedTeaser = buildStructuredFreeTeaser(structured);
  const structuredMood = buildStructuredFaceMood(structured, locale);
  const structuredSummary = buildStructuredSummary(structured, locale);
  const structuredStyle = isAvailableFaceLabField(structured.style) ? structured.style.value : null;
  const structuredColor = isAvailableFaceLabField(structured.color) ? structured.color.value : null;

  return {
    structured,
    free: {
      teaserLine: generatedTeaser || ""
    },
    paid: {
      summary: structuredSummary,
      faceMood: structuredMood,
      faceSummary: Array.isArray(structuredStyle?.stylingDirections) ? structuredStyle.stylingDirections[0] || "" : "",
      hairDirections: Array.isArray(structuredStyle?.hairDirections) ? structuredStyle.hairDirections : [],
      avoidStyles: [],
      styleKeywords: formatFaceLabKeywordList([
        ...(Array.isArray(structuredMood.keywords) ? structuredMood.keywords : []),
        ...(Array.isArray(structuredColor?.palette) ? structuredColor.palette : [])
      ], locale, 8),
      toneDirection: Array.isArray(structuredColor?.directions) ? structuredColor.directions[0] || "" : "",
      reasoningLines: structured.mood.evidence.concat(structured.color.evidence, structured.style.evidence).slice(0, 8),
      practicalGuide: null,
      sections: [],
      steps: []
    }
