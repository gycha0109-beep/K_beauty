...
const SOURCE_REFERENCE_PAGE_SIZE = 1000;
const SOURCE_REFERENCE_CHUNK_SIZE = 100;
...
const LINKED_CUSTOMER_DATA_PURGE_STEPS = Object.freeze([
  Object.freeze({
    sourceTable: "saved_reports",
    sourceType: "premium_report_session",
    sourceColumn: "source_session_id",
    targetTable: "premium_report_sessions",
...
function chunkValues(values, size = SOURCE_REFERENCE_CHUNK_SIZE) {
  const chunks = [];
...
async function getLinkedPremiumReportSessionIds(admin, userId) {
  ...
  for (;;) {
    const { data, error } = await admin
      ...
      .range(offset, offset + SOURCE_REFERENCE_PAGE_SIZE - 1);

    if (error) {
      throw new AccountDeletionError("account_deletion_source_lookup_failed", error);
    }

    for (const row of data || []) {
      const sessionId = typeof row?.source_session_id === "string"
    ...
    if (!Array.isArray(data) || data.length < SOURCE_REFERENCE_PAGE_SIZE) {
      break;
    }

    offset += SOURCE_REFERENCE_PAGE_SIZE;
  }

  return [...sessionIds];
}

async function purgeLinkedCustomerData(admin, userId) {
  const premiumSessionIds = await getLinkedPremiumReportSessionIds(admin, userId);

  for (const sessionIdChunk of chunkValues(premiumSessionIds)) {
    const { data: sharedReferences, error: sharedReferenceError } = await admin
      .from("saved_reports")
      .select("user_id")
      .eq("source_type", "premium_report_session")
      .in("source_session_id", sessionIdChunk)
      .neq("user_id", userId)
      .limit(1);

    if (sharedReferenceError) {
      throw new AccountDeletionError("account_deletion_source_lookup_failed", sharedReferenceError);
    }
...
async function purgeCustomerData(admin, userId) {
  await purgeLinkedCustomerData(admin, userId);
...
export function getAccountDeletionContract() {
  return Object.freeze({
    ...
    linkedCustomerDataPurgeSteps: LINKED_CUSTOMER_DATA_PURGE_STEPS,
