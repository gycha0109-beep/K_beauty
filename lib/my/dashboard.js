...
import { hasSavedPremiumReportEntitlement } from "@/lib/premium-saved-report-access";
...
function getLatestReportPath(report, access) {
  if (!report) {
    return null;
  }
...
export async function getMyDashboardPayload({ localDate } = {}) {
  ...
  try {
    ...
    const premiumAccess = resolvePremiumAccessForUser(user);
    const visibleLatestSavedReport =
      latestSavedReport?.report_type === "premium" &&
    ...
    return {
      ...
      payload: {
        ...
        latestSharePath: getLatestReportPath(visibleLatestSavedReport, premiumAccess),
        hasProfile,
