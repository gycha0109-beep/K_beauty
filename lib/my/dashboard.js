...
function isProductQueryBetaAvailable(user, envLike = process.env) {
  if (!user?.id) return false;

  const activationGate =
    evaluateProductQueryAuthenticatedBetaControlledActivation(envLike);
  if (!activationGate.allowed) return false;
  ...
  if (!staticGate.staticAllowed) return false;
  ...
  return runtimePolicy.allowed === true;
}
  ...
  try {
    ...
    return {
      ...
      payload: {
        ...
        productQueryBetaAvailable: isProductQueryBetaAvailable(user),
        hasProfile,
