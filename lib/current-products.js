...
export const CURRENT_PRODUCT_USE_FREQUENCIES = ["daily", "few_times_week", "weekly_or_less", "as_needed"];
...
export function sanitizeCurrentProducts(input) {
  ...
  for (const item of items) {
    ...
    if (status === "selected") {
      ...
      if (CURRENT_PRODUCT_USE_FREQUENCIES.includes(item.useFrequency)) {
        next.useFrequency = item.useFrequency;
      }
    ...
    if (status === "not_in_db" && CURRENT_PRODUCT_USE_FREQUENCIES.includes(item.useFrequency)) {
      next.useFrequency = item.useFrequency;
    }
