import { buildProductEvidencePresentationProjection } from "./product-evidence-presentation-contract.js";
...
export const PRODUCT_EVIDENCE_PRESENTATION_PROVIDER_VERSION =
  "product-evidence-presentation-provider-v1";
...
const FEATURE_KEY_SET = new Set(PRODUCT_EVIDENCE_PRESENTATION_FEATURE_KEYS);
...
function projectionFromFact(featureKey, fact) {
  if (!fact || fact.fact_key !== featureKey) return missingProjection(featureKey);
  ...
  return buildProductEvidencePresentationProjection({
    featureKey,
    factRef:
      factInstanceId && factAuthorityUsable
        ? `product_fact_instance:${factInstanceId}`
        : null,
    axisRef: null,
...
export function buildProductEvidencePresentationFromAuthorityRead(payload) {
  ...
  const projections = PRODUCT_EVIDENCE_PRESENTATION_FEATURE_KEYS.map((featureKey) => {
    const facts = currentFacts.filter(
      (fact) => fact && FEATURE_KEY_SET.has(fact.fact_key) && fact.fact_key === featureKey
    );
    return facts.length === 1 ? projectionFromFact(featureKey, facts[0]) : missingProjection(featureKey);
  });
