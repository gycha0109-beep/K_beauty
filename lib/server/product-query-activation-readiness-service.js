...
function freezeCase(value) {
  return Object.freeze({
    ...
    allowedConstraintStatuses: Object.freeze([...(value.allowedConstraintStatuses || [])])
  });
}
...
export const PRODUCT_QUERY_ACTIVATION_READINESS_CASES = Object.freeze([
  ...
  freezeCase({
    ...
    query: "피부는 건성이고 민감도가 높은 편이야. 장벽 고민이 있어서 크림 보습제를 원해.",
    ...
    requireNullIntentFields: ["texture"]
  }),
  freezeCase({
    id: "barrier_cream_b",
    family: "barrier_moisturizer",
    query: "건성 피부고 자극에 아주 민감해. 피부 장벽 때문에 보습 크림을 찾는 중이야.",
    expectedCategory: "moisturizer_cream",
    expectedStatus: "ranked",
    expectedConstraintStatus: "resolved",
    requiredIntent: {
  ...
  freezeCase({
    ...
    expectedCategory: "sunscreen",
    expectedStatus: "ranked",
    allowedConstraintStatuses: ["resolved", "partial"],
    requiredIntent: {
      category: "sunscreen",
      skin_type: "oily",
      sunscreen_intent: true,
...
function evaluateCase(testCase, shadow) {
  ...
  if (
    testCase.expectedConstraintStatus &&
    execution.constraintStatus !== testCase.expectedConstraintStatus
  ) {
    failures.push("constraint_status_mismatch");
  }
  if (
    testCase.allowedConstraintStatuses.length > 0 &&
    !testCase.allowedConstraintStatuses.includes(execution.constraintStatus)
  ) {
    failures.push("constraint_status_not_allowed");
  }
