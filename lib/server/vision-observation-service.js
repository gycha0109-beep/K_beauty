...
import { createVisionObservationPrompt, VISION_OBSERVATION_PROMPT_VERSION, VISION_OBSERVATION_SCHEMA_VERSION } from "@/lib/vision-observation-contract";
...
export function createVisionInputFingerprint({ imageBuffer, model = DEFAULT_MODEL, secret = "" } = {}) {
...
export async function analyzeVisionObservation({
  ...
  const rawText = await readBoundedResponse(response);
  if (!response.ok) {
    logProviderRuntimeEvent({
      stage: "vision-observation",
      status: response.status,
