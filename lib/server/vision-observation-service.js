...
import { buildRecommendationFeatureShadow } from "@/lib/recommendation-feature-shadow";
...
function buildShadowSafely(bundle) {
  try {
    return buildRecommendationFeatureShadow(bundle);
  } catch {
    return {
      schemaVersion: "recommendation-feature-v1",
      mode: "shadow",
      productionAuthoritative: false,
      valid: false,
      validationErrors: ["shadow_build_failed"],
      privacy: {
...
export async function analyzeVisionObservation({
  ...
  const recommendationFeatureShadow = buildShadowSafely(bundle);
  const inputTokens = safeTokenCount(providerPayload?.usage?.prompt_tokens);
  const outputTokens = safeTokenCount(providerPayload?.usage?.completion_tokens);
