...
export async function runNaturalLanguageProductQueryShadow(query, options = {}) {
  ...
  if (typeof options.onOperationalObservation === "function") {
    try {
      options.onOperationalObservation(Object.freeze({
        provider: extracted.provider,
        model: extracted.model,
        confidence: extracted.intent?.confidence || "unknown",
        status: execution?.status || "unknown",
        constraintStatus: execution?.constraintStatus || "unknown",
        unresolvedCount: Array.isArray(execution?.unresolvedTerms)
          ? execution.unresolvedTerms.length
          : 0,
      ...
      // Operational observability must never alter Product Query behavior.
    }
  }
