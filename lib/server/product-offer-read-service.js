...
export const PRODUCT_OFFER_READ_RPC = "read_product_offer_presentation_authority_v2";
export const PRODUCT_OFFER_READ_CONTRACT_VERSION =
  "product-offer-presentation-authority-read-v2";
...
async function loadProductOffersByIds(productIds) {
  ...
  try {
    ...
    const rows = await withTimeout(sql`
      select public.read_product_offer_presentation_authority_v2(
...
export async function runProductOfferPresentationRuntimeSecurityProbe(productIds) {
  ...
  try {
    normalizedProductIds = normalizeProductIds(
      Array.isArray(productIds) ? productIds : [productIds],
    );
    if (!normalizedProductIds.length) {
      throw new Error("product_offer_probe_product_ids_empty");
    }
  } catch {
    return Object.freeze({
  ...
  let control = null;
  if (primary.rpcResultClass !== "SUCCESS" && normalizedProductIds.length === 1) {
  ...
  const transportDifferential =
    primary.rpcResultClass === "SUCCESS"
      ...
      : control?.rpcResultClass === "SUCCESS"
        ...
        : normalizedProductIds.length > 1
          ? "MULTI_PRODUCT_PRIMARY_FAILED"
          : "BOTH_FAILED";
