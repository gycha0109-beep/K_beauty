...
export function getAnalysisRequestGuardSecret() {
...
export async function resolveAnalysisGuardAccountUser(request) {
...
export async function resolveAnalysisGuardPrincipal(request, secret, { purposePrefix = "analysis-principal" } = {}) {
  const accountUser = await resolveAnalysisGuardAccountUser(request);
  const cookiesToSet = [];

  if (accountUser?.id) {
    return {
      scope: "user",
      subjectHash: purposePrefix === "analysis-principal"
        ? createPrincipalHash({ scope: "user", value: String(accountUser.id), secret })
        : createGuardHmac(secret, `${purposePrefix}:user`, String(accountUser.id)),
  ...
  return {
    scope: "anonymous",
    subjectHash: purposePrefix === "analysis-principal"
      ? createPrincipalHash({ scope: "anonymous", value: anonymousPayload, secret })
      : createGuardHmac(secret, `${purposePrefix}:anonymous`, anonymousPayload),
    anonymousPayload,
    cookiesToSet
  };
}
...
export function resolveExistingAnonymousCookiePayload(request) {
  const secret = getAnalysisRequestGuardSecret();
...
export async function guardAnalysisRequest({ request, endpoint, fingerprintInput }) {
  ...
  try {
    principal = await resolveAnalysisGuardPrincipal(request, secret);
  } catch {
    return createDeniedResult("analysis_guard_unavailable", { httpStatus: 503 });
  }
