...
export async function issueAnonymousWriteGrants({ supabase, anonymousPayload, result, form, locale }) {
  ...
  if (!secret || !supabase || typeof anonymousPayload !== "string" || !anonymousPayload) {
    return { ok: false, code: "grant_prerequisite_unavailable" };
  }
  ...
  try {
    ...
    return { ok: false, code: "grant_bundle_invalid" };
  }
  ...
  try {
    ...
    return { ok: false, code: "grant_rpc_failed" };
  }
  ...
  if (normalizeRpcJson(rpcResponse?.data)?.created !== 2) {
    return { ok: false, code: "grant_rpc_result_invalid" };
  }
