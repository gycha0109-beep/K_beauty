...
export function buildFreeResultV2FaceLabPreview(faceLabPreview = null, locale = "ko") {
  const primary = typeof faceLabPreview?.primary === "string" ? faceLabPreview.primary.trim() : "";
  const keywords = Array.isArray(faceLabPreview?.keywords)
    ? faceLabPreview.keywords.filter((keyword) => typeof keyword === "string" && keyword.trim()).slice(0, 3)
    : [];
