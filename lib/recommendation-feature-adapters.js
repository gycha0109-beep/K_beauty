...
function buildSkinAxisResult(axis, support) {
  ...
  const level = sourceStatus === "available" ? support.value?.level : null;
  const observationAvailable = sourceStatus === "available";
  const noneObserved = observationAvailable && level === "none";
  const unresolvedNonZero = observationAvailable && ["mild", "moderate", "high"].includes(level);
  const quantizationResolved = noneObserved;
  const legacySignalAvailable = noneObserved;
  ...
  return {
    ...
    available: legacySignalAvailable,
    sourceStatus,
    ...
    quantizationStatus: noneObserved
      ...
      : unresolvedNonZero
        ...
        : sourceStatus === "unsupported"
          ? "unsupported"
          : "unavailable",
    unresolvedReason: unresolvedNonZero ? "unresolved_non_zero" : null,
    observedLevel: level,
    productionAuthoritative: false
  };
}
...
export function buildSkinLegacyShadowAdapter(canonicalBundle, derived) {
  ...
  const comparison = Object.fromEntries(SKIN_LEGACY_AXES.map((axis) => {
    ...
    return [axis, {
      ...
      shadowObservationAvailable: shadow.observationAvailable,
      shadowQuantizationResolved: shadow.quantizationResolved,
      shadowLegacySignalAvailable: shadow.legacySignalAvailable,
      shadowAvailable: shadow.legacySignalAvailable,
      quantizationStatus: shadow.quantizationStatus,
      comparable: shadow.legacySignalAvailable && direct !== null,
      equalWhenComparable: shadow.legacySignalAvailable && direct !== null ? direct === shadow.signal : null
    }];
  }));

  const legacySignalAvailability = Object.fromEntries(
    SKIN_LEGACY_AXES.map((axis) => [axis, axes[axis].legacySignalAvailable])
  );
  ...
  return {
    ...
    observationAvailability: Object.fromEntries(
      SKIN_LEGACY_AXES.map((axis) => [axis, axes[axis].observationAvailable])
    ),
    quantizationResolved: Object.fromEntries(
      SKIN_LEGACY_AXES.map((axis) => [axis, axes[axis].quantizationResolved])
    ),
    legacySignalAvailability,
    availability: legacySignalAvailability,
    metadata: {
      availabilityMeaning: "legacy_numeric_signal_available",
      ...
      unresolvedReason: Object.fromEntries(SKIN_LEGACY_AXES.map((axis) => [axis, axes[axis].unresolvedReason])),
