...
const ENUMERATED_AGGREGATE_COUNT_MAPS = new Set([
  "exposureCounts",
  "laneEligibilityCounts",
  "reasonCodeCounts",
  "divergenceCategoryCounts"
...
function containsForbiddenKey(value, parentKey = null) {
  if (!value || typeof value !== "object") return false;
  return Object.entries(value).some(([key, nested]) => {
    if (ENUMERATED_AGGREGATE_COUNT_MAPS.has(parentKey)) {
      return containsForbiddenKey(nested, key);
    }
    return FORBIDDEN_KEY.test(key) || containsForbiddenKey(nested, key);
  });
}
