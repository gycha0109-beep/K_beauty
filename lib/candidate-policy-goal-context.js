...
} from "./shared-skin-decision-context-v4.js";
import {
...
export function validateCandidatePolicyGoalContext(context) {
  ...
  if (context.source !== GOAL_CONTEXT_SOURCE) errors.push("candidate_goal_context_source_invalid");
  ...
  if (!VALID_GOALS.has(context.rankingGoal)) errors.push("candidate_goal_ranking_invalid");
  if (
  ...
  if (typeof context.goalTension !== "boolean") {
    ...
    if (context.goalTension !== expectedTension) errors.push("candidate_goal_tension_mismatch");
  }
  ...
  if (!currentFindingsValidation.valid) errors.push(...currentFindingsValidation.errors);
...
export function buildCandidatePolicyGoalContext({
  ...
  const context = {
    ...
    detectedPriority,
    rankingGoal,
    goalTension: Boolean(requestedGoal && rankingGoal && requestedGoal !== rankingGoal),
...
export function resolveCandidatePolicyGoalPolicy({
  ...
  const safetyValidation = validateCandidatePolicyRuntimeSafetyContext(candidateSafetyContext);
  if (!validation.valid) {
