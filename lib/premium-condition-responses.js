...
function getExposureList(answers = {}) {
  return Array.isArray(answers.environmentExposure)
    ? answers.environmentExposure.map((item) => String(item || "").trim()).filter(Boolean)
    : [];
}

function hasSensitivePriority(priorityAxis) {
  return ["barrier", "redness", "acne", "dehydration"].includes(priorityAxis);
}

function hasHighSensitiveBurden(scoreCard, priorityAxis) {
  return hasSensitivePriority(priorityAxis) ||
    ["barrier", "redness", "acne"].some((axis) => getConcernTotal(scoreCard, axis) >= 18);
}

function hasActiveBurden(context = {}) {
  const hasHold = Array.isArray(context.currentProductVerdicts) &&
    context.currentProductVerdicts.some((item) => item?.status === "hold");
  const hasFunctionalPause = Array.isArray(context.functionalDecisions) &&
    context.functionalDecisions.some((item) => item?.status === "pause");

  return hasHold || hasFunctionalPause;
}

function hasEnvironmentBurden(answers = {}) {
  const exposures = getExposureList(answers);
  const exposureKeys = new Set(["heat", "humidity", "mask", "aircon", "outdoor", "indoor_dry", "dry_air"]);

  return Boolean(answers.outdoorExposure) || exposures.some((item) => exposureKeys.has(item));
}
...
function hasHighSafetyContext(answers = {}, priorityAxis = "") {
  return answers.sensitivity === "high" && ["redness", "barrier", "acne"].includes(priorityAxis);
}
...
function decideStatus(responseKey, context = {}) {
  ...
  if (responseKey === "active_load") {
    return sensitiveBurden && activeBurden ? "avoid_for_now" : sensitiveBurden ? "reduce" : "maintain";
  }
  ...
  if (responseKey === "texture_routine") {
    if (["oiliness", "pores"].includes(priorityAxis) && !sensitiveBurden) {
      return "maintain";
    }
    return sensitiveBurden ? "reduce" : "maintain";
  }
...
function responseRelevance(definition, context = {}) {
  const priorityAxis = getPriorityAxis(context);
  const scoreCard = context.scoreCard || context.scoring || {};
  const direct = definition.axes.includes(priorityAxis) ? 100 : 0;
  const score = Math.max(...definition.axes.map((axis) => getConcernTotal(scoreCard, axis)), 0);

  return direct + score;
}

function buildReasons(definition, status, context = {}, locale = "ko") {
  ...
  if (definition.axes.includes(priorityAxis)) {
    reasons.push(isEnglish ? "This response is connected to the current priority." : "현재 우선 피부 축과 연결된 대응입니다.");
  }

  if (definition.responseKey === "cleansing_load" && hasCleansingBurden(answers)) {
    reasons.push(isEnglish ? "Tightness or frequent cleansing increases routine burden." : "당김 또는 잦은 세안 신호가 루틴 부담을 높입니다.");
  }

  if (definition.responseKey === "environment_recovery" && hasEnvironmentBurden(answers)) {
    reasons.push(isEnglish ? "Environmental exposure was included in the survey context." : "환경 노출 신호가 설문 맥락에 포함되어 있습니다.");
  }

  if (definition.responseKey === "active_load" && status === "avoid_for_now") {
    reasons.push(isEnglish ? "Current active-burden signals overlap with a sensitive priority." : "현재 부담 신호와 민감한 우선순위가 겹칩니다.");
  } else if (definition.responseKey === "active_load" && status === "reduce") {
    reasons.push(isEnglish ? "The current priority benefits from lowering active-step load first." : "현재 우선순위에서는 기능 단계 부담을 먼저 낮추는 편이 좋습니다.");
  }

  if (!reasons.length) {
    reasons.push(isEnglish ? "This keeps the routine easier to adjust on unstable days." : "흔들리는 날 루틴을 조정하기 쉽게 만드는 기준입니다.");
  }
...
function inclusionRank(status) {
  return { avoid_for_now: 0, reduce: 1, maintain: 2 }[status] ?? 2;
}
...
export function buildPremiumConditionResponses(context = {}) {
  ...
  const answers = context.answers || {};
  const priorityAxis = getPriorityAxis(context);
  const safetyNote = hasHighSafetyContext(answers, priorityAxis)
    ? (locale === "en"
  ...
  return RESPONSE_DEFINITIONS
    .map((definition) => {
      const statusCandidate = decideStatus(definition.responseKey, context);
      const status = CONDITION_STATUSES.has(statusCandidate) ? statusCandidate : "maintain";
      ...
      return {
        ...
        action: definition.responseKey === "hydration_barrier" && safetyNote
          ...
          : copy.action || null,
        relevance: responseRelevance(definition, context),
        inclusionRank: inclusionRank(status)
      };
    })
    .sort((left, right) => {
      const rankDiff = left.inclusionRank - right.inclusionRank;
      if (rankDiff !== 0) {
        return rankDiff;
      }
    ...
    .map(({ relevance, inclusionRank, ...item }) => item);
}
