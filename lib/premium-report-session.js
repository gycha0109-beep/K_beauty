...
import {
  createPremiumReportOwnerBinding,
  matchesPremiumReportOwnerBinding
} from "@/lib/premium-report-session-owner";
...
function writeSessionFailure({ category, dependency, retryable }) {
  writeSafeLog(category === "database_unavailable" ? "error" : "warn", {
...
export async function createPremiumReportSession(payload, options = {}) {
  ...
  const validationReason = classifyPremiumSessionPayload(payload);
  if (validationReason) {
    const diagnosticLogged = logPremiumSessionValidationFailure(
  ...
  const ownerBinding = createPremiumReportOwnerBinding(options.userId, secret);
  if (!ownerBinding) {
    writeSessionFailure({
      category: "validation_rejected",
      dependency: "application",
...
export async function verifyPremiumReportSession(token, options = {}) {
  const secret = getSecret();
  ...
  try {
    ...
    if (!matchesPremiumReportOwnerBinding(payload.owner, options.userId, secret)) {
...
export async function updatePremiumReportSession(token, premiumReport, options = {}) {
  ...
  if (!data?.premium_report) {
    return { ok: false, code: "missing_or_expired_session" };
  }
