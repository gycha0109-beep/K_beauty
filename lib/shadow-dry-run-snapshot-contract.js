...
export function buildShadowBoundaryHintSnapshot(boundaryHintLike = []) {
  const input = Array.isArray(boundaryHintLike) ? { boundaryHints: boundaryHintLike } : (boundaryHintLike || {});
  const rows = Array.isArray(input.boundaryHints) ? input.boundaryHints : [input].filter(Boolean);
  return {
    ...baseSnapshot("shadowBoundaryHintSnapshot"),
    candidateCount: Number(input.candidateCount ?? rows.length),
...
export function buildShadowReceiverSnapshot(receiverLike = []) {
  const input = Array.isArray(receiverLike) ? { receivers: receiverLike } : (receiverLike || {});
  const rows = Array.isArray(input.receivers) ? input.receivers : [input].filter(Boolean);
  const receivers = rows.map((row) => ({
    ...
    safetyMetadataClass: row.safetyMetadataClass == null ? null : String(row.safetyMetadataClass),
    sensitivityUnsafe: row.sensitivityUnsafe === true,
    strongCautionSignal: row.strongCautionSignal === true,
    activeOnlyViolation: row.activeOnlyViolation === true,
    reasonKeys: compactReasonKeys(row.reasonKeys || row.reasons)
  }));
  ...
  return {
    ...
    aggregate: {
      ...
      strongCautionCollapsedReceiverCount: countReceivers(
        receivers,
        (row) => row.futureExposureGroup === "collapsed_candidate" && row.strongCautionSignal
      ),
      sensitivityUnsafeCollapsedReceiverCount: countReceivers(
        receivers,
        (row) => row.futureExposureGroup === "collapsed_candidate" && row.sensitivityUnsafe
      ),
      activeOnlyViolationCount: countReceivers(receivers, (row) => row.activeOnlyViolation)
    }
  };
}
...
export function buildShadowComparisonSnapshot({
  ...
  const sensitivityUnsafeCollapsedReceiverCount = Number(aggregate.sensitivityUnsafeCollapsedReceiverCount || 0);
  const activeOnlyViolationCount = Number(aggregate.activeOnlyViolationCount || 0);
  ...
  const killConditionReasons = [
    ...
    activeOnlyViolationCount > 0 ? "active_only_receiver_contract_violation_not_zero" : null,
