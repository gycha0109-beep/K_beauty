...
const TOP_LEVEL_KEYS = new Set([...LEGACY_TOP_LEVEL_KEYS, ...PROVENANCE_TOP_LEVEL_KEYS]);
...
function hasProvenancePartition(telemetry) {
  return Object.prototype.hasOwnProperty.call(telemetry, "productionSource");
}
function hasPartialProvenancePartition(telemetry) {
  const present = [...PROVENANCE_TOP_LEVEL_KEYS].filter((key) => Object.prototype.hasOwnProperty.call(telemetry, key));
  return present.length > 0 && !hasProvenancePartition(telemetry);
}
...
export function buildExfoliationNormativePolicyRuntimeTelemetry({ control = {}, runtimeEvents = [], comparison = {}, versions = {}, rollbackEventCount = 0, productionSource } = {}) {
  ...
  const provenanceProvided = productionSource !== undefined;
  const source = provenanceProvided
    ? normalizeExfoliationNormativePolicyProductionSource(productionSource)
    : null;
...
export function validateExfoliationNormativePolicyRuntimeTelemetry(telemetry, { skipStopConsistency = false } = {}) {
  ...
  if (hasPartialProvenancePartition(telemetry)) errors.push("partial_production_source_partition");
  if (hasProvenancePartition(telemetry)) {
    ...
    for (const key of ["organicStopReasons","controlledStopReasons","unknownStopReasons"]) if (!Array.isArray(telemetry[key]) || telemetry[key].some((reason) => !EXFOLIATION_NORMATIVE_POLICY_STOP_REASONS.includes(reason))) errors.push(`invalid_${key}`);
