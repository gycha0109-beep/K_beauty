...
export const DATA_AI5_GITHUB_OIDC_ISSUER =
  "https://token.actions.githubusercontent.com";
export const DATA_AI5_RUNTIME_PROBE_AUDIENCE =
  "urn:bejewely:data-ai5:activation-readiness";
export const DATA_AI5_RUNTIME_PROBE_REPOSITORY =
  "gycha0109-beep/K_beauty";
export const DATA_AI5_RUNTIME_PROBE_REPOSITORY_ID = "1205065704";
export const DATA_AI5_RUNTIME_PROBE_WORKFLOW_PATH =
  ".github/workflows/data-ai5-activation-readiness.yml";

const OIDC_METADATA_URL =
  `${DATA_AI5_GITHUB_OIDC_ISSUER}/.well-known/openid-configuration`;
...
function validateClaims(payload, { expectedDeploymentSha, expectedGitRef }, nowSeconds) {
  ...
  const expectedWorkflowRef =
    `${DATA_AI5_RUNTIME_PROBE_REPOSITORY}/${DATA_AI5_RUNTIME_PROBE_WORKFLOW_PATH}@${expectedRef}`;

  if (payload?.iss !== DATA_AI5_GITHUB_OIDC_ISSUER) return "invalid_issuer";
  if (!audienceMatches(payload?.aud, DATA_AI5_RUNTIME_PROBE_AUDIENCE)) {
    return "invalid_audience";
  }
  if (payload?.repository !== DATA_AI5_RUNTIME_PROBE_REPOSITORY) {
    return "invalid_repository";
  }
  if (
    String(payload?.repository_id || "") !== DATA_AI5_RUNTIME_PROBE_REPOSITORY_ID
  ) {
    return "invalid_repository_id";
  }
  if (payload?.event_name !== "push") return "invalid_event_name";
  if (payload?.ref !== expectedRef || payload?.ref_type !== "branch") {
    return "invalid_ref";
  }
  ...
  if (!RUN_ID_PATTERN.test(String(payload?.run_attempt || ""))) {
...
export function getDataAi5BearerTokenFromRequest(request) {
  const authorization = request?.headers?.get?.("authorization") || "";
  const match = authorization.match(/^Bearer\s+([^\s]+)$/i);
  return match ? match[1] : null;
}

export async function verifyDataAi5GitHubActionsOidcToken(
  token,
  {
  ...
  try {
    ...
    if (metadata?.issuer !== DATA_AI5_GITHUB_OIDC_ISSUER) {
  ...
  const claimError = validateClaims(
    payload,
    {
      expectedDeploymentSha: String(expectedDeploymentSha || ""),
      expectedGitRef: expectedGitRef.trim()
    },
