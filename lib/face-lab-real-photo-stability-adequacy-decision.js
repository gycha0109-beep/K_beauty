...
const DECISION_CODES = Object.freeze({
  adequate_for_provisional_research: "PROVISIONAL_RESEARCH_ADEQUATE",
  hold_for_more_evidence: "ADDITIONAL_EVIDENCE_REQUIRED",
  blocked_by_evidence_integrity: "BLOCKED_BY_EVIDENCE_INTEGRITY"
});
const ALLOWED_STATUSES = new Set(Object.keys(DECISION_CODES));
...
export function validateRealPhotoStabilityAdequacyDecision(
  ...
  if (
    ...
    decision.decisionCode !== DECISION_CODES[decision.status] ||
  ...
  const holdReasons = Array.isArray(decision.holdReasons)
    ...
    : [];
  const evidenceIntegrityBlockers = Array.isArray(
    decision.evidenceIntegrityBlockers
  )
    ? decision.evidenceIntegrityBlockers
    : [];
  ...
  if (decision.status === "adequate_for_provisional_research") {
    if (
      ...
      evidenceIntegrityBlockers.length !== 0
    ) {
      throw new Error("real_photo_stability_adequacy_decision_invalid");
    }
  } else if (decision.status === "hold_for_more_evidence") {
    if (
    ...
    decision.provisionalResearchGateGranted !== false ||
    evidenceIntegrityBlockers.length === 0 ||
    evidenceIntegrityBlockers.some((item) => !nonEmpty(item)) ||
    holdReasons.length !== 0
  ) {
    throw new Error("real_photo_stability_adequacy_decision_invalid");
  }
  ...
  return {
    ...
    evidenceIntegrityBlockers: [...evidenceIntegrityBlockers],
