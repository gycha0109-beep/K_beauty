export const EVALUATOR_BOUNDARY_POLICY_RUNTIME_TELEMETRY_VERSION = "2026-07-29.candidate-findings-v1";
...
const CURRENT_FINDINGS_EXPOSURE_STATES = new Set([
  "valid_empty",
  "populated",
  "not_using",
  "unanswered",
...
const TELEMETRY_KEYS = new Set([
  ...
  "legacyFallbackUsed",
  "alignmentStopReason",
  "currentFindingsContextVersion",
  "currentFindingsExposureState",
  "currentFindingsCount",
  "currentFindingsEvaluableCount",
  "currentFindingsUnknownCount",
...
export function buildEvaluatorBoundaryPolicyRuntimeTelemetry({
  ...
  return {
    ...
    currentFindingsContextVersion: runtimeResult?.currentFindingsContextVersion || null,
    currentFindingsExposureState: runtimeResult?.currentFindingsExposureState || "unavailable",
    currentFindingsCount: nonNegativeInteger(runtimeResult?.currentFindingsCount),
    currentFindingsEvaluableCount: nonNegativeInteger(runtimeResult?.currentFindingsEvaluableCount),
    currentFindingsUnknownCount: nonNegativeInteger(runtimeResult?.currentFindingsUnknownCount),
...
export function validateEvaluatorBoundaryPolicyRuntimeTelemetry(telemetry) {
  ...
  for (const key of ["inputCandidateCount", "visibleCandidateCount", "unexpectedReceiverExposureCount", "safetyBlockedCandidateCount", "currentFindingsCount", "currentFindingsEvaluableCount", "currentFindingsUnknownCount", "runtimeExecutionCount", "runtimeErrorCount", "runtimeLatencyMsTotal", "runtimeLatencyMsMax", "disabledExecutionViolationCount"]) {
  ...
  if (telemetry.currentFindingsContextVersion !== null && typeof telemetry.currentFindingsContextVersion !== "string") errors.push("invalid_current_findings_context_version");
  if (!CURRENT_FINDINGS_EXPOSURE_STATES.has(telemetry.currentFindingsExposureState)) errors.push("invalid_current_findings_exposure_state");
  if (telemetry.currentFindingsEvaluableCount > telemetry.currentFindingsCount) errors.push("current_findings_evaluable_count_exceeds_total");
  if (telemetry.currentFindingsUnknownCount > telemetry.currentFindingsCount) errors.push("current_findings_unknown_count_exceeds_total");
