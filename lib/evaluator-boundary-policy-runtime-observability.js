export const EVALUATOR_BOUNDARY_POLICY_RUNTIME_TELEMETRY_VERSION = "2026-07-14.phase46.3a";
...
export function validateEvaluatorBoundaryPolicyRuntimeTelemetry(telemetry) {
  ...
  for (const key of ["runtimeEnabled", "runtimeExecuted", "runtimeConnected", "killSwitchRequested", "killSwitchSuppressedExecution", "scopeValidationFailed", "stopRequired"]) {
    if (typeof telemetry[key] !== "boolean") errors.push(`invalid_${key}`);
  }
  for (const key of ["inputCandidateCount", "visibleCandidateCount", "unexpectedReceiverExposureCount", "runtimeExecutionCount", "runtimeErrorCount", "runtimeLatencyMsTotal", "runtimeLatencyMsMax", "disabledExecutionViolationCount"]) {
...
export function evaluateEvaluatorBoundaryPolicyCanaryGate({ telemetry, comparison = {}, slo = {} } = {}) {
  ...
  if (nonNegativeInteger(comparison.shadowAddedDbMutationDelta) > 0) reasons.push("unexpected_db_delta");
  if (nonNegativeInteger(comparison.shadowAddedStorageMutationDelta) > 0) reasons.push("unexpected_storage_delta");
