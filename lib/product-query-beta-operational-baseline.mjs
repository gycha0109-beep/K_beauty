...
export function parseProductQueryOperationalBaselineStartAt(value) {
...
function validateObservation(event) {
  ...
  if (parseObservationTimestamp(event.timestamp) === null) {
    return Object.freeze({ valid: false, reason: "invalid_timestamp" });
  }
...
export function aggregateProductQueryOperationalBaseline(events = [], options = {}) {
  if (!Array.isArray(events)) {
    throw new TypeError("DATA-AI25 baseline input must be an array");
  }

  const requestedBaselineStartAt = String(
    options.operationalBaselineStartAt ?? ""
  ).trim();
  const operationalBaselineStartAt =
    parseProductQueryOperationalBaselineStartAt(requestedBaselineStartAt);
  if (requestedBaselineStartAt && !operationalBaselineStartAt) {
    throw new TypeError("DATA-AI25 operationalBaselineStartAt must be a valid timestamp");
  }
  const operationalBaselineStartMs = operationalBaselineStartAt
    ? Date.parse(operationalBaselineStartAt)
    : null;
  ...
  let unscopedObservationCount = 0;
  let preBaselineObservationCount = 0;
  ...
  for (const event of events) {
    ...
    if (operationalBaselineStartMs === null) {
      unscopedObservationCount += 1;
      continue;
    }

    const eventTimestampMs = parseObservationTimestamp(event.timestamp);
    if (eventTimestampMs !== null && eventTimestampMs < operationalBaselineStartMs) {
      preBaselineObservationCount += 1;
      continue;
    }
  ...
  return Object.freeze({
    ...
    launchBoundaryEstablished: operationalBaselineStartAt !== null,
    candidateObservationCount,
    unscopedObservationCount,
    preBaselineObservationCount,
