...
function projectScenario(item, locale) {
  ...
  return {
    ...
    maintainRoles: [...(item.maintainRoles || [])],
    reduceActions: [...(item.reduceActions || [])],
    ...(Array.isArray(item.reduceRoles) ? { reduceRoles: [...item.reduceRoles] } : {}),
    pauseRoles: [...(item.pauseRoles || [])],
...
function mergeLegacyFallback(report, responses, conditionPolicy, allowLegacyFallback) {
  ...
  const priorCanonicalResponses =
    report?.conditionPlan?.version === PREMIUM_CONDITION_PROJECTION_VERSION &&
    report?.conditionPlan?.source === "canonical" &&
    Array.isArray(report.conditionPlan.responses)
      ? report.conditionPlan.responses
      : null;
  const legacy = priorCanonicalResponses
    ? priorCanonicalResponses.filter(
        (item) =>
          item?.source === "legacy_snapshot" &&
          item?.legacyCarryover === true
      )
    : Array.isArray(report?.conditionResponses)
      ? report.conditionResponses
      : [];
