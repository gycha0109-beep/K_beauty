...
function collectCandidateReviews(fixtures) {
  return fixtures.flatMap((fixture) =>
    (Array.isArray(fixture.candidateReviewRows)
      ? fixture.candidateReviewRows
      : (Array.isArray(fixture.candidateReviews) ? fixture.candidateReviews : [])
    ).map((item) => normalizeCandidateReviewRow(item, fixture.captureId))
  );
}

function normalizeCandidateReviewRow(item, captureId) {
  const guardDecision = item?.guardDecision || item?.recentInstabilityGuardDecision || null;
  const hardFilterStatus = item?.hardFilterStatus || item?.evaluatorHardFilterStatus || null;

  return {
    ...item,
    captureId: captureId || null,
    candidateConfidence: item?.candidateConfidence || item?.confidence || "unknown",
    evaluatorHardFilterStatus: hardFilterStatus,
    evaluatorHardFilterReasons: Array.isArray(item?.evaluatorHardFilterReasons)
      ? item.evaluatorHardFilterReasons
      : (Array.isArray(item?.hardFilterReasons) ? item.hardFilterReasons : []),
    recentInstabilityGuardDecision: guardDecision,
    recentInstabilityGuardLevel: item?.recentInstabilityGuardLevel || item?.guardLevel || null,
    recentInstabilityGuardReasons: Array.isArray(item?.recentInstabilityGuardReasons)
      ? item.recentInstabilityGuardReasons
      : (Array.isArray(item?.guardReasons) ? item.guardReasons : []),
    exposurePolicyReasons: Array.isArray(item?.exposurePolicyReasons)
      ? item.exposurePolicyReasons
      : (Array.isArray(item?.exposureReasons) ? item.exposureReasons : []),
    blockedBy: item?.blockedBy || {
      evaluator: hardFilterStatus === "blocked",
      guardHardBlock: guardDecision === "hard_block_candidate"
    },
    safetyContext: item?.safetyContext || {
      highSensitivity: false,
      recentInstability: false
    },
    currentProductRelation: item?.currentProductRelation || null,
    currentProductSourceState: item?.currentProductSourceState || null
  };
}
...
function buildHiddenReasonReview(candidates) {
  ...
  const safeLowRiskHiddenByRankingGoal = {};
  const safeLowRiskHiddenBySafetyGoal = {};
  const safeLowRiskHiddenByRecommendationGuard = {};
  const hiddenBlockedSourceDistribution = {};
  const safeLowRiskHiddenBlockedSourceDistribution = {};
  ...
  for (const item of safeLowRiskHidden) {
    ...
    increment(safeLowRiskHiddenByRankingGoal, item.rankingGoal);
    increment(safeLowRiskHiddenBySafetyGoal, item.safetyGoal);
    increment(safeLowRiskHiddenByRecommendationGuard, item.recommendationGuard);
    increment(safeLowRiskHiddenBlockedSourceDistribution, blockedSource(item));
  }
  ...
  return {
    ...
    safeLowRiskHiddenByRankingGoal: sortObject(safeLowRiskHiddenByRankingGoal),
    safeLowRiskHiddenBySafetyGoal: sortObject(safeLowRiskHiddenBySafetyGoal),
    safeLowRiskHiddenByRecommendationGuard: sortObject(safeLowRiskHiddenByRecommendationGuard),
    hiddenBlockedSourceDistribution: sortObject(hiddenBlockedSourceDistribution),
    safeLowRiskHiddenBlockedSourceDistribution: sortObject(safeLowRiskHiddenBlockedSourceDistribution),
    interpretationNotes: [
...
function buildCollapsedReview(fixtures, candidates) {
  ...
  const guardDecisionDistribution = {};
  ...
  for (const item of collapsed) {
    ...
    increment(guardDecisionDistribution, item.guardDecision || item.recentInstabilityGuardDecision);
    increment(implementationHintDistribution, item.implementationHint);
  ...
  return {
    ...
    collapsedGuardDecisionDistribution: sortObject(guardDecisionDistribution),
    collapsedImplementationHintDistribution: sortObject(implementationHintDistribution),
