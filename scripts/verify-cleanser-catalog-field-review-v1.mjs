...
const DIRECT_AUTHORITY_CLASSES = new Set(["official_product_page", "manufacturer_documentation"]);
...
const EXPECTED = {
  ...
  confidenceRubric: "cleanser-catalog-confidence-rubric-v1",
  historicalPreTaxonomyDigest: "2cfa18b985ae76ebd50b7d471f7b242efb633a4e5cbec7cff8e1b8ab823e1f27",
  historicalPreConfidenceDigest: "73645cbcd9bfecdb559297ed2a7bab3e50d9be560dee33460071eadba09b1241",
  productCount: 26,
};
...
function sameArray(actual, expected) { return Array.isArray(actual) && stableJson(actual) === stableJson(expected); }
...
function assertSafeHttps(raw, context) {
  let u;
  try { u = new URL(raw); } catch { fail(`${context}: invalid URL`); }
...
function walkForbiddenKeys(value, trail = "$") {
  if (Array.isArray(value)) { value.forEach((item, i) => walkForbiddenKeys(item, `${trail}[${i}]`)); return; }
  if (!value || typeof value !== "object") return;
  const forbidden = new Set(["candidate_id", "candidateid", "reviewer", "reviewer_id", "reviewerid", "admin_audit", "admin_audit_row", "audit_row", "export_batch_id", "evidence_id"]);
  for (const [key, child] of Object.entries(value)) {
    if (forbidden.has(key.toLowerCase())) fail(`${trail}.${key}: Admin/import lineage key is forbidden in catalog corpus`);
    walkForbiddenKeys(child, `${trail}.${key}`);
  }
}
function destination(product) { return product.review_state === "reviewed_valid" ? product.reviewed_profile : product.review_state; }
...
if (partPaths.length !== 4) fail("product_parts must contain four cleanser corpus shards");
const products = [];
for (const [partIndex, relativePath] of partPaths.entries()) {
  if (typeof relativePath !== "string" || !relativePath.startsWith("evidence/catalog/cleanser-field-review-v1.part-")) fail(`product_parts[${partIndex}]: invalid corpus shard path`);
  const part = JSON.parse(fs.readFileSync(path.resolve(here, "..", relativePath), "utf8"));
...
if (corpus.confidence_rubric_version !== EXPECTED.confidenceRubric) fail("confidence rubric mismatch");
if (corpus.historical_pre_taxonomy_correction_sha256 !== EXPECTED.historicalPreTaxonomyDigest) fail("historical pre-taxonomy digest mismatch");
if (corpus.historical_pre_confidence_finalization_sha256 !== EXPECTED.historicalPreConfidenceDigest) fail("historical pre-confidence digest mismatch");
...
if (corpus.product_count !== EXPECTED.productCount || !Array.isArray(corpus.products) || corpus.products.length !== EXPECTED.productCount) fail("product count must equal 26");
...
const confidenceCounts = Object.fromEntries(ALLOWED_CONFIDENCE.map((key) => [key, 0]));
...
const transition = {};
for (const legacy of ["low_ph", "balanced", "deep_clean", "null"]) transition[legacy] = { low_ph: 0, balanced: 0, deep_clean: 0, reviewed_unknown: 0, reviewed_conflict: 0, not_applicable: 0 };
let legacyUnchanged = 0, legacyChanged = 0, legacyToNull = 0, nullToValid = 0, evidenceTotal = 0;
const sourceUrls = new Set();

for (const [index, product] of corpus.products.entries()) {
  const label = `products[${index}]`;
  if (typeof product.product_id !== "string" || !product.product_id || ids.has(product.product_id)) fail(`${label}: missing or duplicate product_id`);
  ...
  if (!ALLOWED_CONFIDENCE.includes(product.confidence)) fail(`${label}: invalid confidence`);
  if (!ALLOWED_READINESS.includes(product.admin_v2_ingestion_readiness)) fail(`${label}: invalid admin_v2_ingestion_readiness`);
  if (typeof product.admin_v2_ingestion_readiness_rationale !== "string" || !product.admin_v2_ingestion_readiness_rationale.trim()) fail(`${label}: ingestion readiness rationale missing`);
  ...
  confidenceCounts[product.confidence] += 1;
  ...
  for (const [eIndex, evidence] of product.evidence.entries()) {
    ...
    if ("evidence_type" in evidence) fail(`${eLabel}: legacy evidence_type is forbidden`);
    if (typeof evidence.catalog_evidence_id !== "string" || !/^cfrv1-\d{2}-\d{2}$/.test(evidence.catalog_evidence_id) || evidenceIds.has(evidence.catalog_evidence_id)) fail(`${eLabel}: invalid or duplicate catalog_evidence_id`);
    ...
    if (evidence.admin_v2_evidence_type_candidate !== null && !ALLOWED_ADMIN_V2_EVIDENCE_TYPES.includes(evidence.admin_v2_evidence_type_candidate)) fail(`${eLabel}: invalid admin_v2_evidence_type_candidate`);
    if (typeof evidence.admin_v2_ingestion_eligible !== "boolean") fail(`${eLabel}: admin_v2_ingestion_eligible must be boolean`);
    if (evidence.admin_v2_ingestion_eligible !== (evidence.admin_v2_evidence_type_candidate !== null)) fail(`${eLabel}: ingestion eligibility/mapping mismatch`);
    if (NON_INGESTIBLE_SOURCE_CLASSES.has(evidence.source_class) && (evidence.admin_v2_evidence_type_candidate !== null || evidence.admin_v2_ingestion_eligible)) fail(`${eLabel}: non-ingestible source class must remain unmapped`);
    if (["retailer_product_page", "marketplace_product_page", "price_comparison_product_page"].includes(evidence.source_class) && evidence.admin_v2_evidence_type_candidate === "manufacturer_documentation") fail(`${eLabel}: commerce evidence must never map to manufacturer_documentation`);
    if (evidence.admin_v2_ingestion_eligible && DIRECT_MAPPINGS.get(evidence.source_class) !== evidence.admin_v2_evidence_type_candidate) fail(`${eLabel}: ingestion-eligible mapping does not match source class`);
    if (evidence.source_class === "manual_conflict_record") {
      ...
      if (!/does not itself prove a physical product attribute/i.test(evidence.evidence_summary ?? "")) fail(`${eLabel}: manual conflict record must state adjudication-only role`);
    }
  ...
  const supportingEvidence = product.evidence.filter((e) => e.supported_value === product.reviewed_profile);
  const directAuthoritySupport = supportingEvidence.filter((e) => e.admin_v2_ingestion_eligible === true && DIRECT_AUTHORITY_CLASSES.has(e.source_class));
  if (product.review_state === "reviewed_valid") {
    if (product.reviewed_profile === null || product.confidence === "unknown" || supportingEvidence.length < 1) fail(`${label}: invalid reviewed_valid tuple`);
    if (product.confidence === "high" && directAuthoritySupport.length < 1) fail(`${label}: high confidence requires ingestion-eligible official/manufacturer evidence directly supporting reviewed_profile`);
    if (product.confidence === "low" && directAuthoritySupport.length > 0) fail(`${label}: low confidence is inconsistent with direct official/manufacturer authority`);
    if (product.confidence === "medium") {
      if (directAuthoritySupport.length > 0) fail(`${label}: medium confidence is inconsistent with direct official/manufacturer authority; high is required by the locked rubric`);
      const rationale = product.confidence_rationale ?? product.admin_v2_ingestion_readiness_rationale;
      if (typeof rationale !== "string" || !rationale.trim()) fail(`${label}: medium confidence requires rationale explaining why high is not warranted`);
    }
  } else {
    if (product.reviewed_profile !== null || product.confidence !== "unknown") fail(`${label}: non-valid review states require null profile and unknown confidence`);
    if (product.review_state === "reviewed_conflict") {
  ...
  if (product.admin_v2_ingestion_readiness === "eligible_from_current_evidence") {
    ...
    if (product.review_state === "reviewed_valid" && !ingestionSupport.some((e) => e.supported_value === product.reviewed_profile)) fail(`${label}: ingestion-ready reviewed_valid product lacks eligible support`);
    if (product.review_state !== "reviewed_valid" && ingestionSupport.length < 1) fail(`${label}: ingestion-ready non-valid state requires eligible evidence`);
  } else if (product.admin_v2_ingestion_readiness === "evidence_upgrade_required") {
    if (product.review_state === "reviewed_valid" && ingestionSupport.some((e) => e.supported_value === product.reviewed_profile)) fail(`${label}: evidence_upgrade_required already has eligible reviewed-value support`);
  } else if (product.admin_v2_ingestion_readiness === "schema_mapping_required" && product.review_state !== "reviewed_conflict") {
    fail(`${label}: schema_mapping_required is reserved for reviewed_conflict`);
  }
  ...
  else if (product.reviewed_profile !== null) legacyChanged += 1;
...
if (confidenceCounts.unknown !== stateCounts.reviewed_unknown + stateCounts.reviewed_conflict + stateCounts.not_applicable) fail("unknown confidence count must match all non-valid review states");
...
const expectedSummary = {
  ...
  legacy_comparison: { legacy_unchanged: legacyUnchanged, legacy_changed: legacyChanged, legacy_to_null: legacyToNull, null_to_valid: nullToValid },
...
for (const [key, value] of Object.entries(corpus.production_invariance ?? {})) if (value !== 0) fail(`production_invariance.${key} must be 0`);
for (const required of ["db_writes", "hosted_migrations", "admin_activation", "recommendation_activation", "score_ranking_delta", "pr_167_delta"]) if (!(required in (corpus.production_invariance ?? {}))) fail(`production_invariance.${required} missing`);
...
if (actualDigest === EXPECTED.historicalPreTaxonomyDigest || actualDigest === EXPECTED.historicalPreConfidenceDigest) fail("final confidence digest must differ from historical digests");
...
console.log(`confidence_high=${confidenceCounts.high} confidence_medium=${confidenceCounts.medium} confidence_low=${confidenceCounts.low} confidence_unknown=${confidenceCounts.unknown}`);
