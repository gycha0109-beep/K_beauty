...
const p3ModulePath = path.join(
  p3ReferenceRoot,
  manifest.execution_authority.p3_materializer_path
);
...
const coverage = sourceSet.personas.filter((persona) => persona.cohort_type === "COVERAGE_COHORT");
const adversarial = sourceSet.personas.filter((persona) => persona.cohort_type === "ADVERSARIAL_COHORT");
const population = sourceSet.personas.filter((persona) => persona.cohort_type === "POPULATION_PRIOR_COHORT");

invariant(coverage.length === 32, "Coverage source count mismatch");
invariant(adversarial.length === 8, "Adversarial source count mismatch");
invariant(population.length === 0, "P3 source unexpectedly contains Population-Prior Persona");
...
for (const cohort of [coverageLock, adversarialLock]) {
  ...
  invariant(cohort.prng_algorithm === "NONE" && cohort.seed === 0, "P3 deterministic enumeration lineage drift", cohort);
}
...
invariant(new Set([...coverageIds, ...adversarialIds]).size === 40, "Locked technical cohort membership overlap detected");
...
invariant(
  manifest.regression_cohort.lifecycle === "NOT_CREATED",
  "P4 must not silently create a LOCKED_REGRESSION_COHORT"
);
invariant(
  manifest.authority_ceiling.evidence_class === "SYNTHETIC_SIMULATION_EVIDENCE",
...
invariant(new Set(coverageDomainHashes).size === 32, "Coverage Persona collapse detected");
invariant(new Set(adversarialDomainHashes).size === 8, "Adversarial Persona collapse detected");
invariant(new Set([...coverageDomainHashes, ...adversarialDomainHashes]).size === 40, "Cross-cohort domain-state collapse detected");

const allPersonas = [...coverage, ...adversarial];
const summary = {
  ...
  collapse_diagnostics: {
    combined_persona_count: allPersonas.length,
    unique_domain_patterns: new Set([...coverageDomainHashes, ...adversarialDomainHashes]).size,
    skin_type_distribution: countBy(allPersonas.map((persona) => persona.domain.skinType)),
    sensitivity_distribution: countBy(allPersonas.map((persona) => persona.domain.sensitivity)),
    primary_concern_distribution: countBy(allPersonas.map((persona) => persona.domain.primaryConcern)),
    sunscreen_preference_state_distribution: countBy(allPersonas.map((persona) => persona.domain.sunscreen.preferenceState))
  },
...
fs.writeFileSync(
  path.join(artifactRoot, "cohort-freeze-summary-v1.json"),
  `${JSON.stringify(summary, null, 2)}\n`
);
fs.writeFileSync(
  path.join(artifactRoot, "locked-cohort-manifest-v1.json"),
...
console.log(`unique_domain_patterns=${summary.collapse_diagnostics.unique_domain_patterns}/40`);
