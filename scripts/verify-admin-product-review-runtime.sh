  ...
  case "${status}" in
    ...
    *)
      if [[ -n "${output_file}" && -f "${output_file}" ]]; then
        jq -c '{code,message,details,hint}' "${output_file}" 2>/dev/null >&2 || cat "${output_file}" >&2
      fi
...
create_user() {
  ...
  status="$(request_status "${output_file}" \
    ...
    --header 'Content-Type: application/json' \
    --data "${payload}")"
  assert_success "${status}" "create user ${email}" "${output_file}"
...
login_user() {
  ...
  status="$(request_status "${output_file}" \
    ...
    --header 'Content-Type: application/json' \
    --data "${payload}")"
  assert_success "${status}" "login user ${email}" "${output_file}"
...
row_count() {
  ...
  assert_success "${status}" "count ${table_path}" "${output_file}"
...
seed_candidate() {
  ...
  local brand_name="$4"
  local product_form="$5"
  local matched_product_id="${6:-}"
  local output_file="${TEMP_DIR}/candidate-${candidate_id}.json"
  local payload
  local status
  ...
  payload="$(jq -cn \
    ...
    --arg matched_product_id "${matched_product_id}" \
    '{
      id:$id,
      source_name:"hwahae",
      external_type:"goods",
      ...
      normalized_name:($product_name|ascii_downcase|gsub("[^a-z0-9]";"")),
      normalized_brand:($brand_name|ascii_downcase|gsub("[^a-z0-9]";"")),
      service_category:"treatment",
      product_form:(if $product_form == "__NULL__" then null else $product_form end),
      canonical_name:$product_name,
      canonical_brand:$brand_name,
      review_status:"needs_review",
      matched_product_id:(if $matched_product_id == "" then null else $matched_product_id end),
      promotion_payload:{product:{skin_types:["combination","sensitive"],concerns:["acne","redness"],texture:"gel",finish:"natural",irritation_risk:"low",sensitivity_safe:true,price_min:19000,price_max:25000,buy_link:null,image_url:null}}
    }')"

  status="$(rest_call "${service_role_key}" POST product_candidates "${payload}" "${output_file}")"
  assert_success "${status}" "seed candidate ${external_id}" "${output_file}"
}
...
seed_review() {
  ...
  local reason="$3"
  local rank="$4"
  local output_file="${TEMP_DIR}/review-${candidate_id}.json"
  local payload
  local status
  ...
  payload="$(jq -cn \
    ...
    --arg reason "${reason}" \
    --argjson rank "${rank}" \
    '{candidate_id:$candidate_id,status:"queued",priority_score:$priority,selection_reason:$reason,evidence_snapshot:{concerns:[{concern:"acne",latest_rank:$rank}]},rule_version:"ranking-review-v2"}')"

  status="$(rest_call "${service_role_key}" POST candidate_promotion_reviews "${payload}" "${output_file}")"
  assert_success "${status}" "seed review ${candidate_id}" "${output_file}"
}
...
bootstrap_status="$(rpc_call \
  "${service_role_key}" bootstrap_first_admin_owner \
  "$(jq -cn --arg id "${owner_id}" '{p_user_id:$id}')" \
  "${bootstrap_file}")"
assert_success "${bootstrap_status}" "bootstrap owner" "${bootstrap_file}"
...
assert_success "${membership_status}" "create operator and viewer memberships" "${membership_file}"
...
assert_success "${premium_role_status}" "premium role lookup" "${premium_role_file}"
...
assert_success "${existing_product_status}" "seed existing product" "${existing_product_file}"

seed_candidate "${insert_candidate_id}" insert-1 "New Calm Serum" "New Brand" serum
seed_candidate "${merge_candidate_id}" merge-1 "Existing Serum" "Existing Brand" serum "${existing_product_id}"
seed_candidate "${missing_form_candidate_id}" missing-form-1 "Unknown Treatment" "Unknown Brand" __NULL__
seed_candidate "${defer_candidate_id}" defer-1 "Deferred Serum" "Deferred Brand" serum
seed_candidate "${block_candidate_id}" block-1 "Blocked Serum" "Blocked Brand" serum
seed_candidate "${stale_candidate_id}" stale-1 "Stale Serum" "Stale Brand" serum

seed_review "${insert_candidate_id}" 100 "top rank" 1
seed_review "${merge_candidate_id}" 90 "persistent" 2
seed_review "${missing_form_candidate_id}" 80 "needs metadata" 3
seed_review "${defer_candidate_id}" 70 "needs source" 4
seed_review "${block_candidate_id}" 60 "identity risk" 5
seed_review "${stale_candidate_id}" 50 "stale test" 6
...
assert_success "${missing_preflight_status}" "missing product form preflight" "${missing_preflight_file}"
...
assert_success "${insert_preflight_status}" "insert candidate preflight" "${insert_preflight_file}"
...
assert_success "${insert_confirm_status}" "confirm inserted product" "${insert_confirm_file}"
...
assert_success "${insert_retry_status}" "idempotent insert retry" "${insert_retry_file}"
...
assert_success "${merge_preflight_status}" "merge preflight" "${merge_preflight_file}"
...
assert_success "${merge_confirm_status}" "merge confirm" "${merge_confirm_file}"
...
assert_success "${defer_preflight_status}" "defer preflight" "${defer_preflight_file}"
...
assert_success "${defer_confirm_status}" "defer confirm" "${defer_confirm_file}"
...
assert_success "${block_preflight_status}" "block preflight" "${block_preflight_file}"
...
assert_success "${block_confirm_status}" "block confirm" "${block_confirm_file}"
...
assert_success "${stale_preflight_status}" "stale preflight" "${stale_preflight_file}"
...
stale_patch_status="$(rest_call \
  ...
  "${stale_patch_file}")"
assert_success "${stale_patch_status}" "mutate evidence after preflight" "${stale_patch_file}"
...
owner_audit_status="$(rest_call \
  ...
  '' "${owner_audit_file}")"
assert_success "${owner_audit_status}" "owner audit read" "${owner_audit_file}"
...
viewer_audit_status="$(rest_call \
  ...
  '' "${viewer_audit_file}")"
assert_success "${viewer_audit_status}" "viewer audit read" "${viewer_audit_file}"
