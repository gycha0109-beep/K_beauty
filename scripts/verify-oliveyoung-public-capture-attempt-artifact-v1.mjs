...
function assertExactKeys(value, expected, label) {
  assert.equal(value !== null && typeof value === "object" && !Array.isArray(value), true, `${label}_not_object`);
...
const metadataName = `oliveyoung-${OLIVEYOUNG_PUBLIC_CAPTURE_GOODS_NO}-capture-metadata.json`;
const metadataPath = path.join(inputDir, metadataName);
const metadata = JSON.parse(await fs.readFile(metadataPath, "utf8"));

assert.equal(metadata.schema_version, "oliveyoung_public_listing_capture_attempt_v1");
assertIsoTimestamp(metadata.attempted_at, "attempted_at");
assert.deepEqual(metadata.capture_policy, OLIVEYOUNG_PUBLIC_CAPTURE_POLICY);
...
if (metadata.outcome === "captured_source") {
  ...
  assert.equal(metadata.response.requested_url, OLIVEYOUNG_PUBLIC_CAPTURE_TARGET);
  assert.match(metadata.response.final_url, /^https:\/\/www\.oliveyoung\.co\.kr\/store\/goods\/getGoodsDetail\.do\?/);
  assert.equal(metadata.response.status >= 200 && metadata.response.status < 300, true);
  assert.equal(metadata.evidence.seller, "oliveyoung");
  assert.equal(metadata.evidence.listing_url, OLIVEYOUNG_PUBLIC_CAPTURE_TARGET);
  assert.equal(metadata.evidence.source_version, OLIVEYOUNG_PUBLIC_CAPTURE_VERSION);
  ...
  assert.equal(fixture.evidence.listing_url, OLIVEYOUNG_PUBLIC_CAPTURE_TARGET);
  assert.equal(
    payload.includes(Buffer.from(OLIVEYOUNG_PUBLIC_CAPTURE_GOODS_NO, "utf8")),
  ...
  assert.equal(metadata.target, OLIVEYOUNG_PUBLIC_CAPTURE_TARGET);
  assert.equal(metadata.error_code, "oliveyoung_public_capture_http_status:403");
  await assert.rejects(
    fs.access(path.join(inputDir, `oliveyoung-${OLIVEYOUNG_PUBLIC_CAPTURE_GOODS_NO}-parser-fixture.json`)),
  );
} else {
  throw new Error(`unsupported_capture_outcome:${metadata.outcome}`);
}

console.log(`oliveyoung_public_capture_attempt_artifact_v1: PASS (${metadata.outcome})`);
