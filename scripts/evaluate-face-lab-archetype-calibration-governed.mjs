...
function loadEvaluator() {
  const observation = loadModule("lib/face-lab-observation-contract.js", ["FACE_LAB_OBSERVATION_DEFINITIONS"]);
...
function parseArgs(argv) {
  const args = { dataset: null, policies: null, split: "validation", output: null, allowHoldout: false, confirm: null };
...
function resolveInput(value, label) {
  ...
  if (!isInside(rootReal, fileReal)) throw new Error(`${label} must stay under private/face-lab-calibration/`);
  if (statSync(fileReal).size > MAX_INPUT_BYTES) throw new Error(`${label} exceeds the 2 MiB limit`);
  return fileReal;
}
...
try {
  ...
  const report = loadEvaluator()({
    dataset: readJson(resolveInput(args.dataset, "dataset"), "dataset"),
    policySet: readJson(resolveInput(args.policies, "policies"), "policies"),
    split: args.split,
