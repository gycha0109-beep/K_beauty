...
const ineligiblePhoto = build(baseReport({
  photoEvidenceState: {
    status: "ineligible",
    failureReason: "non_photorealistic_face",
  ...
  imageEligibility: {
    ...
    confidence: 0.99,
    evidence: ["The image contains a product and no visible human face."]
  }
}));
equal(ineligiblePhoto.context.photo.status, "ineligible", "ineligible input remains distinct");
equal(ineligiblePhoto.context.photo.failureClass, "input_ineligible", "input failure class remains explicit");
equal(ineligiblePhoto.context.photo.analysisEligible, false, "explicit false eligibility remains false");
check(ineligiblePhoto.context.uncertaintyState.reasons.includes("photo_input_ineligible"), "ineligible input caps certainty");

const providerUnavailable = build(baseReport({
  photoEvidenceState: {
    status: "unavailable",
...
equal(providerUnavailable.context.photo.status, "unavailable", "provider unavailable is not a skin result");
equal(providerUnavailable.context.photo.failureClass, "provider_unavailable", "provider failure class remains explicit");
check(providerUnavailable.context.uncertaintyState.reasons.includes("photo_provider_unavailable"), "provider unavailable caps certainty");

const explicitFalse = build(baseReport({
  imageEligibility: {
  ...
  photoEvidenceState: null
}));
equal(explicitFalse.context.photo.status, "insufficient_evidence", "false skin eligibility is not missing");
equal(explicitFalse.context.photo.analysisEligible, false, "false and missing remain distinct");
equal(explicitFalse.context.photo.failureReason, "heavy_filter_or_editing", "eligibility reason is preserved");
