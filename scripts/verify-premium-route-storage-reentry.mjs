...
const legacyKoReport = structuredClone(baseReport);
delete legacyKoReport.locale;
...
const legacyKo = snapshotModule.buildPremiumReportSnapshot(legacyKoReport);
...
assert.equal(first.fingerprint, legacyKo.fingerprint, "legacy reports without locale must replay as deterministic Korean snapshots");
...
assert.ok(
  fullRoute.includes("verifyPremiumReportSession(premiumCookie, { userId: user?.id })"),
...
assert.ok(
  sessionRoute.includes("verifyPremiumReportSession(premiumCookie, { userId: routeContext.user.id })"),
