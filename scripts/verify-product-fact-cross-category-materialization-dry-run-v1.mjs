...
function validate(o,expected,{strict=true}={}){
  ...
  ok(o.blocked_materialization.some(b=>b.kind==="relationship_identity_blocked"&&b.pilot_id==="T2"&&/sodium_hyaluronate_crosspolymer/i.test(b.candidate_concept||"")),"HA concentration remains RNE relationship-blocked");
  ...
  ok(o.blocked_materialization.filter(b=>b.pilot_id==="P3"&&b.kind==="relationship_identity_blocked"&&b.semantic_status==="reviewed_not_established").length>=2,"P3 acid concentrations remain not established");
