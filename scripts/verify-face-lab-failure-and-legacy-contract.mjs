...
const {
  ...
  ["buildPremiumFaceLabSummary", "sanitizePremiumFaceLabSummary"],
  { buildFaceLabLaunchData }
);
...
assert.ok(diagnosisStep.includes("사진 인식에 실패했습니다"));
assert.equal(
  (diagnosisStep.match(/사진 인식에 실패했습니다/g) || []).length,
  1,
  "failure copy must exist only in the dedicated display branch"
);
...
assert.ok(fullReportRoute.includes("faceLabSource: null"));
assert.ok(fullReportRoute.includes("if (legacyFaceLabSummary?.status === \"available\")"));
assert.ok(fullReportRoute.includes("delete responsePremiumReport.faceLab"));

console.log("Face Lab failure display and legacy compatibility checks passed.");
