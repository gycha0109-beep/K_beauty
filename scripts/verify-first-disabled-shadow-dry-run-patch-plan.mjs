...
const OUTPUT_PATH = path.join(ROOT, "tmp", "first-disabled-shadow-dry-run-patch-plan.json");
const MD_OUTPUT_PATH = path.join(ROOT, "tmp", "first-disabled-shadow-dry-run-patch-plan.md");
...
function runReviewScript() {
  const stdout = execFileSync(process.execPath, ["scripts/review-first-disabled-shadow-dry-run-patch-plan.mjs"], {
    cwd: ROOT,
    encoding: "utf8",
    env: process.env
  });
  assert(stdout.includes("first-disabled-shadow-dry-run-patch-plan summary"));
  assert(existsSync(OUTPUT_PATH), "patch plan JSON should exist");
  assert(existsSync(MD_OUTPUT_PATH), "patch plan markdown should exist");
...
function assertPatchPlan(output) {
  assert.equal(output.evidenceType, "first_disabled_shadow_dry_run_patch_plan");
  ...
  assert(Array.isArray(output.futurePatchScope));
  assert(output.futurePatchScope.length >= 4);
  assert(Array.isArray(output.featureFlagContract.candidateFlags));
  assert.equal(output.featureFlagContract.valuesPrinted, false);
  assert.equal(output.minimalRouteInsertionBlueprint.recommendedInsertionPoint, "route_outside_helper_dev_only_artifact_writer");
  assert.equal(output.minimalRouteInsertionBlueprint.phase38AppliesPatch, false);
  assert(Array.isArray(output.snapshotBuildSequence));
  assert(output.snapshotBuildSequence.length >= 8);
  assert(Array.isArray(output.artifactWriterPlan.rules));
  assert(Array.isArray(output.requiredVerifierChain));
  assert(output.requiredVerifierChain.length >= 10);
  ...
  assert(output.phase39AllowedScope.includes("first_disabled_shadow_dry_run_minimal_patch"));
  assert(output.phase39AllowedScope.includes("dev_only_flag_guard_addition"));
  assert(output.phase39ProhibitedScope.includes("evaluator_runtime_connection"));
  assert(output.phase39ProhibitedScope.includes("candidate_policy_runtime_connection"));
  assert(output.phase39ProhibitedScope.includes("api_response_change"));
  assert(output.phase39ProhibitedScope.includes("recommendation_result_change"));
  assert(output.phase39ProhibitedScope.includes("db_or_supabase_schema_change"));
  assert(output.phase39ProhibitedScope.includes("production_activation"));
}
...
function assertNoRuntimeConnections() {
  ...
  assert.equal(joinedRuntime.includes("review-first-disabled-shadow-dry-run-patch-plan"), false);
  assert.equal(joinedRuntime.includes("first-disabled-shadow-dry-run-patch-plan"), false);
...
function assertNoLeakage() {
  ...
  for (const pattern of FORBIDDEN_VALUE_PATTERNS) {
    assert(!pattern.test(serialized), `patch plan output leaked forbidden value pattern: ${pattern}`);
  }
}

const first = runReviewScript();
assertPatchPlan(first);
...
assert.deepEqual(
  ...
  "patch plan output should be deterministic apart from generatedAt"
);

console.log("verify-first-disabled-shadow-dry-run-patch-plan passed");
