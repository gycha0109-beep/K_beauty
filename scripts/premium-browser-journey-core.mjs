...
function assertSafeObject(value, path = "body") {
  ...
  for (const [key, item] of Object.entries(value)) {
    requireCondition(!FORBIDDEN_KEYS.has(key), FAILURE_CATEGORIES.PRECONDITION, "configuration", "conflict_body_contains_control_key", `${path}.${key}`);
...
export function inspectStorageState(storageState) {
...
export async function deleteSavedReportById(config, id) {
  const rows = await supabaseRest({
    ...
    path: `saved_reports?id=eq.${encodeURIComponent(id)}&select=id`
  });
