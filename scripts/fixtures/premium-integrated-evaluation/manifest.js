...
const activePoreReport = (answers = {}) => { const base = baseReport(); return { ...base, freeResult: { ...base.freeResult, priority: { axis: "pores", score: 25 }, scoring: { concernScores: { ...sensitiveScores, pores: { total: 25 } } }, answers: { ...base.freeResult.answers, sensitivity: "high", ...answers } }, currentProducts: { selections: [selected(active)], summary: { total: 1, selectedCount: 1 } } }; };
export const fixtureManifest = {
  ...
  scenarios: [
    ...
    scenario("S14_STABILIZATION_FALLBACK_BYPASS_ATTEMPT", "Fallback bypass attempt", { ...activePoreReport({ recentSkinChange: "yes" }), topPick: { id: "stale-top", category: "treatment" }, budgetAlternatives: [{ id: "stale-budget", category: "treatment" }], conditionResponses: [{ responseKey: "active_load", status: "avoid_for_now", legacyCarryover: true }] }, { projections: [{ path: "functionalPlan.productCandidates", operator: "length_equals", expected: 0 }, { path: "functionalPlan.budgetAlternatives", operator: "length_equals", expected: 0 }], invariants: [...common, "RAW_POLICY_IMMUTABILITY", "FALLBACK_PROJECTION_ENFORCEMENT"] }, { catalogProducts: [active] }),
