...
### 2026-07-22 / Hosted Preview Account B premium-access failure

- Task type: High-risk hosted Auth/Premium validation. Auth, Provider, Secret, and hosted Preview execution were `Y`; no source, Site URL, OAuth-provider, RLS, schema, migration, deployment, or entitlement configuration was changed.
- Account B completed Google PKCE login on the immutable Preview and was distinct from Account A by hashed Supabase user identity. Account B had Google as provider and no `premium_entitlement`; Account A retained the temporary `paid` entitlement.
- Failure: the Account B `/my` UI exposed an Account A premium saved-report link. Direct navigation to that exact saved-report URL rendered the full Premium Report body, and a full page reload rendered the same body again. The saved report's owner hash matches Account A; Account B has no premium saved-report rows.
- The current full-report route statically filters saved reports by both `id` and `user_id`, so the observed hosted response indicates a runtime principal/session boundary discrepancy or another hosted data-path defect. The failure is not treated as a UI-only condition. Browser console contained only the pre-existing Multiple GoTrueClient warning; Vercel historical log retrieval returned no request lines.
- Stop condition: Account B premium deny failed. Account A entitlement and all temporary Preview Redirect URL entries were intentionally retained; no cleanup was attempted. Evidence is stored outside the repository under the validation temp directory.
- Root cause: the browser code maintained two independent GoTrue clients and auth stores. Premium report reads could fall back from the cookie-backed SSR client to a legacy localStorage client and send that store's bearer token, while `/my` used the cookie principal. This made the UI identity and report-request principal diverge; the saved-report query and deployed RLS were already owner-scoped.
- Fix: premium report reads now use only same-origin auth cookies; the duplicate localStorage GoTrue client was removed. Saved report reads additionally require a paid/admin entitlement, an exact owner/id/type/payload match, and return a generic private 404 on failure. `/my` suppresses premium rows for non-entitled users. User-specific Auth, dashboard, and premium responses are dynamic/private/no-store, and Supabase middleware now preserves cache-safety headers supplied with auth cookie refreshes.
- Regression coverage: `verify-premium-report-ownership-boundary.mjs` covers owner allow, cross-owner/anonymous/non-entitled/missing-id/non-premium/missing-payload deny, service-role exclusion, owner-scoped RLS, single browser auth storage, private cache headers, and dashboard link suppression. Related auth-origin, premium route/reentry, and analysis RLS verifiers also pass.

### 2026-07-22 / Premium session account-switch ownership boundary

- Independent review found that saved-report reads were owner-scoped, but the service-role-backed `premium_report_sessions` record and its signed cookie were not bound to the authenticated account. Signing out also left the path-scoped premium cookie intact. A second entitled account in the same browser could therefore replay the first account's unfinalized premium session after an account switch.
- Premium session tokens now contain only an HMAC owner binding, never a raw user identifier. Creation, read, update, and rotation require the same verified account identity; legacy unbound tokens and cross-account tokens fail closed. Sign-out explicitly expires the premium report cookie using its original path and security attributes.
- Regression coverage exercises owner allow, cross-owner deny, missing-binding deny, raw-identifier non-disclosure, all session call-site bindings, and sign-out expiry. No schema, RLS, provider, entitlement, redirect, environment, or production setting was changed.
- The first focused verifier run failed because its ordering assertion matched the old single-line update call exactly. The assertion now anchors the unchanged update assignment prefix while preserving the original finalized-before-mutation ordering requirement; runtime code was not relaxed to satisfy the verifier.

### 2026-07-22 / Premium security evidence closure

- Task type: High-risk Auth/security review with bounded test-only execution. Auth, Secret handling, Supabase configuration, and hosted Preview execution were `Y`; production application behavior, schema, RLS, provider settings, Site URL, payment, and deployment code were unchanged.
- Gap found: the existing route/re-entry verifier executed the principal selector and owner-binding helper, but token signature, payload tamper, expiry, missing-session, update, and route-result cases still depended partly on source assertions.
- Test-only closure: `verify-premium-security-evidence-closure.mjs` loads the production principal, route-context, full-report route, owner-binding, and premium-session modules inside isolated dependency stubs. It deterministically covers A/B conflicts in both directions, stale and malformed bearer handling, bearer-only and aligned principals, correct/missing/mismatched/legacy/tampered/expired/malformed tokens, missing sessions, and create/read/update/rotation boundaries without printing token, UUID, cookie, or secret material.
- Hosted cache evidence: repeated protected-deployment requests returned `private, no-store, max-age=0, must-revalidate`, `Vary: Cookie, Authorization`, `Age: 0` where present, and `X-Vercel-Cache: MISS`; the unauthenticated `/my` redirect was public but carried no authenticated user data. Existing A/B/A and same-browser account-switch checks showed no shared Premium payload reuse.
- Verification: focused ownership/auth-origin/re-entry/RLS checks, the new execution-based closure verifier, JavaScript syntax check, architecture guard, production build, diff check, and scoped secret/range checks passed. Application code was not modified.
