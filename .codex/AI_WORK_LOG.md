...
### 2026-07-31 / Admin access and product candidate reviews local independent re-audit

- Task type: High-risk review followed by bounded execution on `feature/admin-product-candidate-reviews`. DB/Auth/RLS/Secret handling and local runtime were `Y`; hosted Supabase, Production data/configuration, deployment, GitHub Actions, commit, push, merge, and actual administrator grants were unchanged.
- Branch evidence: local/remote target HEAD remained `6f7d67ee958cb1059563de0e0b4e35bac7db3309`; PR #95 still targets `feature/admin-access-foundation`. The current base HEAD is two no-op workflow-registration commits ahead of the target ancestry, while target is 42 commits ahead of their merge base and 61 commits ahead of `origin/main`.
- Important fixes: added a follow-up migration that rejects sensitive nested audit payload keys/values and fail-closes conflicting existing-product identity references; strengthened Origin/Referer, bounded streaming body, JSON-object, UUID/hash/timestamp/request-id validation; retained one request id across UI retries with in-flight guards; and surfaced source/observation/identity/evidence plus explicit raw-candidate approval blockers. Product navigation now uses the products-read capability.
- Isolated verification: Supabase CLI `2.109.1` replayed the product foundation fixture, admin access migration, both existing review migrations, and the new hardening migration on dedicated ports. SQL runtime covered owner/operator/viewer/privacy/Premium-override boundaries, browser RPC denial, NULL validation, identity conflict, dry-run zero writes, insert, merge, duplicate prevention, defer/block zero writes, request retry/conflict, candidate/review/evidence staleness, audit visibility/tamper/sensitive-payload denial, membership escalation denial, and forced audit-failure transaction rollback. The HTTP admin-access runtime verifier also passed.
- Local application verification: authenticated local preflight returned the fixed not-found contract, cross-origin Referer and missing Origin returned 403, malformed UUID and non-object confirm body returned 400, an over-8-KiB streamed body returned 413, and all responses retained no-store headers. The admin review page rendered both the empty state and an isolated raw-candidate state with approval-block guidance; exact temporary data was deleted.
- Verification: admin access verifier, product review verifier, JavaScript syntax `356/356`, architecture guard, production build, migration replay, role/RLS/RPC matrix, local API/UI probes, and diff check passed. The isolated Supabase project and test files were removed; the unrelated existing `BuildMap` containers remained running.
- Error log: Docker first hit occupied default port `54322`, so only the isolated config moved to `55320`-`55329`. WSL bash was unavailable and Git Bash lacked `jq`; the repository script passed in a disposable Debian container using a CRLF-normalized temporary copy. The first route probe used `127.0.0.1` while Next normalized the runtime host to `localhost`, then the first authenticated probe used the prior dummy build-time public Supabase config; the local-runtime build and canonical localhost origin corrected both. Windows curl inline JSON quoting produced one false 400 and was replaced by exact temporary JSON files. No failed attempt was treated as product evidence.

### 2026-07-31 / ADMIN-PRODUCT-2 review export and enrichment intake

- Task type: High-risk bounded implementation and local verification on `feature/admin-product-review-export-intake`, stacked from clean `feature/admin-product-candidate-reviews` HEAD `33b26f3c95486d69251ebdc37a32fa2554e51abf`. Hosted Supabase, production data, GitHub Actions, deployment, commit, push, PR, product mutation, review mutation, audit mutation, and import confirm were out of scope and unchanged.
- Contract: added a crawler-owned, read-only export command producing deterministic UTF-8/LF `manifest.csv`, `evidence.jsonl`, `batch.json`, and `reviewed-template.csv`; added strict reviewed CSV intake with batch/file/evidence/row SHA-256 checks, current candidate/review/evidence snapshot comparison, decision/enums/evidence validation, create/merge/defer/block planning, fixed error codes, and explicit zero writes. `--confirm` fails with `review_import_confirm_not_implemented`.
- Security: repository-root path confinement, symlink rejection, owned-directory overwrite policy, staged replacement, CSV formula neutralization/rejection, strict duplicate/unknown header handling, bounded UTF-8/CSV/JSON/JSONL parsing, prototype-key rejection, HTTPS/private-host checks, ranking/promotion evidence allowlists, safe CLI errors, and a Node-only service-role read boundary were verified. Dry-run imports no mutation/RPC/audit/confirmation helper.
- Important review fixes: candidate staleness compares authoritative source/raw/canonical/category/form/match/review-flag/promotion state and payload hash rather than relying only on `updated_at`; arbitrary evidence forwarding was replaced with bounded allowlists; source snapshot hash is recalculated during intake; overwrite stages output before replacing owned files.
- Fixtures: five candidates cover approve/create, approve/merge, defer, block, and stale. Negative runtime coverage includes row/batch/hash tamper, duplicate IDs and create identity, invalid enums, missing treatment form, invalid three-state sensitivity, dangerous URLs, product identity conflict, malformed/oversized JSON, formula input, candidate/review/evidence staleness, and confirm rejection.
- Isolated DB: Supabase CLI `2.109.1` replayed the product foundation fixture, admin access migration, all three existing admin product review migrations, and the ADMIN-PRODUCT-2 seed on ports `56320`-`56329`. The existing admin SQL runtime matrix passed. A real service-role export returned five candidates; reviewed dry-run returned 5/5 valid with create 1, merge 1, defer 2, block 1. Before/after counts remained products 2, candidates 5, reviews 5, audits 0, confirmations 0. A local candidate timestamp mutation failed stale detection. The isolated project and containers were removed.
- Validation: crawler `npm ci`, typecheck, export verifier, intake verifier, admin access verifier, admin product review verifier, architecture guard, production build, JavaScript syntax, tracked diff check, and untracked whitespace/final-LF check passed.
- Error log: the first intake runtime pulled unrelated legacy image-policy code through `review.ts`; a lightweight isolated enum contract removed that runtime dependency. An overwrite attempt correctly rejected a directory containing operator-created `reviewed.csv`. A fixed reviewed timestamp predating a live export correctly failed and the local fixture helper switched to current UTC. Docker Desktop was initially stopped, then started for isolated verification and cleaned up. `npm ci` emitted an allow-scripts notice for esbuild, but the installed toolchain passed typecheck and both verifiers.
- Residual: v1 SHA-256 is unkeyed integrity, not batch authorship. Authoritative DB comparison and controlled batch exchange remain required; a signed envelope is optional future hardening. No important code risk remains for the dry-run stage.

### 2026-07-31 / ADMIN-PRODUCT-2 verifier workspace cleanup

- Cause: the Supabase preparation commands in the admin access/product review workflow used repository-local `tmp/...` workdirs. `supabase init` therefore created and later rewrote `supabase/config.toml` inside the checkout. The export and intake verifiers themselves already use OS temporary directories and did not write those files.
- Fix: both workflows now stage migrations under `${{ runner.temp }}`-scoped isolated workdirs, pass that path to every Supabase command, and stop/remove the same validated directory in the always-run cleanup step. Static admin/product verifiers now guard against reintroducing repository-local runtime paths.
- Cleanup: the two reported `tmp/admin-product-review-export-intake-*/supabase/config.toml` paths were absent from this checkout (and are ignored by `tmp/`); no unrelated `tmp` files were removed. No commit, push, hosted DB change, or GitHub Actions run was performed.
- Error log: the first post-cleanup aggregate invoked crawler-only npm scripts from the repository root and reported the expected `Missing script` errors; the commands were rerun from `crawler` and passed. The first static-guard edit called an unavailable helper name in the product verifier; it was corrected to the verifier's existing `includes` helper before the successful rerun.

### 2026-07-31 / ADMIN-PRODUCT-3 reviewed import confirm

- Task type: High-risk bounded execution on `feature/admin-product-review-import-confirm`, based on clean `feature/admin-product-review-export-intake` HEAD `a12d4061830eca43be45f3e23b2d251459bab237`. DB/RPC/RLS/service-role/local isolated runtime were `Y`; hosted Supabase, Production data/secrets, GitHub Actions, commit, push, PR, deployment, administrator UI, crawler ranking policy, automated research, and LLM calls were unchanged.
- Contract: `reviews:import-reviewed --confirm` requires an admin actor UUID and request ID, parses the same protected batch, performs an exact prior-confirmation lookup, requires a current dry-run PASS for every new request, and calls a service-role-only atomic batch RPC. Exact completed retries return the stored ledger result; different payload/request reuse and second confirmation of one export batch fail closed.
- DB: added an RLS-enabled no-direct-access confirmation ledger, service-only exact lookup and confirm RPCs, request/batch advisory transaction locks, sorted candidate/review row locks, candidate/review/evidence optimistic revalidation, create/merge/defer/block handling, existing `promote_product_candidate` reuse, per-row hardened audit, and end-of-transaction ledger insert. Browser roles cannot call either RPC. A failure in any product, candidate, queue, audit, or ledger step rolls the full batch back.
- Important review fixes: completed retries no longer fail on the final state created by their own first transaction; reviewed identity normalization now mirrors the existing promotion SQL and is revalidated in DB; approve RPC validation covers arrays/enums, duplicate intent, verified sources, field evidence/confidence, and contradictions; the compact product fixture now mirrors production normalization; the new migration was sequenced after the existing `20260731150000` hardening migration.
- Isolated runtime: Supabase CLI `2.109.1` used generated OS-temporary projects and copied migrations on dedicated ports `55320..55322`. Final-order replay and reset passed. Five reviewed rows produced create 1, merge 1, defer 2, block 1, product count 2 -> 3, five audit rows, and one ledger row. Viewer denial, stale candidate RPC rejection, exact retry, request conflict, batch conflict, browser RPC denial, ledger direct-write denial, duplicate prevention, and CLI retry passed. A forced audit trigger failed mid-batch and products/candidates/queue/audit/ledger counts all remained unchanged. The existing admin Product Candidate Reviews SQL runtime matrix passed with the corrected fixture.
- Validation: crawler typecheck, export verifier, intake dry-run verifier, confirm verifier, confirm static verifier, admin access verifier, admin product review verifier, existing admin isolated SQL runtime, JavaScript syntax `1062`, architecture guard, production build, ordered migration replay/runtime, rollback test, and `git diff --check` passed. OS-temporary Supabase projects and unique repository batch directories were removed in `finally`.
- Error log: the first generated Supabase project needed an explicit migrations directory; the occupied default local port required only the temporary config copy to use dedicated ports. One TypeScript decision narrowing error was corrected. The first exact CLI retry correctly exposed that final rows make a new dry-run stale, leading to the exact service-only ledger lookup. SQLSTATE `40001` was generalized by the local Data API, so bounded stale checks use `23514` with explicit fixed messages. One static verifier literal was corrected. No failed attempt was reported as PASS.
