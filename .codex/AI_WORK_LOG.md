...
- Branch: codex/functional-plan-section
- Task type: diagnostic / read-only saved-report data quality sampling
- Routing decision: User requested actual saved report sampling for current product functional findings while explicitly forbidding DB/schema/product/saved-report/UI/resolver/findings changes. The work stayed diagnostic and used read-only Supabase selects only.
- Goal: Check whether saved premium reports contain enough `currentProducts` snapshot data for `resolveProductFunctionalProfile()` and `buildCurrentProductFindings()` to evaluate selected products.
- Changed files: scripts/audit-saved-report-functional-data.mjs, .codex/AI_WORK_LOG.md
- Protected areas: No DB migration/schema/policy, products data, saved report data, currentProducts storage structure, UI, resolver logic, findings logic, Premium Functional Plan screen, new table, or Hwahae pipeline change.
- Validation results: `node scripts/audit-saved-report-functional-data.mjs --limit=30` ran against Supabase with read-only `select` queries and wrote ignored local aggregate files under `tmp/audits/`. The connected project currently has 22 `saved_reports` rows, all `report_type='free'`; `report_type='premium'` saved reports with `premium_report` payload are 0. A follow-up read-only structure check found no `currentProducts` payload in the 22 free saved report rows. `premium_report_sessions` has rows, but those are temporary sessions and were not mixed into the saved-report audit sample.
- Notes/risks: Actual saved premium report evaluation could not be performed because the saved-report table has no premium rows in the connected environment. The resulting selected-product and resolver/finding rates are therefore 0/0 and not evidence about resolver quality.
...
### 2026-07-01 / review signal raw fixture dry-run rebuild

- Branch: codex/functional-plan-section
- Task type: diagnostic / guarded execution
- Routing decision: High product-signal pipeline integrity work limited to read-only Supabase SELECT, filesystem scan, placeholder copy quarantine, tmp raw-batch/fixture generation, and import dry-run only. Products data, DB schema, migrations, UI, Functional Plan logic, crawler execution, raw source deletion/move, and existing data raw-batch/fixture overwrite were out of scope.
- Goal: Reconcile Hwahae review-signal JSON by productId, quarantine placeholder-ID files under data/no_ID, generate tmp category raw-batches and import-ready fixtures for DB-matched non-functional products, then verify import impact with `--dry-run`.
- Changed files: scripts/rebuild-review-signal-fixtures-dry-run.mjs, data/no_ID/*, tmp/review-signals-rebuild/*, .codex/AI_WORK_LOG.md
- Protected areas: No DB write, products update, migration, schema change, actual import, crawler run, raw JSON deletion/move, existing `data/hwahae-review-signals` raw-batch/fixture overwrite, UI change, or Functional Plan resolver/findings change.
- Validation: `node scripts/rebuild-review-signal-fixtures-dry-run.mjs` selected 60 DB-matched raw candidates into tmp raw-batches; build fixture commands generated 60 fixture items with non-empty `ingredient_signals.functional`, placeholder 0, duplicate 0, existing functional included 0, legacy/manual included 0. `import-review-signals-to-supabase.mjs --dry-run` ran for all 7 tmp fixture files with would-update 60, updated 0, failed 0. Post-run read-only inventory still showed DB functional products 71. `git diff --check` passed with CRLF warning only.
- Issues/risks: PowerShell logs include Node's existing package module-type warning as native error text even though dry-run exit codes were 0. Existing functional products were preserved; legacy/manual 32 products remain excluded pending merge/overwrite policy. `review-in-supabase.mjs` still needs guard proposals before real import use.
- Import follow-up: After preflight confirmed target 60, placeholder 0, duplicate 0, DB-missing 0, existing-functional overlap 0, and legacy/manual overlap 0, imported only the 7 tmp fixtures from `tmp/review-signals-rebuild` into Supabase products. Category results were cleanser 9, moisturizer_cream 4, moisturizer_lotion_emulsion 4, sunscreen 10, toner_essence 22, toner_pad 4, and treatment 7; all reported skipped 0 and failed 0. Post-import read-only audit confirmed `ingredient_signals.functional` count increased from 71 to 131, baseline functional 71 still functional, and legacy/manual 32 still legacy/manual. Summary files: `tmp/review-signals-rebuild/import-result-summary.json` and `.md`.

### 2026-07-02 / legacy manual review signal normalize dry-run

- Branch: codex/functional-plan-section
- Task type: guarded execution
- Routing decision: High product-signal pipeline work. Proceeded through CSV audit and `review-in-supabase` dry-run only; actual import was blocked because one category failed dry-run. DB schema/migrations, UI, Functional Plan logic, placeholder/no_ID correction, arbitrary product addition, and existing functional overwrite were out of scope.
- Goal: Recollect legacy/manual `ingredient_signals` products from user-provided CSVs and normalize them to the current Hwahae functional format only after dry-run validation.
- Changed files: scripts/review-signals/review-in-supabase.mjs, data/hwahae-review-signals/categories/cleanser/* generated files, data/hwahae-review-signals/categories/moisturizer/lotion/* generated partial dry-run files, tmp/review-signals-legacy-normalize/*, .codex/AI_WORK_LOG.md
- Protected areas: No actual Supabase import was executed, no migration/schema/UI/Functional Plan logic change was made, no `data/no_ID` file was modified, and no placeholder/DB-missing product was imported.
- Validation: CSV audit passed for `data/cleanser_rows.csv` and `data/moisturizer_lotion_emulsion_rows.csv`: 32 rows, 32 unique DB product ids, 32 legacy/manual targets, functional overlap 0, placeholder 0, missing hwahae_url 0. Fixed `review-in-supabase.mjs` so empty product-form is not forwarded as a boolean true to the prepare script. Cleanser dry-run succeeded with 16 extracted raw files, 16 fixture items, functional non-empty 16, import dry-run would-update 16, failed 0. Lotion/emulsion dry-run extracted 8/16 and failed 8 due Hwahae Human Verification pages, so actual import was not run. Post-check inventory still showed functional products 131 and legacy/manual 32.
- Issues/risks: Lotion/emulsion legacy normalization remains blocked by Hwahae Human Verification for 8 products. Since dry-run did not pass globally, cleanser's 16 successful fixture items were not imported in this turn.
- Cleanser import follow-up: Imported only `data/hwahae-review-signals/categories/cleanser/cleanser.review-signals.batch.json` after preflight confirmed 16 targets, placeholder 0, duplicate 0, DB-missing 0, existing-functional overlap 0, and all targets still legacy/manual cleanser products. Actual import reported updated 16, skipped 0, failed 0. Post-import read-only audit confirmed functional products 131 -> 147, baseline functional 131 preserved, and remaining legacy/manual 16 are all `moisturizer_lotion_emulsion`.
- Lotion/emulsion import follow-up: Re-ran `review-in-supabase` for `data/moisturizer_lotion_emulsion_rows.csv`; extraction succeeded for all 16 after the previous Human Verification block, fixture generated 16 functional items, and import dry-run reported would-update 16, failed 0. Actual import of `data/hwahae-review-signals/categories/moisturizer/lotion/moisturizer_lotion_emulsion.review-signals.batch.json` reported updated 16, skipped 0, failed 0. Post-import read-only audit confirmed functional products 147 -> 163, baseline functional 147 preserved, and legacy/manual count 0.
- Single toner import follow-up: After the user corrected `productId` in `data/hwahae-review-signals/categories/toner/toner_essence/c8a6c9e2-9068-4b7a-976b-97bb3438f2e1 세라마이드 쌀 그리미 밀크 토너.json`, built a one-item tmp fixture, verified DB product exists with no placeholder and raw functional evidence, dry-ran import with failed 0, then imported only that product. Actual import reported updated 1, skipped 0, failed 0. Post-import read-only audit confirmed functional products 164, legacy/manual 0, placeholder locations 0, and the toner has 5 functional entries.
- Raw organization follow-up: Moved the remaining category-level product raw JSON files into sibling `raw/` folders without moving `extract-plan`, `raw-batch`, fixture, or sample JSON files. Moved 19 files total: 6 moisturizer cream and 13 toner essence product raw files. Verification found 0 category product raw JSON files outside `raw/`; follow-up inventory showed DB products 164, raw-folder productIds 164, functional products 164, legacy/manual 0, placeholder locations 0. Remaining raw-outside productIds are from raw-batch/fixture outputs, not product raw files.
- Raw dedup follow-up: Removed active category raw duplicates by keeping the canonical `categories/treatment/.../raw` files and moving 8 duplicate `categories/serum/raw` JSON files to `tmp/review-signals-dedup-archive`. Verification found category product raw count 164, duplicate productIds 0, and product raw outside `raw/` 0. Sample JSON files were not changed because they are README/build-script usage fixtures, not active category product raw.
- Serum folder cleanup follow-up: Confirmed `categories/serum` duplicated the treatment serum path: 8 raw files overlapped with `categories/treatment/serum/raw`, and the serum root batch/notes files had identical hashes to `categories/treatment` root files. Removed active `data/hwahae-review-signals/categories/serum` after archiving its remaining root files under `tmp/review-signals-dedup-archive`; verification found `categories/serum` absent, category product raw count 164, duplicate productIds 0, and product raw outside `raw/` 0.
