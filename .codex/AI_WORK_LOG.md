...
### 2026-07-30 / Hosted Preview stack current-main integration

- Task type: High-risk bounded integration. Auth and session ownership were `Y`; DB schema, RLS, Storage, Provider, Payment, Secret values, Production execution, environment files, and deployment configuration were unchanged.
- Integration: rebased the retained #38 Hosted Preview verification gate and #44/#51 hardening implementation onto current `origin/main` through a merge branch. Current-main route, sanitizer, logging, persistence, and release-mode contracts remained authoritative; obsolete stacked implementations were not allowed to overwrite them.
- Remaining security delta: Premium report session tokens now carry an HMAC owner binding, all create/read/update/rotate call sites provide the verified account ID, legacy unbound and cross-account tokens fail closed, and successful sign-out expires the path-scoped Premium report cookie. The existing owner-filtered saved-report query and beta-era saved-report availability contract were preserved.
- Harness: added the hardened same-origin Hosted Preview attestation, credential isolation, UI/error/DB evidence, cleanup, fail-closed gate, examples, and runbooks. Live Preview verifiers remain externally gated because they require an exact deployment, dedicated accounts, and secure fixture paths.
- Local verification: `npm ci`, changed-file JavaScript syntax, Hosted Preview offline contracts, Premium owner/session/reentry/storage contracts, SEC-06, SEC-10 `60/60`, SEC-11 `40/40`, SEC-12 `62/62`, analysis RLS, architecture guard, production build, and diff hygiene passed. The aggregate security suite's unrelated shadow-plan preparation cannot run against a dirty feature worktree because it intentionally rejects any modified `app/api/analyze/route.js`; focused affected verifiers were executed directly instead.

### 2026-07-31 / Stage 10 Hosted Preview Windows attestation CLI repair

- Task type: High-risk Hosted Preview verification enablement. Authentication evidence and Preview execution are `Y`; DB schema/RLS, entitlement, OAuth configuration, environment files, secrets, Production deployment, and deployment configuration are unchanged.
- Confirmed failure: the first live attestation attempt reached the Vercel lookup and failed with `vercel_deployment_lookup_failed:cli_failed:unavailable`. A direct reproduction exposed the hidden Windows exception as `spawn EINVAL`: Node 24 cannot execute the npm `vercel.cmd` shim through `execFile()` as if it were a native executable.
- Minimal repair: on Windows only, the authoritative API client now invokes the existing authenticated Vercel CLI through `ComSpec /d /s /c vercel.cmd`; `gh.exe`, token-backed HTTP modes, Linux/macOS behavior, endpoint validation, and secret handling are unchanged. The unit contract was updated to freeze the Windows invocation shape.
- Verification: the focused authoritative-API verifier passed, followed by a real attestation against Draft PR #96 and READY Preview deployment `dpl_GVMgn1biPdihW8ambgP95t3rVN71`. GitHub SHA, Vercel SHA/ref/PR metadata, immutable host, and null-target PR-bound Preview evidence all matched exact head `e8019aa28f8f6076b6bb689a7154a0fca56145a5`.
- A first Account-A login capture failed closed with `AUTH_BOUNDARY_FAILURE/local-auth-A/test_account_must_be_permanent`. Sanitized dedicated-profile history proved that Google OAuth completed but Supabase returned to the configured Production site URL before `/auth/callback` and `/my`; the immutable Preview retained its pre-login anonymous cookie. The existing canonical OAuth bridge ran only when no Preview auth cookie existed, so that anonymous cookie prevented the freshly authenticated permanent session from being copied to the Preview host.
- The bounded harness fix now prefers the freshly captured canonical OAuth-host cookies before checking the Preview cookie set, then performs the existing normalization and permanent-Google-user validation. This changes only local test-profile cookie handling; OAuth configuration, Production cookies, application authentication code, and server authorization are unchanged. The local browser-journey contract passes with the bridge ordering frozen.
- Account B exposed a second bridge defect after the ordering repair. Safe in-memory session inspection showed a two-chunk permanent Google session on the canonical host and a three-chunk anonymous session on the Preview host. The bridge cleared only target cookie names also present in the source, leaving the third anonymous chunk behind; the mixed cookie set decoded as anonymous and again failed closed. The bridge now removes every target-host Supabase auth-cookie chunk before adding the canonical set. No cookie value, token, email, or raw identifier was logged or persisted in repository evidence.
- The first complete KO/EN journey then reached analyze and Premium session creation but failed at `ko:first-save` with `premium_principal_conflict` and zero created rows. Safe storage-state inspection proved the saved Preview cookie was anonymous even though the separately validated bearer token was Account A. The capture page remained active during bridging; its in-memory anonymous Supabase client rewrote the Preview cookie after the bridge and before `storageState()` persisted it. The harness now discovers public configuration and the canonical host first, closes the active application page, and only then replaces/normalizes the Preview cookies. This removes the client-side rewrite race without weakening the server principal-conflict boundary.
- After that repair, both persisted profiles were revalidated noninteractively as distinct permanent Google accounts on the immutable Preview host. A full KO/EN hosted run on `d257074e102fbc25632ccb290b194efd26906db9` passed once with four reports created and all four deleted. A later exploratory browser upload consumed the analyze quota; the next authoritative rerun failed at `ko:analyze` with HTTP 429 `analysis_rate_limited`, created zero rows, and required no cleanup. This is recorded as an external rate-limit gate rather than a product pass or an authentication regression.
- The operator screenshots showing `Google로 로그인` after OAuth were not a presentation-only defect. Sanitized profile history at the screenshot timestamps showed both exact Preview login attempts were returned by Supabase to `k-beauty-two.vercel.app`, not the initiating immutable Preview host. On the currently deployed Production SHA `a30970b78ff2fb3f5784d947b746223a66954e44`, a permanent Google cookie still triggered `/auth/v1/signup` before `/auth/v1/user`, and the header rendered the login CTA. That build uses a separate localStorage-backed anonymous client in `lib/supabase/browser-client.js` while `AuthNav` uses the cookie-backed SSR client. The #96/#97 Preview has already unified those paths on the cookie client: the same permanent session produced no anonymous signup, rendered `로그아웃` plus `My`, and returned `/auth/v1/user` 200. The remaining immutable-Preview OAuth return-host mismatch is external Auth redirect configuration and is handled only by the local verification bridge; Production settings were not changed.

### 2026-07-31 / Stage 10B Preview OAuth and run-scoped cleanup closure

- Task type: bounded High-risk execution on Draft PR #97. Preview OAuth configuration, permanent test-account sessions, exact Preview execution, and exact-ID cleanup were `Y`; Production deployment, Production Site URL, Google provider callback, DB schema/RLS, global limiter policy, CandidatePolicy, main, merge, and GitHub Actions were unchanged.
- Confirmed external root cause: Supabase Auth retained the Production Site URL and three redirects but had no K Beauty Preview redirect. The project-scoped `https://k-beauty-*-johnny-self.vercel.app/**` entry was appended without replacing existing entries. The caller now accepts only the canonical Production origin, the project-scoped HTTPS Preview hostname, or localhost; an untrusted host safely falls back and an unsafe `next` path is rejected.
- Direct OAuth evidence: both permanent Google test accounts completed fresh OAuth round trips on exact Preview `dpl_HfxF3QksAn67iScd5598RtcVE3b1` / `67521b3def0ce617dd5cde4d806eac9f328b40d8`. Both safe session sources were `target_host`, not the canonical bridge. The first retry attempt timed out while waiting for the operator to close the Account-A system Chrome; the timeout path was fixed to terminate its child Chrome process. The second retry succeeded for A and B.
- Rate-limit classification: the earlier 429 followed repeated verification against the 5/hour authenticated and IP windows. No limiter state or threshold was modified. The single authoritative rerun sent exactly two analyze requests, one KO and one EN, and completed without 429.
- Run-scoped cleanup: the harness tracked four exact Premium session IDs in ignored local runtime state, published hashes/counts only, deleted all four sessions and four saved reports by exact ID, re-queried each exact ID, and confirmed both residue counts were zero. Permanent Auth users and profiles were intentionally retained.
- Authoritative run `premium-e2e-20260731030813-27924c25` passed 29/29 KO/EN browser/API steps, retry/reentry, 409 finalized conflict, principal and cross-account 401 boundaries, rotation, independent second saves, invariant checks, and duplicate-source-tuple zero. Actual Preview sign-out returned 303 and transitioned the header to signed out; SEC-11 continues to freeze Premium cookie expiry.
- Verification: OAuth origin `13` assertions, exact cleanup `15` assertions, Hosted hardening, route/storage/reentry, report reentry, integrated evaluation, browser journey contract/local checks, architecture guard, production build, security closeout `57/57`, and diff hygiene passed. `npm ci` reported four pre-existing high audit findings; dependencies were not changed.
- Error log: a direct `node --check` on a JSX file was invalid for Node's extension loader; the production build validated the JSX. The first aggregate security rerun rejected the newly added verifier as manifest drift; the expected verifier manifest was updated and the complete suite passed. One early PowerShell HTTP command had an empty pipeline, and `Invoke-WebRequest` later misreported successful Preview endpoints; `curl` and Vercel deployment metadata confirmed HTTP 200. A one-off sign-out probe initially targeted `/my`, where the sign-out form is inside the menu rather than the top-level page; the authoritative home-header POST then returned 303 and switched to the signed-out UI. No product PASS was inferred from the failed probe.

### 2026-07-31 / Stage 11A CandidatePolicy runtime responsibility re-evaluation

- Task type: read-only design/runtime audit on `codex/candidate-policy-runtime-reevaluation`, based on Stage 10 branch head `7a32d497744352f43468da144be889893e8e5cba`. CandidatePolicy runtime, flags, recommendation arrays, API/UI/storage contracts, DB/Auth, Production, and GitHub Actions were unchanged.
- Actual call graph: the default-off evaluator runtime is a real production-capable caller in `skin-match-decision-engine`; when enabled it filters `scoredProducts` through `visibleCandidateIds` before Top Pick, alternatives, supporting products, routine, and budget assembly. Canonical `SharedSkinDecisionContext`, effective FunctionalPolicy, and CrossDomainConsistency are built later by `rebuildPremiumDecisionState`, so the current runtime reconstructs a pre-canonical goal instead of consuming the final authority.
- Duplicate responsibility: `functional-candidate-policy` reads current findings and returns group visibility/intent/caps, but has zero production importers and is verifier-only. Evaluator runtime/shadow APIs accept current findings, while both actual engine callers omit them. Wiring the two existing policies directly would create a double-filter and still lose collapsed/hidden/insufficient semantics.
- Decision: option C, a single versioned canonical `CandidateExposurePolicy`, reusing evaluator safety/shadow assets while moving current-product semantics into one per-candidate exposure and lane-eligibility contract. A preserves duplicate/dead policy, B promotes a verifier-only presentation policy without per-candidate safety, and D discards verified evaluator assets. The next implementation stage remains disabled-by-default, response-invariant shadow integration; runtime activation is not authorized.
- Actual catalog evidence: a read-only pure-engine replay loaded 164/164 scorer-compatible rows, ran 4 scenarios and 656 candidate rows, and produced SHA-256 `8f746fbfb277d88d8d59a6d572cdef7514b04b1d6efe08a4b5027ca0bd0a6f9a`. Safe-low-risk hidden and serum category were observed; active-only, metadata-incomplete, and strong-caution were not observed and remain synthetic-only evidence. No DB write or export mutation occurred.
- Environment audit: authenticated read-only Vercel project metadata contained zero CandidatePolicy environment keys in Preview or Production. No value was printed and no configuration was changed; actual hosted state is `runtime_disabled`.
- Verification: the deterministic reevaluation verifier passed 62 assertions; 20 focused CandidatePolicy verifiers passed after supplying the required run-scoped synthetic canary evidence to the kill-switch propagation verifier; the security closeout suite passed `58/58`; architecture guard, production build, and diff hygiene passed. `npm ci` retained the existing four high audit findings without dependency changes.
- Error log: the first direct kill-switch propagation invocation omitted its required `--run-id` evidence argument and failed `synthetic_canary_run_id_required`; a local run-scoped synthetic canary artifact was generated and the verifier then passed. Adding the new `verify-*` file caused the security closeout exact manifest to reject the suite before execution; the new deterministic verifier was added to the frozen manifest and the full suite passed. An earlier PowerShell Vercel-project check had an empty pipeline and `.vercel/project.json` was absent; the authenticated read-only Vercel API was used instead. The initial local replay loaded zero rows because this worktree intentionally lacked `.env.local`; it was rerun with only public Supabase variables injected into the process from the existing sibling checkout, without copying or printing values, and completed 164 rows.

### 2026-07-31 / Stage 11B CandidateExposurePolicy shadow runtime integration

- Task type: Medium-risk bounded execution with High-risk Preview verification. The canonical policy, disabled shadow call, aggregate telemetry, verifier, architecture/result documents, and work log are in scope. DB/Auth/RLS/Storage schema, API response fields, saved-data shape, UI, Production configuration, Production deployment, and runtime filtering remain unchanged.
- Integration: added `candidate-exposure-policy-v1` with exactly five exposure states and per-lane eligibility. The evaluator safety boundary is reused through a canonical adapter after `rebuildPremiumDecisionState`; raw survey goal reconstruction and the verifier-only functional prototype are not connected. The route ignores shadow output, and local checks freeze response, snapshot, and candidate order.
- Control and observability: default-off Preview/development opt-in, explicit kill switch, unconditional Production hard-disable, invalid-environment disable, bounded exception fallback, fixed divergence categories, and identifier-free aggregate telemetry are verified. No Vercel project environment key was added; one exact-SHA immutable Preview received only a deployment-scoped opt-in override.
- Evidence: focused verification passed 193 assertions, 12 current-product fixtures, and 13 safety fixtures. Actual catalog replay covered 164/164 compatible rows, 4/4 scenarios, and 656 candidate rows. `safeLowRiskHidden` and `serumCategory` were observed; active-only, metadata-incomplete, and strong-caution remain synthetic-only. CandidatePolicy families, security closeout `59/59`, architecture guard, production build, and diff hygiene passed before documentation.
- Preview: exact implementation SHA `054a9c91c853c51e1285ba12901c51bbe60c00a6` produced READY default-off and deployment-scoped shadow-on Preview deployments. Initial KO/EN calls accidentally serialized the fixture wrapper instead of `formFields`, returning HTTP 400. The corrected calls also returned 400 and the final EN shadow call reached the shared 5/hour limiter with HTTP 429. None reached the post-canonical shadow point, so hosted divergence/fingerprint/exception evidence is not claimed and the final integration status is external-blocked.
- Error log: the first `npm ci` hit Windows `EPERM` because this workspace's active Next dev process held SWC; only that exact process tree was stopped and `npm ci` then passed. A guessed nonexistent evaluator verifier path was corrected by repository file discovery. The direct current-product verifier needed the repository's existing Next alias loader. The hint-receiver verifier needed its documented integration-design prerequisite artifact. The full suite exposed a stale readiness expectation for a populated pure replay without complete-product capture; the verifier was aligned to `needs_more_evidence_before_design`, then all 59 checks passed. `vercel link` added `.vercel` to `.gitignore`; that unintended tracked edit was reverted and local metadata was excluded only in `.git/info/exclude`. The first manual deployment was therefore discarded; the replacement was built from a clean exact SHA. Two probe harness attempts failed before HTTP because PowerShell/Vercel curl argument forwarding and Windows `vercel.cmd` spawning were invalid. The later payload construction and rate-limit failures above were recorded without persisting response bodies or identifiers.
- Residual risk: exact Preview successful KO/EN shadow execution must be rerun after the analyze quota window resets. Until response/snapshot/order matches, unexpected divergence 0, and shadow exception 0 are observed in hosted telemetry, Stage 11C eligibility must not be proposed.
