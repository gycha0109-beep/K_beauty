...
### 2026-07-28 / CandidatePolicy verifier prerequisite baseline recovery

- Task type: recovery followed by bounded verifier-only execution. CandidatePolicy, FunctionalPolicy, GoalPolicy, evaluator runtime, API, DB/Supabase, environment, product data, deployment, and production behavior are unchanged.
- Baseline `b17d322cf0b04311407be75fdcda88b1a973800a` reproduced both hidden dependencies in a clean worktree: the shadow integration verifier lacked `candidate-exposure-audit.json`, and the hint receiver verifier lacked the integration what-if JSON.
- The recovery adds a deterministic, anonymous contract fixture and reuses the existing producer chain with explicit repository-relative input/output paths and a fixed clock. Generated evidence is schema/provenance/non-empty/exact-set validated and is explicitly labeled fixture evidence, never actual or production evidence.
- Each standalone verifier creates and cleans isolated OS temp directories, overwrites stale invalid same-name artifacts, rejects missing prerequisites and malformed or decision-tampered output, and compares two independent semantic artifact hashes. The security closeout suite now materializes the same prerequisites explicitly and freezes both recovered verifiers in its exact manifest (`51` to `52` entries); no prior entry was removed.
- The first post-change negative-control run failed because its `assert.throws` message regex expected the field name while Node emitted only the strict-equality value diff. The matcher was corrected to require the exact tampered `actual` and canonical `expected` values; verifier behavior and production contracts were unchanged.
- A focused actual-coverage verifier run then exposed that the anonymous contract fixture intentionally exercises exposure audit rows but does not satisfy the later boundary-applicable predicate. The fixture-only branch now requires non-empty candidates and count consistency without weakening the existing actual-evidence requirement that boundary-applicable rows be non-zero; exposure aggregate exactness is enforced at the producer artifact boundary.
- The existing SEC-06 negative-control failure (`negative control 5 was not rejected`) reproduces unchanged on detached baseline `origin/main`; it remains an unrelated closeout failure and was neither removed nor weakened.
- UVA fail-open, stabilization active expansion, FunctionalPolicy/GoalPolicy divergence, missing `currentProductFindings`, and current-product snapshot transport loss remain explicit out-of-scope findings for CandidatePolicy Runtime Safety Hardening.
- Local validation passed both standalone verifiers, the five-verifier CandidatePolicy bundle, actual-coverage verifier, architecture guard, syntax checks, deterministic double generation, production build, and diff hygiene. Integration assertion call sites increased from `45` to `51`; hint-receiver call sites increased from `50` to `60`. Both semantic-hash sets matched exactly. The initial security run passed `50/51`, but review found the integration verifier was only a preparation step because its filename missed the legacy discovery keywords. It is now an explicit exact-manifest entry; both recovery targets must pass in verification, with only the independently baseline-reproduced SEC-06 failure permitted to remain unrelated.

### 2026-07-29 / CandidatePolicy runtime safety hardening

- Exact base `f0505b804313ee5ffbe1fbb84971487f5c81c5f1` retained the recovered standalone verifier baseline. A new read-only 164-row replay reproduced the two scoped P0 defects: the UVA-missing sunscreen was visible `1/1`, and canonical stabilization left 86 active-axis candidates eligible for all downstream pools.
- Root causes were independent: the recommendation mapper dropped source SPF/UVA evidence before scoring, while evaluator runtime/shadow consumed GoalPolicy without canonical SharedSkinDecisionContext/FunctionalPolicy safety state.
- Added immutable `candidate-policy-runtime-safety-context-v1`, built once on the runtime/shadow caller path from canonical context and FunctionalPolicy. Missing/invalid context fails closed only when the runtime/shadow function is executed; the default-off runtime path, kill switch, canary scope, current-product findings, snapshot mapper, and non-safety GoalPolicy ranking remain unchanged.
- The shared runtime/shadow gate routes incomplete sunscreen protection evidence to existing insufficient-evidence exposure and canonical-stabilization active axes to existing hidden exposure. SPF, UVA label, and normalized UV filter type are protection evidence; pilling remains preference-only. Source SPF/UVA are preserved without inference or defaulting.
- Post-change actual replay: the UVA-missing product became visible `0/1`, insufficient `1/1`, and zero in Top Pick/alternative/supporting/routine/budget eligibility. All 86 active-axis candidates became active-visible `0` under stabilization. Protection-complete and protection-complete/pilling-missing sunscreens remained visible; stable-pool unexpected general-product blocks were zero.
- Focused verification initially stopped only on its own assertion-floor bookkeeping after every semantic scenario had passed; explicit scenario assertions replaced the incorrect floor assumption. The CandidatePolicy aggregate was also observed to fail while the engine diff was unstaged because a legacy shadow verifier intentionally rejects dirty protected files; the authoritative run is performed with the exact candidate diff staged/committed, without weakening that guard.
- The focused verifier has 33 assertions, 12 scenarios, 8 negative controls, and matching deterministic semantic hashes. Security exact-manifest membership increases from 52 to 53; the pre-existing SEC-06 `negative control 5 was not rejected` failure remains unrelated and unmodified.
- No DB/Supabase, network, Preview, Hosted, Production, provider, environment-variable mutation, product-data write, migration, backfill, deployment, runtime activation, or canary occurred. Remaining findings are FunctionalPolicy/GoalPolicy divergence, missing `currentProductFindings`, current-product snapshot loss, and source UVA/pilling remediation.

### 2026-07-29 / Current-product snapshot transport repair

- Task type: bounded High-risk data/persistence execution after diagnosis. Storage JSON compatibility is `Y` and explicitly authorized; DB, RLS, Auth, Provider, Payment, Secret, and Production execution are `N`. No database, Supabase, network, hosted target, environment, migration, backfill, or product-data write occurred.
- Exact stacked base `3dcd1f847a2a6ab309681a41eeb96cdf19a7d2f6` reproduced the audited loss with the actual preserved 164-row export: `41/164` affected products, `104` source-present field/product drops, `123/164` current snapshots complete, and `74` sunscreen dropped instances.
- Root cause: both current-product snapshot mappers omitted `spf_value`, `uva_label`, `uv_filter_type`, `tone_up`, `white_cast`, `eye_sting`, and `pilling_risk`; downstream Premium canonicalization, session/saved-report JSON, and response readers otherwise retained additive fields.
- Added one shared, versioned protection-metadata projection used by both mappers. Valid source values survive without coercion; false stays false; explicit null, absent, and invalid source values remain canonical unavailable null. Legacy snapshots keep missing fields absent and parse without inference. `premium-report-snapshot-v1` remains valid because its canonical object contract is additive.
- The audit's field requirements and dropped/defaulted/coerced semantics were not relaxed. Its destination capability declaration now consumes the production snapshot field contract. The unchanged dataset hash remains `f346d90ed722432dd1e1367a50939954ec5030abb9a7ea72fdef61bb1dc93e2f`.
- Post-change actual replay and audit report `164/164` current snapshot transport complete, zero transport drops/defaults/coercions/inaccessible fields, zero source mutations, and zero sunscreen transport drops. Source UVA remains `10/11`; source pilling risk remains `9/11`.
- The focused verifier passes 16 scenarios, 608 assertions, 12 transport negative controls, matching deterministic hashes, exact temp artifact sets, and cleanup. CandidatePolicy runtime safety, recovered shadow/hint, CandidatePolicy bundle, Premium decision/reentry/session, Product Data audit verifier, and current-product verifiers pass.
- Security closeout freezes 54 verifiers and passes `53/54`; only the unchanged baseline SEC-06 `negative control 5 was not rejected` fails. FunctionalPolicy/GoalPolicy divergence, missing `currentProductFindings` caller, source metadata remediation, and Production runtime/canary remain out of scope.

### 2026-07-29 / CandidatePolicy canonical goal-policy alignment

- Task type: bounded High-risk policy execution on exact stacked base `9e21400edbb4ed09467daa18b46fb61b2c3cd6a6` (PR #80). DB/Supabase, network, Hosted/Preview/Production requests, environment mutation, runtime activation, current-product caller wiring, snapshot transport, migration, backfill, and product-data writes are out of scope and did not occur.
- Pre-change deterministic 128-combination replay reproduced the prior relationship: 11 equivalent, 9 compatible-independent, and 108 divergent comparisons. Runtime-effect classification was 16 naming/projection-only rows, 86 ranking divergences, and 26 guard divergences; the requested-first legacy goal helper was the runtime ranking/guard source even when canonical FunctionalPolicy detected another priority.
- Root cause: the runtime and shadow evaluator consumed a compatibility helper that intentionally preserved the requested survey goal, while Premium decision state already held the canonical effective FunctionalPolicy. This made explanation intent and ranking authority indistinguishable.
- Added a versioned, immutable CandidatePolicy goal context. It preserves requested goal and tension for explanation, makes the effective canonical FunctionalPolicy priority authoritative for ranking, keeps PR #79 canonical safety context authoritative for protection/stabilization guards, and fails closed when context/version/provenance is missing or invalid. Runtime and shadow consume the same cached contexts.
- The focused verifier exercises 18 scenarios, 403 assertions, 12 negative controls, isolated exact artifact sets, deterministic double generation, and cleanup. Its matching semantic hashes are `59f4ecb480d18ea410f4bdf0c7d501e63b3a971e688946a7b1f439cf47eb84a9`.
- Post-change actual-export replay used the unchanged 164-row input SHA-256 `2b16bd7c66aa719367cb9a5cd422a40d57ccf2296b780e931892b4d5325aeed6`. All 112 tension combinations retain requested-goal explanation but use canonical ranking; ranking and guard divergence are zero, runtime/shadow divergence is zero, expected canonical-alignment scenario drift is `4`, and unexpected drift is `0`.
- PR #79 safety behavior remains: the UVA-missing sunscreen is visible `0/1` and insufficient `1/1`; all 86 active-axis candidates are active-visible `0` under stabilization; protection-complete and pilling-only-missing sunscreen controls remain visible. PR #80 snapshot transport remains unchanged and `currentProductFindings` is still not passed by production/shadow callers.
- Diagnostic execution initially wrote to a literal `%20` path because an ephemeral file URL was converted incorrectly; the diagnostic runner was corrected to use `fileURLToPath`, and the isolated erroneous output is removed during closeout. The actual replay also requires its local alias loader: direct Node execution failed closed on the repository `@/lib` alias, then the explicit deterministic loader invocation succeeded. One validation batch referenced the nonexistent `verify-current-product-snapshot-transport-repair.mjs`; discovery identified the authoritative `npm run verify:current-product-snapshot-transport` command, which passes unchanged. The CandidatePolicy aggregate also fails intentionally while a protected engine file is dirty; its authoritative run is therefore performed only against the exact committed clean head, without weakening the legacy dirty-tree guard.
- Security exact-manifest membership increases from 54 to 55. The pre-existing SEC-06 `negative control 5 was not rejected` remains the only unrelated closeout failure and is neither normalized nor removed. Remaining findings are missing `currentProductFindings` wiring, source UVA/pilling remediation, and Production runtime/canary verification.
