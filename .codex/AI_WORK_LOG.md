...
- Root cause: the OAuth flow used the cookie-backed `@supabase/ssr` browser/client callback path, while `getBrowserSupabaseAccessToken()` read a separate localStorage-backed `supabase-js` client. Immediately after login, the latter could return an anonymous token and shadow the authenticated cookie session. Deployment aliases also isolated the host-only premium cookie and `sessionStorage` from the canonical origin when an origin transition occurred.
- Minimal fix: production document navigations now redirect deployment aliases to the validated HTTPS `NEXT_PUBLIC_SITE_URL` origin before analysis/auth state is created. The access-token helper prefers the cookie-backed OAuth session before the anonymous fallback. Premium cookie attributes remain HttpOnly, host-only, `SameSite=Lax`, production-Secure, and path-scoped to `/api/full-report`.
- 401 contract/UI: `/api/full-report` now returns only `login_required` or `premium_session_missing_or_expired` for its 401 paths. The full-report UI provides direct login recovery for the former and free-result re-preparation guidance for the latter instead of the generic result-empty state.
- Verification: focused origin unit tests, the full-report auth/origin verifier, premium-release verifier, anonymous-write-grant verifier, architecture guard, two final `npm run build` executions, a fresh Playwright browser-context alias redirect plus local `login_required` response check, same-origin `skinTestResult` sessionStorage preservation, response/header token-marker rejection, and `git diff --check` passed.
- Verification corrections: the first local server readiness harness used an unsuitable PowerShell HTTP client loop; subsequent checks used the listening socket and Playwright request context. A missing-code callback test redirected through the intentional home cleanup path and was replaced with the applicable same-origin result-to-full-report navigation. The first final runtime launch omitted the test-only canonical URL and then inherited the local closed premium mode; it was restarted with isolated runtime overrides for canonical origin and `beta_open`. Application code was not changed to accommodate these harness issues.
...
### 2026-07-15 / Full-report re-entry UX investigation blocked by missing analysis linkage

- Task type: High-risk diagnosis. Auth, DB, RLS, and Production contract impact are `Y`; no hosted target, database, migration, policy, environment, or authentication behavior was changed.
- Confirmed storage: `saved_reports` persists premium rows by owner and `source_session_id`, while `premium_report_sessions` persists an expiring session payload. Neither storage contract has `analysis_run_id`, report-completion state, or an equivalent durable current-analysis key.
- Confirmed runtime: `/api/analyze` emits `analysisRunId` only for the anonymous write-grant path. Logged-in results do not receive a durable analysis-run identifier, and the premium session/saved report payload does not persist one. `saved_reports` lookup for a premium row is currently by `id + user_id`, not by analysis identity.
- Stop condition: querying the latest user premium row, matching an inferred free-result payload, or reusing the session ID would violate the required same-`analysisRunId` rule and could connect a different report. Adding the missing durable link/status requires an approved DB/data-contract decision; no workaround was implemented.
- Minimal decision needed: approve a durable owner-scoped `analysis_run_id` and completion marker on the existing premium report path (or identify an existing authoritative equivalent), including how legacy completed reports should be handled. Existing `saved_reports` rows are updated per `source_session_id`, so explicit regeneration cannot preserve a prior version without a corresponding approved persistence change.

### 2026-07-15 / Full-report current-session re-entry UX

- Follow-up decision: the prior analysis-linkage blocker is resolved for this bounded UX by using the already server-issued, signed premium `sessionId` as the authoritative current-generation identity. No `analysis_run_id`, migration, schema, policy, or production operation was added.
- Current-session lookup: `/api/full-report/session` verifies the account user, the HttpOnly premium cookie, and the server-restored session before querying one owner-scoped `saved_reports` row by `report_type`, `source_type`, `source_session_id`, and non-null `premium_report`. Failures return only `hasSavedReport: false`.
...
### 2026-07-15 / Shared premium decision context PR verification and stale-state repair

- Task type: Medium-risk review followed by bounded execution. API runtime impact was `Y`; DB, Auth, RLS, Storage, Provider, Payment, Secret, Production execution, response field names, saved-data schema, migration, policy, and deployment were `N`.
- Review found two stale-state paths: persisted bundle survey answers took precedence over newly supplied free-result answers, and an explicit empty current-product submission skipped the decision-state rebuild because the verdict builder was called only for a truthy snapshot.
...
- Minimal fix: preserve the deterministic engine's `decision.premiumReport` immediately after construction. If that optional field is absent at the session boundary, construct the session source solely from the server-generated decision and free-result objects; no client-provided Premium payload is accepted. The public response shape and session-cookie contract are unchanged.
- Local verification: direct session-payload fallback regression, premium decision-state, premium release-mode, browser-journey contract/local checks, architecture guard, and production build passed. Hosted cookie and full KO/EN journey verification remain pending the new Preview deployment.

### 2026-07-27 / Premium session runtime boundary attestation (Phase A)

- Task type: bounded High-risk diagnostic execution. Preview runtime observability and authentication-adjacent session creation are `Y`; no OAuth/Supabase configuration, schema, RLS, entitlement, environment file, provider setting, or production target changed.
- Added an opt-in, Preview-only S0-S9 Premium session pipeline diagnostic. A per-run UUID correlation header activates it, and logs retain only fixed stage names, structural booleans/counts, a bounded size estimate, a fixed validation reason, and the Vercel Git SHA. Report values, images, tokens, cookies, user identifiers, and provider output are never serialized.
- The cookie diagnostic now sends the correlation ID and verifies response attestation headers for diagnostic version, correlation match, deployed commit, and final reached stage. Production and malformed/missing-correlation requests emit neither diagnostic headers nor stage logs.
- Added an executable verifier for Preview gating, identifier validation, bounded/cycle-safe measurement, secret-value non-disclosure, S0-S9 log allowlisting, exact payload-validation reasons, and response attestation. The hosted diagnostic remains pending the Git-generated Preview for this commit.
- Local verification passed the new diagnostic verifier, browser local/contract checks, premium payload/decision/reentry/storage/RLS checks, SEC-10 image-origin checks, architecture guard, JavaScript syntax, production build, and diff check. The existing SEC-12 verifier has a baseline exact-set drift in `app/api/full-report/route.js` (`15` terminal paths discovered vs frozen `12`); none of the Phase A changes touch that route or its response paths.

### 2026-07-27 / Premium image sanitizer runtime-capacity repair (Phase B)

- Authoritative Preview evidence at commit `80257766a8379803d62c94a7054a99f08e7c20e8` attested every S0-S9 log with the same correlation ID and runtime SHA. S5 was a record with 21 top-level keys and more than 2,000 measured entries; S6 became null; session validation then failed with exact reason `premium_report_missing`. The response still returned HTTP 200 with zero Premium Set-Cookie headers and diagnosis `server_did_not_emit_premium_cookie`.
...
### 2026-07-28 / Premium identical-retry blocker classification and bounded Preview diagnostic

- Task type: High-risk diagnosis followed by bounded Preview-only diagnostic execution. Auth, Storage, Secret, and Preview runtime are `Y`; DB schema/RLS, Provider, Payment, Production, environment files, and deployment configuration are unchanged.
- Existing Hosted run `premium-e2e-20260727013951-e3a1e46e` classifies as case A: `ko:first-save` returned HTTP 200 with persistence `saved`, while `ko:identical-retry` returned HTTP 409 with `premium_snapshot_finalized`. Cleanup completed with one created and one deleted report.
- The redacted artifact does not contain the candidate snapshot or first differing path. Added a cycle/depth/entry/path-bounded snapshot differential helper and an opt-in Preview-only replay diagnostic using the existing validated correlation ID. It emits only fingerprints, context hashes/revisions, bounded JSON paths/types/counts/hashes, input-state enums, locale, Face Lab persistence decision, and source stage; report values and identifiers are not serialized. Production and invalid/missing-correlation requests emit no replay diagnostic.
- Added Account-A-only `diagnose:premium:identical-retry` flow: analyze, cookie assertion, first save, identical retry, immutable-row/duplicate-tuple checks, redacted evidence, mandatory cleanup, and cleanup verification. It excludes Account B, EN, rotation, and changed-request conflict.
- Focused snapshot-diff, runtime diagnostic, Premium route/storage/reentry, browser contract/local checks, production build, and diff hygiene passed. The known SEC-12 frozen exact-set drift remains `15` actual versus `12` expected and was not changed. One initial verification command referenced a nonexistent `scripts/verify-error-redaction.mjs`; it was corrected to the repository-owned `scripts/verify-sec12-error-log-boundary.mjs`, which reached the known exact-set failure after all preceding SEC-12 cases passed.
- Root cause and minimum product fix remain intentionally unclaimed until the Git-generated Preview targeted run returns the first safe differing path.
- First targeted invocation reached analyze HTTP 200 and created no saved report, but the diagnostic harness called the Playwright `APIResponse.headers` method as a Fetch `Headers` object and stopped with `HARNESS_FAILURE/unhandled_error`. Cleanup was vacuously complete with zero created reports. The harness was corrected to normalize both response-header interfaces before any second Preview attempt.
- Targeted run `premium-identical-retry-1785210374486` on Preview commit `d720be99a10a9aec99ade60918aa41a37594efe3` reproduced HTTP 409 and identified the first differing path as `conditionPlan.responses[0].action`; the next safe paths were `conditionKey`, `confidence`, and `escalationCriteria`. The saved row and `updated_at` remained unchanged, duplicate source tuples were zero, and the one created report was deleted with zero residue.
- Root cause: `mergeLegacyFallback()` treated every generated `conditionResponses` entry as legacy on the next canonical rebuild because public compatibility responses intentionally omit the `source` and `legacyCarryover` markers retained in `conditionPlan.responses`. Repeated canonicalization therefore changed response identity/order/shape and the snapshot fingerprint. The minimum fix reuses only explicitly marked legacy carryovers from a prior canonical `conditionPlan`; fresh canonical responses are rebuilt as canonical rather than promoted to legacy.
- The new execution verifier failed before the fix at the identical-retry assertion (`conflict` versus expected `existing`). It covers KO/EN, three repeated rebuilds, context and object-key stability, absent/explicit-empty/nonempty semantics, stored-locale authority, a meaningful changed-product conflict, and saved-snapshot immutability.
- After the fix, the new identical-retry verifier, snapshot diff/replay diagnostics, Premium decision state, current-product findings, report reentry, route/storage/reentry, analysis RLS audit, session payload/runtime diagnostics, browser journey local/contract checks, image sanitizer capacity, SEC-10 image origin `44/44`, architecture guard, production build, and diff hygiene passed.
- Two pre-existing direct-Node verifier limitations remain outside this fix: `verify-current-product-active-semantics.mjs` cannot resolve its existing `@/lib` alias, and `verify-sec06-saved-report-boundary.mjs` rejects its own negative control 5 due the previously recorded stale source-string contract. Neither failure was treated as a pass or changed in this commit.
- The first fixed-Preview targeted run still returned HTTP 409. Its initial diff paths included properties omitted by JSON serialization (`undefined` object members), even though the snapshot fingerprint is computed from `JSON.stringify`. The differential helper was corrected to mirror JSON fingerprint semantics for omitted object properties, array `undefined`/holes, and non-finite numbers before selecting the next actionable path; no snapshot or replay classification behavior changed.
- A follow-up Preview still reported the same path names, so the opt-in diagnostic header was extended with the already value-free diff contract (types, presence, counts, and SHA-256 hashes) for the four bounded paths. This changes no production response body or replay behavior and remains inactive outside Preview with a valid correlation ID.
- Targeted run `premium-identical-retry-1785211031907` on Preview commit `6aa62b442d5318283fed5f19b87cf7526aab62c8` returned HTTP 409 with the first exact safe difference `consistency` (`null` versus object). The paired paths showed the same object alternating between top-level `consistency` and `decisionBundle.consistency`; later shared nodes likewise became null under `decisionBundle.context.conditionSignalState` and `decisionBundle.functionalPlan.budgetAlternatives[0]`. The saved row and `updated_at` remained unchanged, duplicate source tuples were zero, and cleanup deleted the one created report with zero residue.
- Final root cause: `sanitizePurchaseLinkPayload()` kept every visited object in one global `WeakSet` for the entire traversal. Canonical Premium reports intentionally contain shared, non-cyclic object references, so the sanitizer misclassified the second path to each shared object as a cycle and replaced it with null. Rebuilding the decision state changed reference placement, making the first and retry boundary payloads hash differently. The minimum fix treats `seen` as the active recursion stack by removing each object in `finally`; true cycles, depth/array/object limits, dangerous-key removal, and purchase-link stripping remain fail-closed.
- The shared-node execution regression failed before the fix with the second `consistency.status` missing and passes after it, while the existing cyclic regression remains intact. The Premium identical-retry verifier and focused snapshot/decision/current-products/reentry/storage checks pass after the fix.
- Validation command correction: an attempted aggregate used nonexistent package script aliases (`verify:premium-snapshot`, `verify:premium-decision-state`, `verify:premium-report-reentry`, `verify:premium-report-session`, `verify:premium-browser-journey-local`, `verify:image-sanitizer-capacity`, `verify:image-source-policy`, `verify:purchase-link`, and `verify:architecture`) and a nonexistent `scripts/verify-analysis-rls-audit.mjs`. Repository-owned script paths/package names are used for the authoritative rerun. The separate known SEC-07 `response_wiring` failure remains pending until the Hosted blocker is cleared.
- Fixed Preview targeted run `premium-identical-retry-1785211395123` passed on commit `9193dfa0f93d8170045c1786990532217bb0ff88`: analyze HTTP 200, one Premium cookie, first persistence `saved`, identical retry HTTP 200/`existing`, identical fingerprint and saved report, unchanged `updated_at`, zero duplicate source tuples, and cleanup deleted one created report with zero residue.
- The one authorized Hosted rerun `premium-e2e-20260728040357-6aacac60` completed KO/EN, replay, reopen, cross-account conflict, immutable conflict, rotation, independent second saves, secret scan, and cleanup with `HOSTED_PREVIEW_PASS`. Four reports were created and all four were deleted; cleanup completed with zero reported residue.

### 2026-07-28 / SEC-07 execution wiring and SEC-12 terminal exact-set audit

- SEC-07's runtime sanitizer contract was already intact; only `response_wiring` depended on the obsolete inline source shape. The verifier now parses the routes, proves the analyze Premium-session dataflow from purchase sanitizer through image sanitizer into the session payload, proves the public response's nested sanitizer chain, and proves both saved-report and session full-report responses call the shared boundary helper. Existing execution fixtures still cover direct, fallback, unsafe, nested Premium, and free-result nodes. SEC-07 passes `42/42`; no route or sanitizer behavior was weakened.
- SEC-12's initially visible Full Report drift was `15` terminal return nodes versus frozen `12`. The exact comparison against the frozen baseline found four intentional additions and one removal (net `+3`), all integrated by merge `219359714124e4a92360b77b4e11d79b2cbfd0d6`: principal-conflict rejection, missing account-storage rejection, finalized-snapshot lookup storage failure, and finalized replay existing/conflict; the saved-report-only image validation was removed when canonical image validation moved before the saved/session branch. The paths are reachable terminal responses, distinct rather than duplicate, and preserve authentication, storage, and immutable-snapshot boundaries.
- Continuing the audit after the first mismatch exposed a second hidden drift in Full Report Session POST: `8` terminal nodes versus frozen `6`. Principal conflict and general missing-session handling were split, and Premium creation denial became a distinct path (three current paths replacing one baseline path, net `+2`), also integrated by `219359714124e4a92360b77b4e11d79b2cbfd0d6`. These are intentional rotation authorization boundaries exercised by the Premium journey contracts.
- The SEC-12 manifest now freezes `125` total terminal response nodes and adds semantic exact multisets for the two changed handlers, so same-count removal/addition drift is rejected instead of relying on counts. The Preview-only diagnostic header decorator is audited as a response pass-through; both its inactive and active returns preserve the already no-store response. SEC-12 passes `62/62`, all `125/125/125` paths are verified, unsafe/unresolved/dead-helper counts are zero, and the mutation suite rejects `37/37`, including a new same-count Full Report terminal substitution.

### 2026-07-29 / Mobile fullscreen camera UX

- Task type: Medium-risk bounded UI execution. DB, Auth, RLS, Storage, Provider, Payment, Secret, Production, API response fields, saved-data structure, deployment configuration, and package dependencies were `N`.
- Branch: `feature/mobile-camera-fullscreen`, created from fetched `origin/main` HEAD `1ea3b0d79f094fbf358a0de740b212acc3415f61`.
- Implementation: the existing `PhotoUploadStep` stream, canvas capture, file-selection flow, and front-camera mirror contract remain authoritative. Below the existing `lg` breakpoint, the measured capture circle expands through a Framer Motion portal with a 14% press keyframe, fixed `100dvw`/`100dvh` camera surface, safe-area controls, responsive dimmed oval guide, scroll lock, reverse close, focus restoration, reduced-motion handling, transition fallback, request/capture race guards, and idempotent track cleanup. Desktop retains the existing in-card camera.
- Verification: focused Playwright camera mock scenarios passed `7/7` for opening phases, close/cleanup, re-entry, full-resolution capture, denied permission, small portrait plus resize, reduced motion, gallery selection, and desktop regression. Image upload boundary passed `55/55`; architecture guard, production build, and diff check passed.
- Error log: the first focused Playwright batch clicked server-rendered controls before client hydration in later cases, producing four false-negative timeouts. The tests now wait for local network idle before interaction and pass repeatedly. `npm run lint` could not execute because this checkout has no ESLint dependency/config and `next lint` opened its interactive first-time setup prompt; no package/config change was made. A visual dev-server check also observed the existing `app/layout.js` nonce hydration warning (`nonce` server value versus empty client attribute), outside this UI change.
- Data/change control: no protected surface, existing `.codex-worktrees/`, environment file, API, DB, stage, commit, push, merge, or deployment was changed.

### 2026-07-29 / Mobile camera history close and first-frame transition

- Task type: Medium-risk bounded UI execution. Only `PhotoUploadStep`, `MobileFullscreenCamera`, the existing mobile-camera Playwright spec, and this work log changed; DB, Auth, API, saved-data structure, package dependencies, deployment configuration, and existing `.codex-worktrees/` were not changed.
- Camera opening now records one same-URL, namespaced history entry per mobile request cycle. A `popstate` consumes that entry, cancels the active request, and uses the same idempotent close lifecycle as X/capture: track stop, reverse animation where applicable, body-scroll restoration, history-listener removal, and capture-button focus restoration. X removes its entry through `history.back()` without leaving a later empty camera entry.
- The fullscreen portal stays opacity-zero and non-interactive while permission and stream preparation run. The stream is attached to the video element and expansion begins only after `video.play()`, `loadeddata`/`canplay`/`playing`, `readyState >= HAVE_CURRENT_DATA`, intrinsic dimensions, and one paint frame. Abort-aware listeners, timeout cleanup, request tokens, and stale-stream track stopping cover pending permission, close, unmount, denied permission, and first-frame timeout.
- Verification: focused Playwright passed `11/11` (existing open/close/re-entry, capture resolution, denied permission, small viewport/resize, reduced motion, gallery, desktop, plus browser-back close, X history cleanup, delayed-first-frame gating, and pending-request stale-stream cleanup). SEC-08 image boundary passed `55/55`; architecture guard and production build passed; `git diff --check` passed.
- Error log: the first focused Playwright batch had one test-only matcher error (`expect(locator).toEvaluate` is unavailable in this Playwright version). It was replaced with `expect.poll(locator.evaluate(...))`; the rerun passed `11/11`. `npm run lint` remains unavailable without entering the existing interactive Next ESLint setup prompt; no lint configuration or dependency was added. `agent-browser` is not installed in this environment, so browser verification used the repository's Playwright suite.

### 2026-07-30 / MediaPipe same-origin asset integrity and real runtime verification

- Task type: Medium-risk bounded runtime repair on `feature/mediapipe-face-guide`. The existing same-origin MediaPipe policy, pinned `@mediapipe/tasks-vision@0.10.35`, model, face evaluator, capture thresholds, CSP, Auth, DB, API response fields outside the MediaPipe asset route, environment files, and deployment configuration were not changed.
- Confirmed root cause: the MediaPipe proxy passed the upstream compressed `Content-Length` through while Node fetch returned a decoded body. The old Preview returned HTTP 200 but truncated SIMD JS to `78,444/322,044` bytes, SIMD WASM to `2,987,039/11,153,617`, no-SIMD JS to `78,368/321,847`, and no-SIMD WASM to `2,901,534/10,481,398`. Real Chromium then raised `SyntaxError: Unexpected end of input` and the hook eventually published `cpu_delegate_initialization/unavailable`. The first attempt waited for the damaged loader fetch/parse; browser/server cache made reopen fail sooner. Existing instance cleanup and CPU recovery could not repair identically corrupted runtime assets.
- Repair: the allowlisted asset route now buffers the decoded body, validates the pinned byte count, rejects empty/oversized/HTML/malformed-WASM responses, and emits a recomputed decoded `Content-Length` plus a bounded asset-byte header. The client preserves the first recognized browser WASM/script exception through MediaPipe's generic wrapper, and the hook exposes only allowlisted stage/category/code/name/delegate/attempt/recovery/capability diagnostics in DOM attributes and a safe terminal console object. Raw exception text, URLs, image data, tokens, and user data are not exposed. CSP and security headers were unchanged because real local runtime execution produced no CSP violation.
- Real runtime verification: a Playwright suite with no `window.__bejewelyFaceLandmarkerFactory` loaded the application package, same-origin WASM/model, created a real Face Landmarker, ran `detectForVideo()` on a continuously painted blank canvas video, closed/recreated it, and returned `faceLandmarks: []`. A second path loaded the pinned real bundle and explicitly created/ran the CPU delegate. Complete asset byte contracts, truncated loader propagation, WASM 404, model 404, and safe UI diagnostics passed. The existing mock GPU-inference-to-CPU recovery regression and all mobile camera regressions also passed.
- Verification: `npm ci`, JavaScript syntax `356/356`, face evaluator plus asset-route unit tests `7/7`, architecture guard, production build, real runtime Playwright `6/6`, mock recovery plus mobile camera Playwright `13/13`, and `git diff --check` passed. `npm run lint` opened the repository's existing interactive first-time ESLint setup, so it was stopped without creating configuration or dependencies. The aggregate security closeout suite stopped at its unrelated evaluator-boundary prep because the current fixture reports `needs_more_evidence_before_design` while the verifier expects `blocked_by_source_unavailability`; the focused failing verifier reproduced the same pre-existing assertion and no changed file participates in that evaluator boundary.
- Error log: an initial PowerShell asset-audit formatting command ended with an empty pipeline and was corrected. One search command had an unterminated PowerShell quote and was corrected. The first real-runtime assertions incorrectly required CPU even though this Chromium could initialize the real GPU delegate despite GPU-disable flags; the success assertion now accepts the delegate actually selected, while a separate real CPU create/inference test verifies CPU execution. The first corrupted-loader test saw MediaPipe wrap the browser `SyntaxError` as `Error`; a scoped browser error capture preserves the recognized parse exception without retaining its message. Existing mobile fixtures painted only one frame, freezing evaluations made during the opening transform; the fixture now emits continuous frames, and stale copy/manual-auto-capture timing assertions were aligned with the current UI contract before the complete suite passed. The first two pushed workflow revisions failed before jobs were created because GitHub's path-filter glob parser treated the literal `[...asset]` segment as glob syntax even after YAML quoting; the filter now uses the bounded `app/api/mediapipe/**` directory pattern.
- Deployment: Vercel reported the free-plan daily deployment limit (`api-deployments-free-per-day`, more than 100 deployments). Per user direction, local verification, commit/push, Draft PR update, and CI continue, but an exact-SHA READY Preview cannot be claimed until that external limit resets.
- Hosted evidence: Vercel completed the product-equivalent `21117173206f01665e5d534d60c3280c18e88623` deployment before the later workflow-only commits. Its home, model, loader, and WASM returned HTTP 200 with exact decoded byte counts and asset-byte headers; real browser initialization, blank-frame inference, close/reopen, explicit CPU inference, and all safe failure-stage cases passed. Vercel omitted `Content-Length` from Playwright's hosted response headers while preserving the exact body and route-owned byte header, so the browser contract treats that platform-managed header as optional while the direct route unit test continues to require its exact emitted value.

### 2026-08-02 / Local MediaPipe module-resolution diagnosis

- Task type: diagnosis; DB, Auth, RLS, Storage, Provider, Payment, Secret, and Production impact were `N`. The development build error at `lib/face-guide/face-landmarker-client.js` is caused by local dependency drift: `package.json` and `package-lock.json` declare pinned `@mediapipe/tasks-vision@0.10.35`, but `node_modules/@mediapipe/tasks-vision/package.json` is absent and `npm ls @mediapipe/tasks-vision --depth=0` reports `(empty)`. No application source or dependency declaration was modified; reinstall the lockfile dependencies before restarting the local server.

### 2026-08-02 / Dependency high-severity remediation

- Task type: Medium-risk bounded security dependency execution. DB, Auth, RLS, Storage, Provider, Payment, Secret, and Production execution were `N`; no hosted target, environment file, API contract, schema, or application source was changed. `next` was raised from `15.5.18` to `15.5.22`, `sharp` from `0.34.5` to `0.35.3`, and the existing dependency override now references the direct secure `postcss` and `sharp` declarations so Next cannot retain nested vulnerable copies. `sharp@0.35.3` requires Node `>=20.9.0`; the checked CI workflows use Node 20 and this workspace uses Node 24.14.0.
- Verification: `npm ci`, full and production-only `npm audit` each report `0` vulnerabilities; `npm ls` resolves Next's PostCSS and Sharp to the secure deduped versions; direct Next production build passed; `npm run architecture:guard` and `git diff --check` passed. `npm run build` itself failed only before the build phase with the local npm script shell reporting `next` as unrecognized, although `node node_modules/next/dist/bin/next build` passed with the same installed dependency tree. No repository configuration was changed for that environment-specific script-shell condition.

### 2026-08-02 / Mobile survey floating next shortcut

- Task type: Medium-risk bounded UI execution. Only `SurveyFlow`, the existing Visuali Playwright spec, and this work log changed; DB, Auth, RLS, Storage, Provider, Payment, Secret, Production, API/data contracts, survey schema/questions/options/copy, desktop layout, global scroll architecture, dependencies, and deployment settings were `N`.
- The mobile-only helper is a 44px right-side coral/pink arrow button with a safe-area-aware inset, accessible localized name, focus ring, reduced-motion-respecting 160ms entry, and no companion previous control. It is mounted only for a valid non-final question when the existing footer next button is less than 75% visible; an unavailable observer or failed initial measurement leaves it hidden. It calls the existing `handleNext`, which remains the single validation, answer-persistence, state-transition, and scroll path; a ref-backed pending guard prevents duplicate advance clicks.
- Verification: Next production build passed; focused Playwright survey navigation plus mobile floating-next scenarios passed `2/2`, covering pre-answer absence, out-of-view appearance, footer-in-view disappearance, floating action advancing exactly one question, desktop suppression, keyboard focus, 44px dimensions, and no horizontal overflow. Architecture guard and `git diff --check` passed. `npm run lint` could not run because this repository still has no ESLint configuration and Next opened its existing interactive setup prompt; no lint config or dependency was created. `agent-browser` is unavailable in this environment, so visual runtime coverage used the existing Playwright browser suite.

## 2026-08-04 — ADMIN-PRODUCT-INTEGRATION-1

- Type: execution with DB/Auth/RLS protected-surface static integration.
- Delta: selected Product Candidate Reviews, export/intake, atomic confirm, and Admin Import UI semantics onto current main `4202bd2c9a83f276436e226aee9d9bbc9ace2a8f` from audited source `d089bff52f5a48e11acb5c190da78b8aa81db776`.
...
## 2026-08-09 — PF-3 local replay precondition failure

- Type: High-risk local-only DB execution on isolated worktree `test/product-fact-storage-local-replay-v1` at exact main `300c21ec51450395da57186f4f4299137f5fb133`. Hosted and Production targets were not accessed; the original dirty checkout was not modified.
- Clean replay: Supabase CLI 2.82.0 and Docker 29.6.1 started a unique disposable local stack. `supabase db reset --local --no-seed` copied and replayed all 36 canonical migrations, but stopped at the first migration, `20260410_safe_review_and_promotion_layer.sql`, because `public.products` does not exist (`SQLSTATE 42P01`, statement 12). The repository has no root `supabase/config.toml`, no tracked migration that creates `public.products`, and the authoritative pre-PF2 tree has the same baseline gap. PF-2 was not reached, so this is not classified as a PF-2 migration SQL defect.
- Static regression: `node --check scripts/verify-product-fact-storage-migration-v1.mjs` passed. The verifier execution failed before its expected summary because `removeNamedConstraint` matches LF-only lookaheads while this Windows checkout has CRLF migration text; the first observed failure was `negative fixture did not remove product_fact_subjects_validity_check` at line 445.
- Stop decision: clean replay is FAIL and the required canonical prestate cannot be reconstructed from production migration history without an unapproved baseline/bootstrap contract. Upgrade replay, rollback verification, PF-3 harness implementation, commit, push, and Draft PR were not performed. The disposable Supabase containers and volumes were stopped and removed; no tracked migration was modified.
- Recheck: a second authority preflight still resolved `origin/main` to the required SHA. The existing Admin Product Review foundation fixture was tested only as a runtime prehistory adapter: its unchanged form failed at the first canonical migration with `SQLSTATE 42P13` because `normalize_basic_text` changes the input parameter name, and a table/type-only runtime copy failed with `SQLSTATE 22P02` because the current fixture taxonomy lacks the historical `serum` enum value required by the first migration. These failures confirm that the Admin fixture is not an authoritative full-history baseline. The temporary stack, volumes, runtime copies, and logs were removed.

## 2026-08-10 — PF-3P replay provenance audit and CRLF repair

- Type: High-risk, repository-only provenance diagnosis plus bounded static-verifier repair on isolated worktree `test/product-fact-storage-local-replay-v1` at exact main `300c21ec51450395da57186f4f4299137f5fb133`. Hosted and Production databases were not accessed, and no production migration was modified.
- Provenance audit: searched all 142 local refs, SQL path history, deleted/renamed SQL artifacts, content history for `products`, `product_candidates`, schema/bootstrap/dump terms, and all unreachable commits/blobs. No canonical historical SQL or root `supabase/config.toml` was found. Commit `8c1f093c1b9fc2a9af9c86174093759bcdd700a5` and its expanded descendant `a3cb2c1a923974e4e448a3bc4e1ea0c53381b20e` contain a four-table local replay adapter, but their tracked headers and PR evidence explicitly classify it as replay-equivalent and not exact historical DDL; neither is an ancestor of authoritative main.
- Prehistory dependency inventory: the tracked chain assumes `public.products`, `public.product_candidates`, and `public.source_rankings` before `20260410_safe_review_and_promotion_layer.sql`, and `public.recommendation_logs` before `20260711032649_sec_05_anonymous_write_grants.sql`. The first migration requires the legacy product scalar columns it converts, UUID-compatible product/candidate identities, candidate source/name/timestamp fields, and ranking evidence fields. Exact historical types, defaults, constraints, indexes, privileges, and RLS provenance remain missing, so no baseline fixture or smoke replay was created. Verdict: `PF3_BASELINE_PROVENANCE_UNRESOLVED`.
- Config verdict: no historical root config was found. The prior PF-3 run and the repository-owned local replay template prove that a generated, isolated local-only config is sufficient; a tracked root config is not required for replay execution and is distinct from schema provenance.
- CRLF repair: the PF-2 verifier now normalizes CRLF/bare-CR to LF once at ingestion before all audits and mutation helpers. Deterministic LF and CRLF-derived audits execute the same 158 semantic assertions, the CRLF-derived named-constraint mutation is detected, the existing Git-scope checks preserve the reported 165 assertions, and all eight original negative cases remain detected. `node --check` and the verifier pass against the actual CRLF migration checkout.
- Scope stop: because authoritative baseline provenance is unresolved, no prehistory fixture, precondition verifier, disposable smoke replay, PF-3 full replay, PF-4 work, schema edit, or hosted action was performed.

## 2026-08-10 — PF-3 replay baseline authority decision

- Type: High-risk DB/migration authority design performed repository-only on isolated branch `design/product-fact-replay-baseline-authority-v1` from exact main `300c21ec51450395da57186f4f4299137f5fb133`. Hosted, Production, local DB replay, migrations, replay adapters, runtime, and PF-3P Draft PR #185 were not modified or accessed beyond read-only Git/PR evidence.
- Decision: exact historical predecessor DDL remains unavailable and is not required for Clean/Rollback. PF-3 adopts split authority: Clean uses a versioned governed replay-equivalent predecessor contract; Upgrade uses a stronger deterministic pre-PF2 repository-derived schema/data fingerprint; Rollback uses a minimal PF-2 prerequisite-compatible governed prestate. None may claim historical Production or Hosted identity.
- Existing adapter verdict: `ADAPTER_REQUIRES_REVISION`. The historical local adapter covers the four known predecessor relations and is honest about its status, but it is not on authoritative main and lacks an immutable version/digest, independent prerequisite classification, mutation guards, separated mid-chain bridge governance, and the pre-PF2 Upgrade fingerprint.
- State boundary: `PF3_REPLAY_BASELINE_AUTHORITY_DECIDED = YES`; Clean, Upgrade, and Rollback authority models are approved, but no baseline package or replay evidence exists. `PF3_LOCAL_DB_REPLAY_VERIFIED = NO` and `PRODUCT_FACT_LOCAL_MIGRATION_REPLAY_NOT_VERIFIED = UNRESOLVED` remain unchanged. The next implementation gate is `PF-3B — Implement Governed Local Replay Baseline v1`.

## 2026-08-10 — SOLO-DIVERSITY-1 plan-derived Solo wave shape

- Type: Medium-risk bounded Solo contract/runtime/test extension. Provider, T4, human screening, target reveal, checkpoint, Hosted, Production, DB, Auth, RLS, Storage service, Payment, and Secret impact were `N`.
- Root cause: Solo source preflight, private-map integrity, session validation, and assessment-set validation derived cardinality from legacy wave ordinals (`4/8/8`) instead of the stored Campaign Plan. Diversified Wave 1 therefore rejected its authoritative eight slots before creating a session.
- Contract: immutable `solo-wave-shape-v1` derives slot and A/B/C/D counts from the verified plan. Non-legacy shapes use additive session/private-map/assessment-set v2 artifacts bound to the plan digest, shape digest, private map, and authoritative slot-set digest. Legacy v1 schemas and fixed digest semantics remain unchanged.
- Verification: legacy Wave 1/2/3, diversified Wave 1, slot/distribution/ordinal/plan mismatch negatives, v1 digest regression, v2 storage round-trip, full `synthetic:verify` (`195` pass, `3` platform skips, `0` fail), architecture guard, and diff hygiene passed. Live Campaign `crun_140d7a156fb69f754db7a780` `prepare_wave --preflight` returned eight target-withheld review items with A2/B2/C2/D2 shape, `writesPerformed=0`, and zero run-specific Solo files before/after.

## 2026-08-12 — SOLO-ALIGNMENT-2 derived cue-alignment diagnostics

- Type: Medium-risk additive Solo contract/runtime/test extension. Provider, T4 execution, human screening, target reveal, operator decision, checkpoint, Hosted, Production, DB, Auth, RLS, Payment, Secret, and Campaign artifact writes were `N`.
- Root cause: `solo-intent-assessment-v1` required both derived target relation and operator-only usability/disposition/recommendation, while `solo-wave-brief-v1` exposed only a single combined relation count. Evidence comparison could not be persisted without fabricating an operator decision or losing cue-specific uncertainty denominators.
- Contract: added immutable `solo-cue-alignment-v1` rows and `solo-wave-alignment-report-v1`, bound to session, screening, reveal, candidate, observation, plan-derived wave shape, slot set, and source projection. Human/T4 target relations, Human-T4 agreement, unsupported T4 count semantics, deterministic diagnostic flags, typed limitations, and exact denominator arithmetic remain separate from all operator decisions. Existing v1 schemas/digests and target withholding are unchanged.
- Verification: uncertainty/exact/under/over, diagnostic classification, unsupported T4 count, denominator/duplicate/foreign/tamper negatives, legacy Solo digest behavior, append-only storage, full `synthetic:verify`, architecture guard, and diff hygiene passed. Live Campaign `crun_140d7a156fb69f754db7a780` alignment preflight failed closed with `solo_alignment_reveal_required`, `writesPerformed=0`; file count stayed `841`, reveal files `0`, and alignment files `0`.

## 2026-08-13 — FACE-EVAL-B Human annotation / consensus dataset contract v1

- Type: Medium-risk production-separated evaluation contract and documentation. DB, Auth, RLS, Storage service, Provider, Payment, Secret, Hosted, Production runtime, Synthetic local artifacts, and Human data execution were `N`.
- Contract: added exact-key pure validators for opaque Real Human review items, pseudonymous blind sessions, immutable ranked/ambiguous/uncertain/not-assessable annotations, independent sealed annotation sets, algorithm-versioned non-forced consensus, append-only adjudication, and subject/leakage-protected dataset manifests. Canonical payload and supplied SHA-256 verification bind each artifact without adding Node-only runtime or I/O to `@bejewely/face-contracts`.
- Boundary: raw paths/base64/PII are excluded, withdrawal blocks new annotation binding and active inclusion, synthetic evidence remains separate, and current Archetype registry/scorer/decision lifecycle, weights, thresholds, production API, storage, and UI are unchanged.
- Verification: focused positive/negative contract fixtures, digest/tamper and foreign-binding checks, blind/forced-label/privacy/withdrawal checks, consensus summary derivation, supersession conflict, and cross-split subject/leakage negatives passed; architecture guard, existing scorer verifier, workspace resolution, and diff hygiene were included in the final validation set.

## 2026-08-14 — V2.1-8E recovered source-gap Hosted adoption freeze

- Type: High-risk two-phase execution. Phase A is a repository-only deterministic adoption-plan freeze; Phase B is an explicitly authorized Hosted Product Fact controlled-RPC adoption after exact-head CI, squash merge, and exact-main closeout. DB, RLS, Auth-adjacent admin authority, Hosted, and Production verification are `Y`; no schema, migration, RLS, Registry, application runtime, package, recommendation, or Decision Axis production-consumption change is allowed.
- Authority: actual `origin/main` matched `da4e499b9b33af5a36c33e2c4c189462d731786b`. The three immutable V2.1-8D Git-blob/LF hashes matched `587afd...`, `9324d6...`, and `76e9d4...`. Read-only Hosted prestate matched all 12 required counts, M1 Subject/source/Current `0/0/0`, eight adopted products, exactly one active canonical `admin_owner`, and the three expected Product Fact migrations.
- Plan: exactly one ILLIYOON M1 Subject, one Amorepacific source, one `exact_subject_match/narrower` binding, two evidence records, and the exact `primary_use_role=multi_area` and `barrier_support_claim=true` propositions. Build A/B, proposition regeneration, pre/post count models, controlled RPC order, preflight/stale/idempotency requirements, historical 23 Current invariance, blocked-product isolation, business fingerprints, and security/production postconditions are frozen in six dedicated V2.1-8E files.
- Boundary: Phase A contains no Hosted write. Phase B may use only the five existing controlled RPCs and authoritative readback; direct Product Fact writes, DDL, migration/RLS changes, duplicate Subject/source creation, M3/P1/P2 adoption, recommendation activation, and V2.1-9 remain forbidden.

## 2026-08-14 — FACE-EVAL-CX1G-D2C-F target-axis operational definitions v1

- Type: Medium-risk evaluation-only contract freeze. DB, Auth, RLS, Storage service, Provider, Payment, Secret, Hosted, Production runtime, observation execution, generation execution, and Human judgment impact were `N`.
- Contract: froze visible anchors, within-face references, neighboring contrasts, ambiguity/not-assessable behavior, evidence tags, and retain/composite/decompose dispositions for the 11 W1 target axes. The deterministic reviewer-safe projection excludes generation, historical, target, Vision, and scorer metadata. Eight axes are ready for a future blind Human cue packet, two require validation, and `featureContrast` requires decomposition.
- Boundary and verification: production observation/generation/scoring consumption remains false; W1/W1M and D1/D2C history is not reinterpreted; W2 remains locked. Focused digest/parity/leakage/threshold/population guards, architecture guard, scorer and Human-evaluation verifiers, W1/W1M preflight recomputation, D1 integrity, and full synthetic verification (`201` pass, `3` platform skips, `0` fail) passed with zero Provider or Human execution.

## 2026-08-14 — FACE-EVAL-CX1G-D2D-P independent blind Human cue audit preparation

- Type: Medium-risk evaluation-only protocol, deterministic local packet builder, and verifier. DB, Auth, RLS, Storage service, Provider, Payment, Secret, Hosted, Production runtime, generation, observation, Human judgment, and consensus impact were `N`.
- Protocol: planned three genuinely independent reviewer slots over fourteen existing canonical synthetic candidates. Part A applies the same eight Human-ready axes to every image; Part B separately applies the same two validation-required axes. Direct `featureContrast` judgment remains excluded pending decomposition. Blank response templates preserve `uncertain` and `not_assessable`, confidence constraints, evidence tags, independence attestation, and target-withheld reveal ordering without creating Human evidence.
- Local boundary: reviewer assets, opaque permutations, packet manifests, blank templates, private candidate/observation map, and inventories remain under ignored `.synthetic-local`. All fourteen candidate/spec/prompt/observation bindings and unique canonical hashes were verified; canonical PNGs contained no semantic metadata requiring removal, and 14/14 decoded pixel digests matched. A second build produced identical IDs, order, packet files, private map, and authority digests. Packets were not distributed and no Human identity was bound.
- Production boundary: current observer, generation compiler, scorer, weights, thresholds, taxonomy, W1/W1M history, and D2C-F authority remain unchanged; W2 remains locked. Focused contract/local packet verification, D2C-F verification, architecture/scorer/Human-evaluation regressions, full synthetic verification, and diff hygiene form the completion gate.

## 2026-08-14 — FACE-EVAL-CX1G-D2D-UI1 Korean offline reviewer UI

- Type: Medium-risk evaluation-only UI and local distribution projection. Storage and Secret handling are `Y` only for the local transfer archive and authenticated Google Drive upload; DB, Auth, RLS, Provider, Payment, Production runtime, generation, observation, Human judgment, and consensus execution are `N`.
...
- Type: High-risk governed Admin/Product Fact identity action for #541. Base main at implementation start: `cc029c0f1c6d36304049570641193f94335ed7a3`. Branch exact-head before work-log append: `6104b95c87e4933fec285321b94ee7bde7cc445c`.
- Authority: queue visibility remains `admin.products.read`; Subject action requires authenticated `admin.products.review`. Phase 5-B does not create a second Subject writer: the only Product Fact Subject mutation is delegated to existing service-role-only `admin_register_product_fact_subject_v1`, followed by existing canonical `process_catalog_trust_product_v1` reconciliation.
- Identity contract: exact deployed `product-fact-subject-identity-v1` seven-field serializer is preserved. Without reviewed semantic variant evidence, `variant_key=null`. The formulation revision is an opaque `trust-phase5-review-*` key derived deterministically from reviewed frozen catalog identity evidence; administrative reviewer/time metadata is excluded from formulation identity but retained in proposal provenance/stale-preflight binding.
- Fail-closed behavior: exact task/intake/candidate lineage, catalog review/adoption contract continuity, official-provider convergence, proposal digest, mutable preflight state, semantic-key replay, and competing current product/market Subject drift are checked. Exact retries remain idempotent only for the same governed proposal.
- Boundary: no direct Product Fact DML, no Evidence adoption, no Product Fact confirmation, no Recommendation mutation, no catalog/Product write, and no Production Subject registration was executed by this implementation step.
- Verification: exact-head workflow `35413414743` PASS. Focused Phase 5-B verifier: 90 assertions PASS; existing Subject authority verifier: 38 assertions; canonical TRUST resolver/presentation + Phase 5-A queue regression PASS; architecture guard PASS; production Next.js build PASS; isolated Phase 1/2 Supabase runtime PASS; exact-head diff hygiene PASS.
