...
### 2026-07-15 / Face Lab and Skin Match image eligibility hard gate

- Task type: diagnosis followed by bounded High-risk execution. Provider-facing prompt and API result-contract impact were `Y`; DB, Auth, RLS, Storage, Payment, Secret, migration, deployment, and hosted execution were `N`.
- Root cause: both existing Vision routes asked directly for downstream analysis. Face Lab accepted any structurally complete model response with text evidence, and Skin Match normalized arbitrary model signals/observations. A product or non-photorealistic image could therefore pass with hallucinated evidence, while a refusal sentence could be retained as a photo observation and displayed as a signal.
- Minimal architecture: no preflight request was added. `/api/analyze` and `/api/face-reading` keep their existing parallel Vision calls, independently return the same eligibility shape, and share one deterministic prompt/validator helper. Missing, malformed, inconsistent, or evidence-free eligibility fails closed.
- Gate behavior: exactly one photorealistic human face is required. Face Lab and skin eligibility remain independent. Ineligible Face Lab responses become unavailable before raw mood/color/hair/style normalization. Ineligible Skin Match photo data receives zero photo weights, empty evidence/signals, survey-only interpretation, and structured failure state; model refusal text is discarded.
- Compatibility: new Face Lab envelopes with explicit ineligibility cannot be unwrapped as available, while legacy envelopes remain readable under the existing envelope contract. Skin eligibility is preserved in the free response and premium session JSON without a DB schema change.
- Verification: `node scripts/verify-image-analysis-eligibility.mjs`, `node scripts/verify-face-lab-keyword-summary.mjs`, `node scripts/verify-analysis-request-guard.mjs`, `node scripts/verify-provider-runtime-log-sanitization.mjs`, `npm run build`, `npm run architecture:guard`, and `git diff --check` passed. The architecture guard emitted its expected review warning and the related contracts were updated.
- Unperformed: no live OpenAI image fixture set, hosted API, production runtime, Supabase, DB, Storage, or deployment was invoked. Product/illustration/animal/document and difficult real-face fixtures remain provider-level smoke checks.

### 2026-07-15 / Face Lab photo-ineligible summary and premium legacy boundary

- Task type: bounded High-risk execution following the completed compatibility diagnosis. API persistence behavior was narrowed without changing DB schema, auth, RLS, storage policy, provider calls, secrets, deployment settings, or saved-report ownership checks.
- Free result: the original Face Lab envelope is classified before unwrapping. Only the explicit Vision photo-failure reason allowlist produces `photo_ineligible`; provider, API, malformed eligibility, unknown, and insufficient-feature failures remain unavailable. The diagnosis card receives this state separately from preview values and shows one fixed failure message without mood, keyword, style, fallback, placeholder, carousel arrows, or dots.
...
### 2026-07-27 / Mobile analyze browser flow

- Task type: Medium execution. Added Pixel 5 Playwright coverage for the real English onboarding and result pages without changing the production API, Provider orchestration, DB, migrations, fixtures, or response contracts.
- Coverage: independent success, Face Lab unavailable/photo-ineligible, and bounded HTTP 503 scenarios each dispatch exactly one multipart `POST /api/analyze`; loading, duplicate-submit blocking, retry availability, result states, and mobile overflow are asserted.
- Isolation: the suite uses a localhost-only browser environment and intercepted bounded responses. External network, real Provider calls, page errors, unexpected console errors, and horizontal overflow all remain zero.
- Verification: mobile Playwright completed 3 passed with retries disabled. The Face Lab package verifier now normalizes CRLF/LF only for workflow text inspection, preserving its semantic checks.
