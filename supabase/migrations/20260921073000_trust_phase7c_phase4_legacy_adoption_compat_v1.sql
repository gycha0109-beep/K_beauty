...
-- TRUST Phase 7-C / Phase 4 legacy adoption compatibility.
-- Narrowly extends the existing Phase 4 planning gate for Phase 7-B materialized
-- legacy exact Subjects and Phase 7-C controlled official-source reviews.
-- Non-legacy identity/source behavior and the explicit-confirmation boundary remain unchanged.

do $$
begin
  if to_regprocedure('public.trust_phase7c_legacy_subject_scope_ready_v1(uuid)') is null
    or to_regclass('public.trust_official_source_binding_reviews') is null then
    raise exception 'trust_phase7c_phase4_compat_dependency_missing' using errcode='55000';
  end if;
end;
$$;
...
comment on function public.trust_phase4_build_adoption_plan_v1(uuid,uuid) is
  'TRUST Phase 4 adoption plan with Phase 7-C legacy exact-Subject and controlled official-source compatibility. Relational facts and final confirmation remain separately governed.';
