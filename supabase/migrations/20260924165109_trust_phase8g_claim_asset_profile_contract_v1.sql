  ...
  if p_baseline_kind = 'historical_replay' then
    ...
    if not (
         ...
         (
           ...
           and btrim(p_adapter_version) = 'v1'
           and coalesce(v_source.source_metadata ->> 'direct_claim_asset_url', '') ~ '^https://'
           and coalesce(
             nullif(btrim(v_source.source_metadata ->> 'current_direct_claim'), ''),
             nullif(btrim(v_source.source_metadata ->> 'direct_claim'), ''),
             nullif(btrim(v_source.source_metadata ->> 'observed_claim'), ''),
             ''
           ) <> ''
           and coalesce(p_canonical_baseline ->> 'claim_asset_url', '') =
               coalesce(v_source.source_metadata ->> 'direct_claim_asset_url', '')
           and coalesce((p_canonical_baseline ->> 'page_asset_binding_present')::boolean, false) is true
           and coalesce(p_canonical_baseline ->> 'page_final_url', '') ~ '^https://'
           and coalesce(p_canonical_baseline ->> 'asset_final_url', '') =
               coalesce(p_canonical_baseline ->> 'claim_asset_url', '')
           ...
           and coalesce((p_canonical_baseline ->> 'qualification_observations')::integer, 0) >= 3
           and lower(coalesce(p_canonical_baseline ->> 'qualification_asset_digest', '')) =
