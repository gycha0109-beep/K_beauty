...
create table if not exists public.ranking_snapshots (
  ...
  constraint ranking_snapshots_item_count_check check (item_count >= 0),
  constraint ranking_snapshots_status_check check (status in ('collected', 'ingested', 'failed')),
  constraint ranking_snapshots_ingest_key_not_blank check (btrim(ingest_key) <> '')
);
...
create index if not exists ranking_snapshots_snapshot_hash_idx
  on public.ranking_snapshots (snapshot_hash);

create index if not exists ranking_snapshots_job_collected_at_idx
  on public.ranking_snapshots (job_id, collected_at desc);
...
alter table public.source_rankings
  add column if not exists snapshot_id uuid references public.ranking_snapshots(id),
  add column if not exists candidate_id uuid references public.product_candidates(id),
  add column if not exists raw_item jsonb;

create unique index if not exists source_rankings_snapshot_rank_position_key
  on public.source_rankings (snapshot_id, rank_position)
  where snapshot_id is not null;

create unique index if not exists source_rankings_snapshot_candidate_id_key
  on public.source_rankings (snapshot_id, candidate_id)
  where snapshot_id is not null
    and candidate_id is not null;

create index if not exists source_rankings_snapshot_id_idx
  on public.source_rankings (snapshot_id);

create index if not exists source_rankings_candidate_id_idx
  on public.source_rankings (candidate_id);

revoke insert, update, delete on table public.source_rankings from anon, authenticated;
grant select, insert on table public.source_rankings to service_role;
...
alter table public.product_candidates
  ...
  add column if not exists latest_raw_source jsonb;
...
set first_seen_at = coalesce(first_seen_at, created_at, now()),
    ...
    seen_count = greatest(coalesce(seen_count, 0), 1)
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'product_candidates_seen_count_check'
      and conrelid = 'public.product_candidates'::regclass
  ) then
    alter table public.product_candidates
      add constraint product_candidates_seen_count_check check (seen_count >= 0);
  end if;
end $$;

create unique index if not exists product_candidates_source_external_key
...
create index if not exists product_candidates_source_normalized_name_idx
  on public.product_candidates (source_name, normalized_brand, normalized_name);

revoke insert, update, delete on table public.product_candidates from anon, authenticated;
grant select, insert, update on table public.product_candidates to service_role;
...
comment on table public.ranking_snapshots is
  'Raw ranking collection snapshots. Phase 1 observational data only; does not modify products.';
...
comment on column public.product_candidates.external_id is
  'Source-native product identifier. When present, source_name + external_type + external_id is the canonical candidate upsert key.';

comment on index public.product_candidates_source_normalized_name_idx is
  'Fallback lookup support for candidates without external_id. This is intentionally not unique because older category-scoped candidates may already exist.';
