...
async function main(): Promise<void> {
  ...
  const counts = {
    ...
    manifest_conflict: rows.filter((row) => row.status === "manifest_conflict").length,
  };
  const blockers = counts.stale_product + counts.identity_conflict + counts.manifest_conflict;
  ...
  console.log(`- manifest_conflict: ${counts.manifest_conflict}`);
