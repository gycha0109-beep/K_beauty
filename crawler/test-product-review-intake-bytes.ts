...
async function main(): Promise<void> {
  ...
  const temporaryRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "bejewely-review-intake-bytes-"),
  );

  try {
    const directory = await writeReviewExportBatch(
      temporaryRoot,
      "batch",
      fixture.batch,
      false,
    ...
    assert.equal(parsed.directory, "<memory>");
    assert.equal(parsed.reviewedRows.length, fixture.reviewedRows.length);
    assert.equal(
      parsed.batch.export_batch_id,
      fixture.batch.batch.export_batch_id,
    );
    ...
    expectCode(
      () =>
        parseReviewedBatchFiles({
          ...files,
          batch: Buffer.from([0x7b, 0x00, 0x7d]),
        }),
    ...
    expectCode(
      () =>
        parseReviewedBatchFiles({
          ...files,
          evidence: Buffer.alloc(MAX_EVIDENCE_FILE_BYTES + 1),
        }),
      "review_evidence_unreadable",
    );

    const tamperedManifest = Buffer.from(files.manifest);
    tamperedManifest[tamperedManifest.length - 1] ^= 1;
    expectCode(
      () =>
        parseReviewedBatchFiles({
          ...files,
