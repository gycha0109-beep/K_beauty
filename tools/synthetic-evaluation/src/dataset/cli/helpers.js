...
export function dataRoot() { return path.resolve(process.env.BEJEWELY_SYNTHETIC_DATA_ROOT || path.join(process.cwd(), ".synthetic-local")); }

export async function readRequest(root, relativePath, field) {
...
export function print(value) { process.stdout.write(`${stableStringify(value)}\n`); }
export function fail(error) { print({ ok: false, errors: [{ code: error?.code || "cli_execution_failed", path: "$", detail: null }] }); process.exitCode = 1; }
