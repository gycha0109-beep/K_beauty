...
function uniqueStrings(value, pattern = null) {
  return Array.isArray(value) && value.every((item) => typeof item === "string" && (!pattern || pattern.test(item))) && new Set(value).size === value.length;
}
...
function validateRate(value) {
  return exactKeys(value, ["numerator", "denominator", "fractionLabel", "percent"]) &&
    ...
    value.fractionLabel === `${value.numerator}/${value.denominator}` &&
...
export function validateCampaignEvidenceSnapshot(value) {
  ...
  if (!Array.isArray(value.sourceRuns) || value.sourceRuns.length < 1 || value.sourceRuns.length > 2 || value.sourceRuns.some((run) => !exactKeys(run, ["campaignRunId", "campaignPlanDigest", "finalProjectionDigest", "closeoutDigest", "comparisonGroupId", "providerProfileId", "closedAt"]) || !RUN_ID.test(run.campaignRunId || "") || !HEX64.test(run.campaignPlanDigest || "") || !HEX64.test(run.finalProjectionDigest || "") || !HEX64.test(run.closeoutDigest || "") || !(run.comparisonGroupId === null || TOKEN.test(run.comparisonGroupId || "")) || !TOKEN.test(run.providerProfileId || "") || !isIso(run.closedAt))) errors.push(error("report_source_snapshot_invalid", "sourceRuns"));
...
export function validateCampaignMetricSet(value) {
  const errors = [];
  const keys = ["schemaVersion", "sourceSnapshotDigest", "policy", "stageMetrics", "terminalOutcomes", "conditionSummaries", "failureGroups", "comparison", "metricSetDigest"];
  if (!exactKeys(value, keys)) return result([error("report_metric_set_invalid", "$")]);
  if (value.schemaVersion !== CAMPAIGN_METRIC_SET_SCHEMA_VERSION || !HEX64.test(value.sourceSnapshotDigest || "") || !HEX64.test(value.metricSetDigest || "")) errors.push(error("report_metric_set_invalid", "$"));
  ...
  if (!isObject(value.stageMetrics) || Object.keys(value.stageMetrics).sort().join(",") !== [...T8_STAGE_METRICS].sort().join(",") || Object.values(value.stageMetrics).some((rate) => !validateRate(rate))) errors.push(error("report_metric_set_invalid", "stageMetrics"));
  if (!isObject(value.terminalOutcomes) || Object.keys(value.terminalOutcomes).sort().join(",") !== [...PILOT_TERMINAL_OUTCOMES].sort().join(",") || Object.values(value.terminalOutcomes).some((count) => !Number.isInteger(count) || count < 0) || Object.values(value.terminalOutcomes).reduce((sum, count) => sum + count, 0) !== 20 * value.policyRunCount) errors.push(error("report_metric_set_invalid", "terminalOutcomes"));
  if (!Array.isArray(value.conditionSummaries) || value.conditionSummaries.length !== 4 * value.policyRunCount) errors.push(error("report_metric_set_invalid", "conditionSummaries"));
  if (!isObject(value.failureGroups) || Object.values(value.failureGroups).some((count) => !Number.isInteger(count) || count < 0)) errors.push(error("report_metric_set_invalid", "failureGroups"));
  if (!(value.comparison === null || isObject(value.comparison))) errors.push(error("report_metric_set_invalid", "comparison"));
  return result(errors);
}
...
export function validateCampaignReport(value) {
  ...
  if (!exactKeys(value.scope, ["campaignRunIds", "comparisonGroupId", "primaryDenominatorPerRun", "closedAtByRun"]) || !uniqueStrings(value.scope?.campaignRunIds, RUN_ID) || value.scope.campaignRunIds.length < 1 || value.scope.campaignRunIds.length > 2 || !(value.scope.comparisonGroupId === null || TOKEN.test(value.scope.comparisonGroupId || "")) || value.scope.primaryDenominatorPerRun !== 20 || !isObject(value.scope.closedAtByRun) || Object.keys(value.scope.closedAtByRun).sort().join(",") !== [...value.scope.campaignRunIds].sort().join(",") || Object.values(value.scope.closedAtByRun).some((item) => !isIso(item))) errors.push(error("campaign_report_invalid", "scope"));
  ...
  if (!exactKeys(value.g4TimeBoundary, ["mode", "currentStatusAppendixIncluded", "statusVerifiedAt"]) || value.g4TimeBoundary.mode !== "as_of_closeout" || typeof value.g4TimeBoundary.currentStatusAppendixIncluded !== "boolean" || !(value.g4TimeBoundary.statusVerifiedAt === null || isIso(value.g4TimeBoundary.statusVerifiedAt))) errors.push(error("campaign_report_invalid", "g4TimeBoundary"));
...
export function validateCampaignExportManifest(value) {
  ...
  if (!Array.isArray(value.files) || value.files.length === 0 || value.files.some((file) => !exactKeys(file, ["relativePath", "mediaType", "sha256", "byteLength", "role"]) || typeof file.relativePath !== "string" || !SAFE_RELATIVE_PATH.test(file.relativePath) || typeof file.mediaType !== "string" || !HEX64.test(file.sha256 || "") || !Number.isInteger(file.byteLength) || file.byteLength < 0 || !T8_EXPORT_FILE_ROLES.includes(file.role))) errors.push(error("campaign_export_manifest_invalid", "files"));
