...
async function installBrowserGuards(
  ...
  page.on("console", (message) => {
    if (message.type() === "error") {
      ...
      if (!isExpectedAnalyze503NetworkDiagnostic) {
...
test.describe("mobile analyze browser flow", () => {
  ...
  test("selects a photo, shows loading, and renders the result with Face Lab", async ({
    ...
    const disabledObserved = page.evaluate(
      () =>
        new Promise<boolean>((resolve) => {
          const button = document.querySelector<HTMLButtonElement>('[data-testid="analyze-submit"]');
          if (!button) {
            resolve(false);
            return;
          }
    ...
    expect(await disabledObserved, "LOADING_OR_DUPLICATE_SUBMIT_FAILED").toBe(true);
  ...
  test("renders a bounded analyze error after one request", async ({ page }) => {
    ...
    responseGate.resolve();
    await expect(page.getByText("Analysis is temporarily unavailable.")).toBeVisible();
    await expect(page.getByRole("heading", { name: "Tell us your skin context" })).toBeVisible();
