...
-- Zero-write preflight and exact serializer replay.
do $$
declare
...
begin
  ...
  perform pg_temp.assert_true((v_preflight ->> 'open_assignment_count')::integer = 0, 'trust_p4_preflight_assignment_count');
  ...
  v_expected_prop := public.product_fact_controlled_sha256_json_v1(
    jsonb_build_object(
  ...
  insert into trust_p4_results values (
    'preflight_zero_write_and_serializer', 'PASS',
    jsonb_build_object('proposition_key', v_expected_prop)
  );
end;
$$;

-- Non-READY candidates fail closed without writes.
do $$
declare
  v_actor constant uuid := '92000000-0000-4000-8000-000000000001';
  v_candidate constant uuid := '84000000-0000-4000-8000-000000000001';
  v_message text;
begin
  update public.trust_evidence_candidates set candidate_state = 'REVIEW_REQUIRED' where candidate_id = v_candidate;
  begin
    perform public.admin_preflight_trust_evidence_adoption_v1(v_actor, v_candidate);
  ...
  update public.trust_evidence_candidates set candidate_state = 'READY' where candidate_id = v_candidate;
  insert into trust_p4_results values ('non_ready_fail_closed','PASS',jsonb_build_object('message',v_message));
end;
$$;

-- Product/Subject lineage mismatch fails closed.
do $$
declare
  v_actor constant uuid := '92000000-0000-4000-8000-000000000001';
  v_candidate constant uuid := '84000000-0000-4000-8000-000000000001';
  v_original_product uuid;
  v_message text;
begin
  select product_id into v_original_product from public.trust_evidence_candidates where candidate_id=v_candidate;
  update public.trust_evidence_candidates
  set product_id = '00000000-0000-4000-8000-000000000305'
  ...
  update public.trust_evidence_candidates set product_id=v_original_product where candidate_id=v_candidate;
  insert into trust_p4_results values ('identity_mismatch_fail_closed','PASS',jsonb_build_object('message',v_message));
end;
$$;

-- Frozen source observation tampering is detected.
do $$
declare
  v_actor constant uuid := '92000000-0000-4000-8000-000000000001';
  v_candidate constant uuid := '84000000-0000-4000-8000-000000000001';
  v_observation uuid := '83000000-0000-4000-8000-000000000001';
  v_original jsonb;
  v_message text;
begin
  select observed_claim into v_original from public.trust_source_observations where observation_id=v_observation;
  update public.trust_source_observations
  set observed_claim = '{"claim":"tampered"}'::jsonb
  ...
  update public.trust_source_observations set observed_claim=v_original where observation_id=v_observation;
  insert into trust_p4_results values ('observation_digest_tamper_fail_closed','PASS',jsonb_build_object('message',v_message));
end;
$$;

-- Candidate digest tampering is detected.
do $$
declare
  v_actor constant uuid := '92000000-0000-4000-8000-000000000001';
  v_candidate constant uuid := '84000000-0000-4000-8000-000000000001';
  ...
  v_message text;
begin
  select canonical_evidence_digest into v_original from public.trust_evidence_candidates where candidate_id=v_candidate;
  update public.trust_evidence_candidates set canonical_evidence_digest=repeat('0',64) where candidate_id=v_candidate;
  begin
    perform public.admin_preflight_trust_evidence_adoption_v1(v_actor, v_candidate);
  ...
  update public.trust_evidence_candidates set canonical_evidence_digest=v_original where candidate_id=v_candidate;
  insert into trust_p4_results values ('candidate_digest_tamper_fail_closed','PASS',jsonb_build_object('message',v_message));
end;
$$;

-- Existing governed Evidence with the candidate digest but a different proposition
-- is a serializer/identity collision and must fail closed. Roll back this adversarial setup.
begin;
do $$
declare
  v_actor constant uuid := '92000000-0000-4000-8000-000000000001';
  ...
  v_bad_prop text := repeat('b',64);
  v_message text;
begin
  select subject_id, canonical_evidence_digest into v_subject, v_digest
  from public.trust_evidence_candidates where candidate_id=v_candidate;
  ...
  perform public.admin_ingest_product_fact_evidence_v1(
    ...
    jsonb_build_object(
      ...
      'evidence', jsonb_build_object(
        'registry_version','trust-phase4-fixture-registry-v1',
        'fact_key','phase4_fixture_claim',
        'proposition_key',v_bad_prop,
        'proposition_serializer_version','product-fact-proposition-pilot-v1',
        'proposition_value_identity','true'::jsonb,
        'parent_proposition_key',null,
  ...
  begin
    perform public.admin_preflight_trust_evidence_adoption_v1(v_actor, v_candidate);
    raise exception 'trust_p4_governed_collision_not_rejected';
  exception when others then
    get stacked diagnostics v_message = message_text;
    if v_message = 'trust_p4_governed_collision_not_rejected' then raise; end if;
    perform pg_temp.assert_true(v_message = 'trust_phase4_governed_evidence_collision', 'trust_p4_governed_collision_wrong_error');
  end;
end;
$$;
rollback;
...
-- Governed adoption reaches existing confirmation preflight but does not create Current.
...
declare
  ...
  v_evidence uuid := (v_result ->> 'governed_evidence_id')::uuid;
  v_assignment uuid := (v_result ->> 'assignment_id')::uuid;
begin
  ...
  perform pg_temp.assert_true((select count(*) from public.product_evidence_records where evidence_id=v_evidence)=1,'trust_p4_evidence_missing');
  perform pg_temp.assert_true((select count(*) from public.product_fact_review_assignments where assignment_id=v_assignment and operational_state='ready_for_confirm')=1,'trust_p4_assignment_not_ready');
  ...
  insert into trust_p4_results values ('governed_handoff_without_confirm','PASS',jsonb_build_object('evidence_id',v_evidence,'assignment_id',v_assignment));
end;
$$;

-- Exact replay reuses governed Evidence and the open assignment.
do $$
declare
...
-- Final confirmation remains an explicit separate call to the pre-existing authority.
do $$
declare
  v_actor constant uuid := '92000000-0000-4000-8000-000000000001';
  v_result jsonb := (select result from trust_p4_adoption);
...
-- Once Current exists, Phase 4 will not consume the candidate again.
do $$
declare
  v_message text;
begin
...
begin
  ...
  perform pg_temp.assert_true(not has_function_privilege('public','public.admin_adopt_trust_evidence_candidate_v1(uuid,text,uuid)','EXECUTE'),'trust_p4_public_adopt_acl');
...
select pg_temp.assert_true((select count(*) from trust_p4_results where status='PASS')=10,'trust_p4_required_result_count');

select 'TRUST_PHASE4_RUNTIME_JSON=' || jsonb_build_object(
  'status','PASS',
  'phase','TRUST_PHASE4_CONTROLLED_EVIDENCE_ADOPTION_RUNTIME',
  'tests',(
    select jsonb_agg(jsonb_build_object('name',test_name,'status',status,'detail',detail) order by test_name)
    from trust_p4_results
  ),
