...
function sanitizePremiumReport(report) {
  ...
  return {
    ...
    photoEvidenceState: sanitizeCanonicalDecisionArtifact(report.photoEvidenceState),
...
function buildFreeDecisionPayload(decision) {
  return {
    ...
    photoEvidenceState: sanitizeCanonicalDecisionArtifact(decision.photoEvidenceState),
  ...
  try {
    ...
    if (apiKey && canonicalImage.bytes) {
      try {
        ...
        const skinStatus = observationResult.bundle.skin?.status;
        const eligibility = observationResult.bundle.eligibility;
        photoEvidenceState = skinStatus === "available" && eligibility?.skinAnalysisEligible === true
          ? { status: "available", failureReason: null, failureClass: null, source: "vision" }
          : eligibility?.status === "ineligible"
            ? {
                status: "ineligible",
                failureReason: eligibility?.skinFailureReason || "unknown",
                failureClass: "input_ineligible",
                source: "vision"
              }
            : {
                status: "insufficient_evidence",
                failureReason: eligibility?.skinFailureReason || "image_quality_insufficient",
                failureClass: "input_insufficient",
                source: "vision"
              };
        photoAnalysis = { ...photoAnalysis, photoEvidenceState };
        ...
        photoAnalysis = { ...buildFallbackPhotoAnalysis(locale), photoEvidenceState };
      ...
      photoEvidenceState = {
        status: "unavailable",
        failureReason: apiKey ? "vision_request_failed" : "api_key_missing",
        failureClass: apiKey ? "provider_failure" : "provider_unavailable",
        source: "vision"
      };
      photoAnalysis = { ...photoAnalysis, photoEvidenceState };
