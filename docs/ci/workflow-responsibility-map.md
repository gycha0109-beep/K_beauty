...
The current repository has 92 migration files and 89 direct semantic-name matches against the observed Production ledger. It also has one audited historical migration-version collision: both `20260824_add_product_localized_names.sql` and `20260824_backfill_product_english_display_names.sql` use version `20260824`; this remains an explicit full-replay blocker rather than being silently normalized. Timestamp rewrites, the split DATA-TAXONOMY2 Production rollout, one Production-only emergency credential-revocation migration, and repository-only canonical replay/data migrations are recorded explicitly in `docs/ci/database-integration-authority.json` rather than being misclassified as generic drift. Supabase's documented `db reset` reproducibility model assumes a complete local project baseline; BEJEWELY does not claim that state yet.
