...
7. `shared-skin-decision-context` already has an options boundary and v4 already forwards that boundary to v3. Therefore the smallest justified architecture is to make the existing shared safety computation source-scale aware rather than duplicating safety semantics in a domain adapter.

## Architecture decision

Use a source-aware shared-context contract with a domain-scoped Skin Match boundary.
