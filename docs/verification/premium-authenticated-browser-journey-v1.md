...
Artifacts contain target metadata, hashed account and session identifiers, response contracts, saved-report IDs, fingerprints, version metadata, timestamps, and verdicts. They exclude credentials, cookies, authorization headers, email addresses, raw photos, and full report bodies. A secret scan is mandatory. If it detects secret material, response evidence is replaced with a minimal quarantine artifact set and scanned again.
